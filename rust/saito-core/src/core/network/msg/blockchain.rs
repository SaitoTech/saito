...
//!
//! There are no `(block_id, hash)` tuples. `payload_earliest_*` / `payload_latest_*`
//! still describe bounds for that chunk on the wire.
//!
//! ### Ordered payload (`reference_count > 0`)
//!
//! Each tuple is a `(block_id, block_hash)` the recipient may queue for fetch;
...
/// Byte length of a serialized [`RequestBlockchain`].
pub const REQUEST_BLOCKCHAIN_WIRE_LEN: usize = 8 + 32 + 32 + 1;
...
pub struct Blockchain {
    ...
    pub payload: Vec<(BlockId, BlockHash)>,
}

impl Blockchain {
    /// Total wire size for this payload (header + count + payload tuples).
    pub fn wire_len(&self) -> usize {
        BLOCKCHAIN_HEADER_WIRE_LEN + 4 + self.payload.len() * (8 + 32)
    }
}

impl Serialize<Self> for RequestBlockchain {
    fn serialize(&self) -> Vec<u8> {
        let mut out = Vec::with_capacity(REQUEST_BLOCKCHAIN_WIRE_LEN);
        ...
        debug_assert_eq!(out.len(), REQUEST_BLOCKCHAIN_WIRE_LEN);
        out
    }

    fn deserialize(buffer: &Vec<u8>) -> Result<Self, Error> {
        if buffer.len() != REQUEST_BLOCKCHAIN_WIRE_LEN {
            warn!(
                "Deserializing RequestBlockchain: expected {} bytes, got {}",
                REQUEST_BLOCKCHAIN_WIRE_LEN,
                buffer.len()
            );
            return Err(Error::from(ErrorKind::InvalidData));
        }
...
impl Serialize<Self> for Blockchain {
    fn serialize(&self) -> Vec<u8> {
        ...
        let mut out = Vec::with_capacity(BLOCKCHAIN_HEADER_WIRE_LEN + 4 + n * 40);
        out.extend_from_slice(&self.latest_known_block_id.to_be_bytes());
        out.extend_from_slice(self.latest_known_block_hash.as_slice());
        out.extend_from_slice(self.fork_id.as_slice());
    ...
    fn deserialize(buffer: &Vec<u8>) -> Result<Self, Error> {
        ...
        let body_len = BLOCKCHAIN_HEADER_WIRE_LEN + 4 + reference_count.saturating_mul(8 + 32);
...
mod tests {
    ...
    fn request_blockchain_roundtrip() {
        ...
        let buf = r.serialize();
        assert_eq!(buf.len(), REQUEST_BLOCKCHAIN_WIRE_LEN);
    ...
    fn request_blockchain_rejects_unknown_sync_type() {
        let mut buf = vec![0u8; REQUEST_BLOCKCHAIN_WIRE_LEN];
        buf[72] = 99;

...
impl GoldenTicket {
    ...
    pub fn deserialize_from_net(bytes: &[u8]) -> Result<GoldenTicket, Error> {
        if bytes.len() != 97 {
...
mod tests {
    ...
    #[test]
    fn deserialize_rejects_invalid_lengths() {
        assert!(GoldenTicket::deserialize_from_net(&vec![0; 96]).is_err());
        assert!(GoldenTicket::deserialize_from_net(&vec![0; 98]).is_err());
    }

...
function FormatInformation(formatInfo) {
  ...
  this.Equals = function (o) {
    var other = o;
    return (
      this.errorCorrectionLevel == other.errorCorrectionLevel && this.dataMask == other.dataMask
    );
  };
}
      ...
      for (var j = 0; j < s; j++) {
        if (i != j) {
          denominator = this.field.multiply(
            denominator,
            GF256.addOrSubtract(1, this.field.multiply(errorLocations[j], xiInverse))
          );
        }
      }
      ...
      for (var dy = 0; dy < areaHeight; dy++) {
        for (var dx = 0; dx < areaWidth; dx++) {
          bitmap[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] =
            grayScale[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] < middle[ax][ay]
              ? true
              : false;
        }
      }
...
function QRCodeDataBlockReader(blocks, version, numErrorCorrectionCode) {
  ...
  this.getNextBits = function (numBits) {
    ...
    if (numBits < this.bitPointer + 1) {
      ...
      var bitsFirstBlock =
        (this.blocks[this.blockPointer] & mask1) << (numBits - (this.bitPointer + 1));
      this.blockPointer++;
      ...
      var bitsThirdBlock =
        (this.blocks[this.blockPointer] & mask3) >> (8 - (numBits - (this.bitPointer + 1 + 8)));

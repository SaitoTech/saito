...
class YoutubeClient extends ModTemplate {
  ...
  getAllSupportedMimeTypes(...mediaTypes) {
    ...
    return [
      ...new Set(
        CONTAINERS.flatMap((ext) => mediaTypes.flatMap((mediaType) => [`${mediaType}/${ext}`]))
      ),
      ...new Set(
  ...
  visibilityChange() {
    salert(
      'You navigated away from this tab. To ensure the best streaming experience, please keep this tab in focus'
    );
  }
}

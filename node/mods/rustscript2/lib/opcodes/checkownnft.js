...
module.exports = {
  ...
  execute: function (node, context) {
    ...
    let utxokey1 = lookupField(node, 'utxokey1');
    let utxokey2 = lookupField(node, 'utxokey2');
    let utxokey3 = lookupField(node, 'utxokey3');

    if (!nftid) { return false; }
    if (isUnsetFieldValue(utxokey1) || isUnsetFieldValue(utxokey2) || isUnsetFieldValue(utxokey3)) {
      return false;
    }

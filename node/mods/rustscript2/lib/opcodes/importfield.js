...
`,
  ...
  execute(node, context) {
    ...
    const value = context.app.browser.resolveVarReference(
      context,
      lookupField(node, field_name)
    );
    const signature = context.app.browser.resolveVarReference(
      context,
      lookupField(node, 'signature')
    );

    if (isUnsetFieldValue(value) || isUnsetFieldValue(signature)) {
      return false;
    }

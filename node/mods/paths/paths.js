...
class PathsOfGlory extends GameTemplate {
  ...
  initializeGame(game_id) {
    ...
    this.importUnit('sb_army01', {
      ...
      rmovement: 3,
      checkSupplyStatus: (paths_self, spacekey) => {
        if (paths_self.game.spaces[spacekey].country == 'serbia') {
          return 1;
        }
      }
    });
  ...
  canPlayReinforcementPoints(faction = 'allies') {
    if (this.game.state.events.influenza) {
      return 0;
    }
    if (faction == 'allies') {
      if (this.game.state.allies_rounds.length > 0) {
  ...
  returnMobilizationDeck(type = 'all') {
    ...
    if (type == 'allies' || type == 'all') {
      deck['ap01'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.turn == 1) {
            return 0;
          }
          if (paths_self.game.state.allies_reinforcements_br > 0) {
            return 0;
          }
          return 1;
        },
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tallies_reinforcements_br\t' + paths_self.game.state.round
            );
      ...
      deck['ap03'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.state.allies_reinforcements_ru > 0) {
            return 0;
          }
          return 1;
        },
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tallies_reinforcements_ru\t' + paths_self.game.state.round
            );
      ...
      deck['ap04'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].ckey === 'RU') {
              return 1;
            }
          }
          for (let i = 0; i < defender_units.length; i++) {
            if (defender_units[i].ckey === 'RU') {
              return 1;
            }
          }
          return 0;
        ...
        onEvent: function (paths_self, faction) {
          ...
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].ckey === 'RU') {
              paths_self.game.state.combat.attacker_drm++;
              return 1;
            }
          }
          for (let i = 0; i < defender_units.length; i++) {
            if (defender_units[i].ckey === 'RU') {
              paths_self.game.state.combat.defender_drm++;
              return 1;
            }
          }
          return 1;
      ...
      deck['ap05'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].ckey === 'SB') {
              return 1;
            }
          }
          for (let i = 0; i < defender_units.length; i++) {
            if (defender_units[i].ckey === 'SB') {
              return 1;
            }
          }
          return 0;
      ...
      deck['ap09'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.turn <= 2) {
            return 1;
          }
          return 0;
        },
      ...
      deck['ap10'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.state.allies_reinforcements_fr > 0) {
            return 0;
          }
          return 1;
        },
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tallies_reinforcements_fr\t' + paths_self.game.state.round
            );
      ...
      deck['ap12'] = {
        ...
        onEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            ...
            // get eligible spaces
            //
            let options = paths_self.returnSpacesWithFilter((key) => {
              if (paths_self.game.spaces[key].trench > 0) {
                return 0;
              }
      ...
      deck['ap13'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (
            ...
            paths_self.game.state.general_records_track.allies_war_status < 4
          ) {
            return 1;
          }
          return 0;
    ...
    if (type == 'central' || type == 'all') {
      deck['cp01'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.turn == 1 && paths_self.game.state.round == 1) {
            return 1;
          }
          return 0;
        },
        ...
        handleGameLoop: function (paths_self, qe, mv) {
          if (mv[0] === 'landwehr') {
            ...
            let filter_fnct = (spacekey, unit) => {
              if (paths_self.returnPowerOfUnit(unit) == 'allies') {
                return 0;
              }
              if (unit.ckey !== 'GE') {
                return 0;
              }
              if (unit.damaged == 1 && unit.destroyed != 1 && unit.army == 1) {
                return 1;
              }
              return 0;
            };
            ...
            let execute_fnct = (spacekey, unit_idx) => {
              ...
              paths_self.addMove(
                `NOTIFY\t${paths_self.game.spaces[spacekey].units[unit_idx].name} repaired in ${paths_self.returnSpaceNameForLog(spacekey)}`
              );
              paths_self.addMove(
                `repair\tcentral\t${spacekey}\t${unit_idx}\t${paths_self.game.player}`
              );
            ...
            if (count == 0) {
              paths_self.game.queue.push(
                'NOTIFY\tNo eligible units for ' + paths_self.popup('cp05')
              );
              return 1;
            }
              ...
              let update_filter_fnct = (spacekey, unit) => {
                ...
                if (unit.damaged == 1 && unit.destroyed != 1 && unit.army) {
                  ...
                  paths_self.updateLog(
                    `${unit.name} repaired in ${paths_self.game.spaces[spacekey].name}`
                  );
                  paths_self.shakeSpacekey(spacekey);
                }
                return 1;
      ...
      deck['cp07'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.state.central_reinforcements_ge > 0) {
            return 0;
          }
          return 1;
        },
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tcentral_reinforcements_ge\t' + paths_self.game.state.round
            );
      ...
      deck['cp09'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.general_records_track.central_war_status > 10) {
            return 0;
          }
          return 1;
        },
      ...
      deck['cp12'] = {
        ...
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            ...
            paths_self.playerAddReinforcements(
              'central',
              ['ge_corps', 'ge_corps', 'ge_army10'],
              'germany'
            );
          }
          return 0;
      ...
      deck['cp13'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.turn >= 3) {
            return 1;
          }
          if (paths_self.game.state.events.moltke == 1) {
            return 1;
          }
          return 0;
        },
      ...
      deck['cp14'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.state.central_reinforcements_ah > 0) {
            return 0;
          }
          return 1;
        },
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tcentral_reinforcements_ah\t' + paths_self.game.state.round
            );
            paths_self.playerAddReinforcements(
              'central',
              ['ah_army07', 'ah_corps', 'ah_corps'],
              'austria'
            );
          }
          return 0;
  ...
  returnLimitedWarDeck(type = 'all') {
    ...
    if (type == 'allies' || type == 'all') {
      ...
      deck['ap16'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.neutral_entry == 0) {
            return 1;
          }
          return 0;
        },
        onEvent: function (paths_self, faction) {
          ...
          paths_self.addUnitToSpace('ro_corps', 'bucharest');
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.playerPlaceUnitOnBoard(
              'romania',
              ['ro_corps', 'ro_corps', 'ro_corps', 'ro_corps'],
              () => {
      ...
      deck['ap19'] = {
        ...
        canEvent: function (paths_self, faction) {
          let attacker_units = paths_self.returnAttackerUnits();
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].ckey == 'BR' || attacker_units[i].ckey == 'FR') {
              return 1;
            }
          }
          return 0;
      ...
      deck['ap22'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (
            paths_self.game.state.events.italy &&
            paths_self.game.state.allies_reinforcements_it == 0
          ) {
            return 1;
          }
          return 0;
        ...
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tallies_reinforcements_it\t' + paths_self.game.state.round
            );
      ...
      deck['ap23'] = {
        ...
        handleGameLoop: function (paths_self, qe, mv) {
          if (mv[0] === 'cloak_and_dagger') {
            ...
            if (paths_self.game.player == p1) {
              paths_self.addMove(
                'cloak_and_dagger_results\t' + JSON.stringify(paths_self.game.deck[0].hand)
              );
              paths_self.endTurn();
            } else {
          ...
          if (mv[0] === 'cloak_and_dagger_results') {
            ...
            for (let z = 0; z < hand.length; z++) {
              paths_self.updateLog(' ' + paths_self.popup(hand[z]));
            }
            paths_self.updateLog('###################################');
      ...
      deck['ap26'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (
            paths_self.game.state.events.blockade &&
            !paths_self.game.state.events.zimmerman_telegram
          ) {
            return 1;
          }
          return 0;
      ...
      deck['ap27'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.state.events.great_retreat = 1;
          return 1;
        }
      };
      ...
      deck['ap28'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.queue.push('player_play_ops\tallies\tap28\t4');
          paths_self.game.state.events.landships = 1;
          return 1;
        }
      };
      ...
      deck['ap30'] = {
        ...
        onEvent: function (paths_self, faction) {
          ...
          //
          paths_self.game.state.central_rounds[paths_self.game.state.central_rounds.length - 1] =
            'sr';
          ...
          if (paths_self.game.player == p) {
            ...
            let continue_fnct = () => {
              ...
              if (units_moved >= max_units_movable) {
                return 0;
              }
              let count = paths_self.countUnitsWithFilter(filter_fnct);
              if (count == 0) {
            ...
            let execute_fnct = (spacekey, unit_idx) => {
              ...
              paths_self.prependMove(
                `move\t${faction}\t${spacekey}\t${unit_idx}\tsalonika\t${paths_self.game.player}`
              );
              paths_self.displaySpace(spacekey);
      ...
      deck['ap31'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (
            paths_self.game.state.events.salonika != 1 &&
            paths_self.game.state.events.turkey == 1
          ) {
            return 1;
          }
          return 0;
        ...
        onEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.player == paths_self.returnPlayerOfFaction('allies')) {
            let filter_fnct = (spacekey) => {
              if (
                paths_self.game.spaces[spacekey].control == 'central' &&
                paths_self.game.spaces[spacekey].units.length > 0
              ) {
                return 0;
              }
              if (spacekey == 'gallipoli') {
                return 1;
              }
              if (spacekey == 'canakale') {
                return 1;
              }
              if (spacekey == 'izmir') {
                return 1;
    ...
    if (type == 'central' || type == 'all') {
      ...
      deck['cp17'] = {
        ...
        handleGameLoop: function (paths_self, qe, mv) {
          if (mv[0] === 'matahari') {
            ...
            if (paths_self.game.player == p1) {
              paths_self.addMove(
                'matahari_results\t' + JSON.stringify(paths_self.game.deck[1].hand) + '\t2'
              );
              paths_self.endTurn();
            } else {
              paths_self.updateStatus('opponent revealing hand...');
        ...
        canEvent: function (paths_self, faction) {
          ...
          for (let i = 0; i < defender_units.length; i++) {
            if (defender_units[i].ckey === 'GE') {
              if (paths_self.game.spaces[paths_self.game.state.combat.key].trench > 0) {
                return 1;
              }
            }
          }
      ...
      deck['cp25'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.state.events.high_seas_fleet = 3;
          return 1;
        }
      };
      ...
      deck['cp26'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          if (
            paths_self.game.spaces[paths_self.game.state.combat.key].country == 'france' &&
            paths_self.game.spaces[paths_self.game.state.combat.key].fort > 0
          ) {
      ...
      deck['cp27'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.state.events.zeppelin_raids = 1;
          return 1;
        }
      };
      ...
      deck['cp28'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.russian_capitulation_track == 3) {
            return 1;
          }
          return 0;
        },
      ...
      deck['cp30'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          for (let i = 0; i < attacker_units.length; i++) {
            if (
              attacker_units[i].ckey == 'GE' &&
              (paths_self.game.spaces[attacker_units[i].spacekey].terrain === 'mountain' ||
                paths_self.game.spaces[paths_self.game.state.combat.key].terrain === 'mountain')
            ) {
              return 1;
            }
          }
        ...
        onEvent: function (paths_self, faction) {
          if (paths_self.returnPlayerOfFaction('allies') == paths_self.game.player) {
            let filter_fnct = (spacekey, unit) => {
              ...
              if (unit.corps && unit.damaged == 0 && unit.ckey == 'BR') {
                return 1;
              }
            };
            let filter_fnct2 = (spacekey, unit) => {
              ...
              if (unit.corps && unit.damaged == 1 && unit.ckey == 'BR') {
                return 1;
              }
            };
      ...
      deck['cp34'] = {
        ...
        onEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.playerPlaceUnitOnBoard(
              'bulgaria',
              ['bu_corps', 'bu_corps', 'bu_corps', 'bu_corps'],
              () => {
                paths_self.addMove('SETVAR\tstate\tneutral_entry\t1');
                paths_self.endTurn();
              }
  ...
  returnTotalWarDeck(type = 'all') {
    ...
    if (type == 'allies' || type == 'all') {
      ...
      deck['ap36'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.mine_attack != 1) {
            let space = paths_self.game.spaces[paths_self.game.state.combat.key];
      ...
      deck['ap38'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.over_there == 1) {
            return 1;
          }
          return 0;
        },
        onEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tallies_reinforcements_us\t' + paths_self.game.state.round
            );
      ...
      deck['ap39'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.state.events.they_shall_not_pass = 1;
          return 1;
        }
      };
      ...
      deck['ap41'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.turkey == 1) {
            return 1;
          }
          return 0;
        },
        onEvent: function (paths_self, faction) {
          ...
          for (let key in paths_self.game.spaces) {
            if (paths_self.game.spaces[key].country == 'arabia') {
              options.push(key);
            }
          }
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
      ...
      deck['ap47'] = {
        ...
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            ...
            paths_self.playerAddReinforcements('allies', ['us_army02', 'us_corps'], 'usa');
          }
          return 0;
        }
      };
      ...
      deck['ap48'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.landships == 1) {
            return 1;
          }
          return 0;
        },
        onEvent: function (paths_self, faction) {
          ...
          if (
            space.terrain == 'normal' &&
            (space.country == 'france' || space.country == 'belgium' || space.country == 'germany')
          ) {
      ...
      deck['ap50'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.sinai_pipeline == 1) {
            return 1;
          }
          return 0;
        },
      ...
      deck['ap51'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (
            paths_self.game.state.events.blucher == 1 ||
            paths_self.game.state.events.michael == 1 ||
            paths_self.game.state.events.peace_offensive == 1
          ) {
            return 1;
          }
          return 0;
      ...
      deck['ap53'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (
            paths_self.game.spaces['salonika'].control == 'allies' &&
            paths_self.game.spaces['salonika'].units.length < 3
          ) {
            return 1;
          }
          return 0;
      ...
      deck['ap54'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.us_commitment_track == 2) {
            return 1;
          }
          return 0;
        },
      ...
      deck['ap55'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.zimmerman_telegram == 1) {
            return 1;
          }
          return 0;
        },
      ...
      deck['ap56'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.queue.push(`rp\tcentral\tap56`);
          paths_self.game.queue.push(`paris_taxis`);
          return 1;
        },
        handleGameLoop: function (paths_self, qe, mv) {
          if (mv[0] === 'paris_taxis') {
            ...
            let filter_fnct = (spacekey, unit) => {
              if (paths_self.returnPowerOfUnit(unit) == 'central') {
                return 0;
              }
              if (unit.damaged == 1 && unit.destroyed != 1 && unit.army) {
                return 1;
            ...
            //
            // players can flip 1 damaged armies back to full strength
            //
            paths_self.playerSelectUnitWithFilter(
              'Select Unit to Repair / Deploy',
              filter_fnct,
      ...
      deck['ap57'] = {
        ...
        onEvent: function (paths_self, faction) {
          ...
          if (paths_self.game.player == p) {
            ...
            if (paths_self.game.player == paths_self.returnPlayerOfFaction('allies')) {
              paths_self.playerPlaceAllUnitsInSpacekey(russian_spaces, ['ru_cav', 'ru_cav'], () => {
                paths_self.endTurn();
              });
            } else {
              paths_self.updateStatus('Russia placing unit...');
      ...
      deck['ap59'] = {
        ...
        canEvent: function (paths_self, faction) {
          let attacker_units = paths_self.returnAttackerUnits();
          let italian_troops = 0;
          let only_italian_troops = 1;
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].ckey == 'IT') {
              italian_troops = 1;
            }
            if (attacker_units[i].ckey != 'IT') {
              only_italian_tro0ps = 0;
            }
          }
          if (italian_troops == 1 && only_italian_troops == 1) {
            return 1;
          }
          return 0;
        },
      ...
      deck['ap63'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.state.events.backs_to_the_wall = 1;
          paths_self.game.state.combat.backs_to_the_wall = 1;
          return 1;
        }
      };
      ...
      deck['ap65'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.updateLog('Influenza Pandemic restricts playing OPS for Replacements...');
          paths_self.game.state.events.influenza = 1;
          return 1;
        }
      };
    }
    ...
    if (type == 'central' || type == 'all') {
      ...
      deck['cp36'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.hl_take_command) {
            return 1;
          }
          return 0;
        },
      ...
      deck['cp37'] = {
        ...
        onEvent: function (paths_self, faction) {
          paths_self.game.state.events.hottman = 1;
          return 1;
        }
      };
      ...
      deck['cp42'] = {
        ...
        onEvent: function (paths_self, faction) {
          if (paths_self.game.player == paths_self.returnPlayerOfFaction(faction)) {
            paths_self.addMove(
              'SETVAR\tstate\tcentral_reinforcements_tu\t' + paths_self.game.state.round
            );
      ...
      deck['cp43'] = {
        ...
        canEvent: function (paths_self, faction) {
          ...
          for (let i = 0; i < defender_units.length; i++) {
            if (defender_units[i].ckey != 'IT') {
              valid_defender = false;
            }
          }
      ...
      deck['cp45'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (paths_self.game.state.events.bolshevik_revolution == 1) {
            return 1;
          }
          return 0;
        },
      ...
      deck['cp52'] = {
        ...
        canEvent: function (paths_self, faction) {
          if (
            ...
            paths_self.game.state.events.bolshevik_revolution != 1
          ) {
            return 1;
          }
          return 0;
...
***** OPTIONAL ******/
  ...
  returnAttackerCombatPower() {
    ...
    for (let i = 0; i < this.game.state.combat.attacker.length; i++) {
      let unit =
        this.game.spaces[this.game.state.combat.attacker[i].unit_sourcekey].units[
          this.game.state.combat.attacker[i].unit_idx
        ];
      if (unit) {
        if (unit.damaged) {
          x += unit.rcombat;
  ...
  returnAttackerLossFactor() {
    ...
    if (this.game.spaces[this.game.state.combat.key].fort > 0) {
      ...
      if (
        this.game.spaces[this.game.state.combat.key].control ==
        original_spaces[this.game.state.combat.key].control
      ) {
  ...
  canFlankAttack() {
    ...
    //
    for (let i = 0; i < attacker_units.length; i++) {
      if (!attacker_spaces.includes(attacker_units[i].spacekey)) {
        attacker_spaces.push(attacker_units[i].spacekey);
      }
      if (attacker_units[i].type == 'army') {
        is_one_army_attacking = true;
      }
    }
    ...
    if (attacker_spaces.length > 1) {
      are_attacks_from_two_spaces = true;
    }

    if (
      is_geography_suitable == true &&
      is_one_army_attacking == true &&
      are_attacks_from_two_spaces == true
    ) {
  ...
  addHighlightToSpacekey(spacekey = '', htype = '') {
    ...
    if (htype == 'central' && obj) {
      obj.classList.add('central-highlight');
    }
    if (htype == 'allies' && obj) {
      obj.classList.add('allies-highlight');
    }
  }
  ...
  displaySpace(key) {
    if (key === 'arbox' || key === 'crbox' || key === 'aeubox' || key === 'ceubox') {
      this.displayReserveBoxes();
      this.displayEliminatedUnitsBoxes();
      return;
    }
    ...
    try {
      ...
      if (
        ...
        space.country == 'bulgaria' ||
        space.country == 'turkey'
      ) {
  ...
  returnCardImage(cardname) {
    ...
    if (deck[cardname]) {
      ...
      try {
        if (
          !can_event_card ||
          (this.game.state.player_turn_card_select == true && card.cc == true)
        ) {
  ...
  displayTurnTrack() {
    try {
      ...
      if (this.game.state.turn == 4) {
        document.querySelector('.turn-track-4').classList.add('active');
      }
      if (this.game.state.turn == 5) {
        document.querySelector('.turn-track-5').classList.add('active');
      }
      if (this.game.state.turn == 6) {
        document.querySelector('.turn-track-6').classList.add('active');
      }
      if (this.game.state.turn == 7) {
        document.querySelector('.turn-track-7').classList.add('active');
      }
      if (this.game.state.turn == 8) {
        document.querySelector('.turn-track-8').classList.add('active');
      }
      if (this.game.state.turn == 9) {
        document.querySelector('.turn-track-9').classList.add('active');
      }
      if (this.game.state.turn == 10) {
        document.querySelector('.turn-track-10').classList.add('active');
      }
      if (this.game.state.turn == 11) {
        document.querySelector('.turn-track-11').classList.add('active');
      }
      if (this.game.state.turn == 12) {
        document.querySelector('.turn-track-12').classList.add('active');
      }
      if (this.game.state.turn == 13) {
        document.querySelector('.turn-track-13').classList.add('active');
      }
      if (this.game.state.turn == 14) {
        document.querySelector('.turn-track-14').classList.add('active');
      }
      if (this.game.state.turn == 15) {
        document.querySelector('.turn-track-15').classList.add('active');
      }
      if (this.game.state.turn == 16) {
        document.querySelector('.turn-track-16').classList.add('active');
      }
      if (this.game.state.turn == 17) {
        document.querySelector('.turn-track-17').classList.add('active');
      }
      if (this.game.state.turn == 18) {
        document.querySelector('.turn-track-18').classList.add('active');
      }
      if (this.game.state.turn == 19) {
        document.querySelector('.turn-track-19').classList.add('active');
      }
      if (this.game.state.turn == 20) {
        document.querySelector('.turn-track-20').classList.add('active');
      }
    } catch (err) {
  ...
  displayGeneralRecordsTrack() {
    try {
      document.querySelectorAll('.general-records-track').forEach((el) => {
      ...
      document.querySelector(
        `.general-records-track-${this.game.state.rp['central']['GE']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_ge.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['central']['AH']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_ah.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['central']['TU']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_tu.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['central']['BU']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_bu.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['central']['CP']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_cp.png" />`;

      // allies
      document.querySelector(
        `.general-records-track-${this.game.state.rp['allies']['A']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_a.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['allies']['BR']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_br.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['allies']['FR']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_fr.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['allies']['IT']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_it.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['allies']['RU']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_ru.png" />`;
      document.querySelector(
        `.general-records-track-${this.game.state.rp['allies']['AP']}`
      ).innerHTML += `<img src="/paths/img/rp/rp_ap.png" />`;

      let central_rp = 0;
      for (let key in this.game.state.rp['central']) {
        central_rp += this.game.state.rp['central'][key];
      }

      let allied_rp = 0;
      for (let key in this.game.state.rp['allies']) {
        allied_rp += this.game.state.rp['allies'][key];
      }

      document.querySelector(`.general-records-track-${central_rp}`).innerHTML +=
        `<img src="/paths/img/rp/rp_cp.png" />`;
      document.querySelector(`.general-records-track-${allied_rp}`).innerHTML +=
        `<img src="/paths/img/rp/rp_allied.png" />`;
      ...
      ).innerHTML += `<img src="/paths/img/vp.png" />`;
  ...
  displayActionRoundTracks() {
    ...
    document.querySelectorAll(`.central-action-round-track`).forEach((el) => {
      el.innerHTML = '';
    });
    document.querySelectorAll(`.allies-action-round-track`).forEach((el) => {
      el.innerHTML = '';
    });
    ...
    for (let z = 0; z < this.game.state.allies_rounds.length; z++) {
      ...
      if (allies_move == 'sr') {
        document.querySelector('.allies-action-round-track-5').innerHTML =
          `<img src="/paths/img/action_ap${z + 1}.png" />`;
      }
      if (allies_move == 'rp') {
        document.querySelector('.allies-action-round-track-6').innerHTML =
          `<img src="/paths/img/action_ap${z + 1}.png" />`;
      }
    }
    for (let z = 0; z < this.game.state.central_rounds.length; z++) {
      ...
      if (central_move == 'sr') {
        document.querySelector('.central-action-round-track-7').innerHTML =
          `<img src="/paths/img/action_cp${z + 1}.png" />`;
      }
      if (central_move == 'rp') {
        document.querySelector('.central-action-round-track-8').innerHTML =
          `<img src="/paths/img/action_cp${z + 1}.png" />`;
      }
    }

    if (parseInt(this.game.state.central_reinforcements_ge) > 0) {
      document.querySelector(`.central-action-round-track-2`).innerHTML =
        `<img src="/paths/img/action_cp${this.game.state.central_reinforcements_ge}.png" />`;
    }
    if (parseInt(this.game.state.central_reinforcements_ah) > 0) {
      document.querySelector(`.central-action-round-track-3`).innerHTML =
        `<img src="/paths/img/action_cp${this.game.state.central_reinforcements_ah}.png" />`;
    }
    if (parseInt(this.game.state.central_reinforcements_tu) > 0) {
      document.querySelector(`.central-action-round-track-4`).innerHTML =
        `<img src="/paths/img/action_cp${this.game.state.central_reinforcements_tu}.png" />`;
    }

    if (parseInt(this.game.state.allies_reinforcements_fr) > 0) {
      document.querySelector(`.allies-action-round-track-2`).innerHTML =
        `<img src="/paths/img/action_ap${this.game.state.allies_reinforcements_fr}.png" />`;
    }
    if (parseInt(this.game.state.allies_reinforcements_br) > 0) {
      document.querySelector(`.allies-action-round-track-3`).innerHTML =
        `<img src="/paths/img/action_ap${this.game.state.allies_reinforcements_br}.png" />`;
    }
    if (parseInt(this.game.state.allies_reinforcements_ru) > 0) {
      document.querySelector(`.allies-action-round-track-4`).innerHTML =
        `<img src="/paths/img/action_ap${this.game.state.allies_reinforcements_ru}.png" />`;
    }
    if (parseInt(this.game.state.allies_reinforcements_it) > 0) {
      document.querySelector(`.allies-action-round-track-5`).innerHTML =
        `<img src="/paths/img/action_ap${this.game.state.allies_reinforcements_it}.png" />`;
    }
    if (parseInt(this.game.state.allies_reinforcements_us) > 0) {
      document.querySelector(`.allies-action-round-track-6`).innerHTML =
        `<img src="/paths/img/action_ap${this.game.state.allies_reinforcements_us}.png" />`;
    }
  ...
  displayMandatedOffensiveTracks() {
    document.querySelectorAll('.central-mandated-offensive-track').forEach((el) => {
      el.classList.remove('active');
    });
    document.querySelectorAll('.allies-mandated-offensive-track').forEach((el) => {
      el.classList.remove('active');
    });
  ...
  displayRussianCapitulationTrack() {
    try {
      document.querySelectorAll('.russian-capitulation-track').forEach((el) => {
        el.classList.remove('active');
      });
  ...
  addEvents(obj) {
    ...
    if (obj.handleGameLoop == null) {
      obj.handleGameLoop = function (paths_self, qe, mv) {
        return 1;
    ...
    if (obj.menuOptionTriggers == null) {
      obj.menuOptionTriggers = function (paths_self, stage, player, faction) {
        return 0;
      };
    }
    if (obj.menuOption == null) {
      obj.menuOption = function (paths_self, stage, player, faction) {
        return 0;
      };
    }
    if (obj.menuOptionActivated == null) {
      obj.menuOptionActivated = function (paths_self, stage, player, faction) {
        return 0;
      };
    }
  ...
  canPlayerMoveUnitIntoNearEast(faction = '', unit = null) {
    ...
    if (faction == 'allies') {
      if (unit.ckey == 'RU') {
        if (this.game.state.events.fall_of_the_tsar) {
          return 0;
        }
        if (unit.corps) {
          if (
  ...
  trackDeploymentIntoNearEast(faction, unit) {
    ...
    if (faction == 'allies') {
      if (unit.ckey == 'BR') {
        this.game.state.has_british_corps_deployed_into_ne = 1;
      }
      if (unit.ckey == 'RU') {
        this.game.state.has_russian_corps_deployed_into_ne = 1;
      }
    }
  }
  ...
  isSpaceOnNearEastMap(spacekey) {
    ...
    if (c == 'turkey') {
      if (['adrianople', 'gallipoli', 'canakale', 'balikesir'].includes(spacekey)) {
        return 0;
      }
      return 1;
    }
  ...
  returnArrayOfSpacekeysForPlacingReinforcements(country = '') {
    ...
    if (country == 'england') {
      if (
        this.game.spaces['london'].control == 'allies' &&
        this.game.spaces['london'].units.length < 3
      ) {
        options.push('london');
      }
    }
    if (country == 'usa') {
      if (
        this.game.spaces['larochelle'].control == 'allies' &&
        this.game.spaces['larochelle'].units.length < 3
      ) {
        options.push('larochelle');
      }
      if (
        this.game.spaces['nantes'].control == 'allies' &&
        this.game.spaces['nantes'].units.length < 3
      ) {
        options.push('nantes');
      }
      if (
        this.game.spaces['cherbourg'].control == 'allies' &&
        this.game.spaces['cherbourg'].units.length < 3
      ) {
        options.push('cherbourg');
      }
      if (
        this.game.spaces['lahavre'].control == 'allies' &&
        this.game.spaces['lahavre'].units.length < 3
      ) {
        options.push('lahavre');
      }
      if (
        this.game.spaces['calais'].control == 'allies' &&
        this.game.spaces['calais'].units.length < 3
      ) {
        options.push('calais');
      }
      if (
        this.game.spaces['bordeaux'].control == 'allies' &&
        this.game.spaces['bordeaux'].units.length < 3
      ) {
        options.push('bordeaux');
      }
    }
    if (country == 'france') {
      if (
        this.game.spaces['paris'].control == 'allies' &&
        this.game.spaces['paris'].units.length < 3
      ) {
        options.push('paris');
      }
      if (
        ...
        if (
          this.game.spaces['orleans'].control == 'allies' &&
          this.game.spaces['orleans'].units.length < 3
        ) {
          options.push('orleans');
        }
      }
    }
    if (country == 'italy') {
      if (
        this.game.spaces['rome'].control == 'allies' &&
        this.game.spaces['rome'].units.length < 3
      ) {
        options.push('rome');
      }
    }
    if (country == 'romania') {
      if (
        this.game.spaces['bucharest'].control == 'allies' &&
        this.game.spaces['bucharest'].units.length < 3
      ) {
        options.push('bucharest');
      }
    }
    if (country == 'russia') {
      if (
        this.game.spaces['moscow'].control == 'allies' &&
        this.game.spaces['moscow'].units.length < 3
      ) {
        options.push('moscow');
      }
      if (
        this.game.spaces['caucasus'].control == 'allies' &&
        this.game.spaces['caucasus'].units.length < 3
      ) {
        options.push('caucasus');
      }
      if (
        this.game.spaces['kharkov'].control == 'allies' &&
        this.game.spaces['kharkov'].units.length < 3
      ) {
        options.push('kharkov');
      }
      if (
        this.game.spaces['petrograd'].control == 'allies' &&
        this.game.spaces['petrograd'].units.length < 3
      ) {
        options.push('petrograd');
      }
    }
    if (country == 'serbia') {
      if (
        this.game.spaces['belgrade'].control == 'allies' &&
        this.game.spaces['belgrade'].units.length < 3
      ) {
        options.push('belgrade');
      }
    }

    if (country == 'germany') {
      if (
        this.game.spaces['berlin'].control == 'central' &&
        this.game.spaces['berlin'].units.length < 3
      ) {
        options.push('berlin');
      }
      if (
        this.game.spaces['breslau'].control == 'central' &&
        this.game.spaces['breslau'].units.length < 3
      ) {
        options.push('breslau');
      }
      if (
        this.game.spaces['essen'].control == 'central' &&
        this.game.spaces['essen'].units.length < 3
      ) {
        options.push('essen');
      }
    }
    if (country == 'austria') {
      if (
        this.game.spaces['vienna'].control == 'central' &&
        this.game.spaces['vienna'].units.length < 3
      ) {
        options.push('vienna');
      }
    }
    if (country == 'bulgaria') {
      if (
        this.game.spaces['sofia'].control == 'central' &&
        this.game.spaces['sofia'].units.length < 3
      ) {
        options.push('sofia');
      }
    }
    ...
    for (let i = options.length - 1; i > 0; i--) {
      if (this.game.spaces[options[i]].fort > 0 && this.game.spaces[options[i]].besieged) {
        options.splice(i, 1);
      }
    }
  ...
  activateNearEastRestriction(spacekey) {
    if (this.isSpaceOnNearEastMap(spacekey)) {
      this.game.state.has_player_activated_ne_space_this_turn = 1;
    }
  }
  ...
  doesSpaceHaveEnemyUnits(faction, spacekey) {
    return this.doesSpaceContainEnemyUnits(faction, spacekey);
  }
  ...
  isSpaceEnemyControlled(faction, spacekey) {
    if (
      this.game.spaces[spacekey].control != 'neutral' &&
  ...
  checkSupplyStatus(faction = '', spacekey = '') {
    ...
    if (faction == '' && spacekey == '') {
      ...
      if (toggle_constantinople_port == 1) {
        this.game.spaces['constantinople'].port = 1;
      }
    ...
    if (!this.game.state.events[country]) {
      return 1;
    }
    ...
    if (
      ...
      sources.push(...['essen', 'breslau', 'sofia', 'constantinople']);
      controlling_faction = 'central';
    }
    if (faction == 'be' || faction == 'belgium') {
      sources.push('london');
    }
    if (faction == 'mef') {
      sources.push(...[this.game.state.events.mef_beachhead, 'london']);
    }
    if (faction == 'fr' || faction == 'france') {
      sources.push('london');
    }
    if (faction == 'ap' || faction == 'allies') {
      sources.push('london', 'moscow', 'petrograd', 'kharkov', 'caucasus');
    }
    if (faction == 'ru' || faction == 'russia') {
      sources.push(...['moscow', 'petrograd', 'kharkov', 'caucasus']);
    }
    if (faction == 'ro' || faction == 'romania') {
    ...
    if (faction == 'sb' || faction == 'serbia') {
      ...
      if (this.returnControlOfSpace('salonika') == 'allies') {
        sources.push('salonika');
      }
    }
    if (sources.length == 0) {
      sources = ['london'];
    ...
    if (country == 'albania') {
      if (this.game.spaces['taranto'].control != 'central') {
        sources.push('taranto');
      }
    }
    ...
    while (pending.length > 0) {
              ...
              if (this.game.spaces[s].units.length > 0) {
                ...
                if (this.returnPowerOfUnit(this.game.spaces[s].units[0]) == controlling_faction) {
                  ...
                  for (let z = 0; z < this.game.spaces[s].units.length; z++) {
                    if (this.game.spaces[s].units[z].army) {
                      units_num += 1000;
                    } else {
                      units_num++;
                    }
      ...
      if (ports_added == false) {
        ...
        if (
          ...
          this.game.spaces[current].port == 1 &&
          this.game.spaces[current].control == 'allies'
        ) {
        ...
        if (
          controlling_faction == 'central' &&
          this.game.spaces[current].port == 2 &&
          this.game.spaces[current].control == 'central'
        ) {
          for (let i = 0; i < ports.length; i++) {
  ...
  returnActivationCost(faction, key) {
    ...
    for (let i = 0; i < space.units.length; i++) {
      if (!units.includes(space.units[i].ckey)) {
        ...
        if (key == 'antwerp' || key == 'ostend' || key == 'calais' || key == 'amiens') {
          if (ckey == 'BE') {
            ckey = 'BR';
          }
        }
        if (ckey == 'ANA' || ckey == 'BR' || ckey == 'AUS' || ckey == 'CND' || ckey == 'PT') {
          ckey = 'BR';
        }
        if (ckey == 'US' || ckey == 'FR') {
          if (space.country == 'france') {
            ckey = 'FR';
          }
        }
        if (ckey == 'SN') {
    ...
    if (faction == 'allies') {
      ...
      if (
        ...
        if (this.game.state.events.mef_beachhead) {
          ...
          if (mefs.control == 'allies') {
            ...
            if (this.game.spaces[key].oos) {
              tracing_supply_through_mef_beachhead = true;
            }
      ...
      if (this.game.state.events.everyone_into_battle == 1) {
        if (space.country == 'italy' || space.country == 'france' || space.country == 'belgium') {
          return 1;
        }
      }
    }
    ...
    if (faction == 'central') {
      if (this.game.state.events.eleventh_army == 1) {
        ...
        for (let z = 0; z < space.units.length; z++) {
          ...
          if (space.units[z].key == 'ge_army11') {
            has_eleventh_army = true;
          }
          if (space.units[z].key != 'ge_army11' && space.units[z].army) {
            has_other_army = true;
          }
        }
        if (has_eleventh_army) {
          return this.game.spaces[key].units.length - number_cp_corps;
        }
        if (has_other_army) {
          return this.game.spaces[key].units.length - number_cp_corps + 1;
        }
      }

      if (
        this.game.state.events.falkenhayn != 1 &&
        this.game.state.events.moltke == 1 &&
        (space.country == 'france' || space.country == 'belgium')
      ) {
        if (space.units.length == 1) {
          return 1;
        }
        if (space.units.length == 2) {
          return 2;
        }
        if (space.units.length == 3) {
          return 3;
        }
      }
      if (this.game.state.events.sudarmy == 1) {
        if (countries['GE'] >= 1 && countries['AH'] == 1) {
          ...
          for (let z = 0; z < space.units.length; z++) {
            if (space.units[z].ckey == 'AH' && space.units[z].army) {
              sud_army_eligible = true;
            }
            if (space.units[z].ckey == 'AH' && space.units[z].corps) {
              sud_army_excess_cost++;
            }
            if (space.units[z].ckey == 'GE' && space.units[z].army) {
              sud_army_excess_cost++;
            }
            if (space.units[z].ckey != 'GE' && space.units[z].ckey != 'AH') {
              sud_army_excess_cost++;
            }
          }
        ...
        if (passthrough) {
          ...
          if (hop != -1) {
            ...
            for (let z = 0; z < old.length; z++) {
              if (old[z].spacekey == news[i]) {
                does_old_include_spacekey = true;
              }
            }
  ...
  onNewRound() {
    ...
    if (this.game.state.events.high_seas_fleet > 1) {
      this.game.state.events.high_seas_fleet--;
    }
    ...
    for (let key in this.game.spaces) {
      ...
      if (this.game.spaces[key].trench_roll_modifier < 0) {
        this.game.spaces[key].trench_roll_modifier++;
        redisplay = true;
      }
  ...
  calculateRussianCapitulationTrack() {
    ...
    if (this.game.state.events.tsar_takes_command) {
      position++;
    }

    if (position >= 3) {
      if (this.game.state.general_records_track.combined_war_status + count >= 33) {
        position++;
      }
    }
    ...
    if (
      count == 7 ||
      (count > this.game.state.events.fall_of_the_tsar_russian_vp &&
        this.game.state.events.fall_of_the_tsar_russian_vp != -1)
    ) {
      position++;
    }

    if (this.game.state.events.bolshevik_revolution) {
      position++;
    }
  ...
  calculateVictoryPoints() {
    ...
    //
    for (let key in this.game.spaces) {
      if (this.game.spaces[key].vp > 0 && this.game.spaces[key].control == 'central') {
        central_controlled_vp_spaces++;
        results.central_spaces.push(key);
      }
    }
    ...
    let expected_central_vp_spaces = this.countSpacesWithFilter((spacekey) => {
      if (this.game.spaces[spacekey].country == 'germany' && this.game.spaces[spacekey].vp > 0) {
        if (this.game.spaces[spacekey].control != 'central') {
          results.allied_spaces.push(spacekey);
        }
        return 1;
      }
      if (this.game.spaces[spacekey].country == 'austria' && this.game.spaces[spacekey].vp > 0) {
        if (this.game.spaces[spacekey].control != 'central') {
          results.allied_spaces.push(spacekey);
        }
        return 1;
      ...
      if (this.game.state.events.bulgaria) {
        if (this.game.spaces[spacekey].country == 'bulgaria' && this.game.spaces[spacekey].vp > 0) {
          if (this.game.spaces[spacekey].control != 'central') {
            results.allied_spaces.push(spacekey);
          }
          return 1;
      ...
      if (this.game.state.events.turkey) {
        if (this.game.spaces[spacekey].country == 'turkey' && this.game.spaces[spacekey].vp > 0) {
          if (this.game.spaces[spacekey].control != 'central') {
            results.allied_spaces.push(spacekey);
          }
          return 1;
    ...
    if (this.game.state.events.high_seas_fleet == 1) {
      results.events.push('High Seas Fleet (+1)');
      vp++;
    }
    if (this.game.state.events.rape_of_belgium) {
      results.events.push('Rape of Belgium (-1)');
      vp--;
    }
    if (this.game.state.events.reichstag_truce) {
      vp++;
      results.events.push('Reichstag Truce (+1)');
    }
    if (this.game.state.events.lusitania) {
      vp--;
      results.events.push('Lusitania (-1)');
    }
    if (this.game.state.events.war_in_africa_vp) {
      vp++;
      results.events.push('War in Africa (+1)');
    }
    if (this.game.state.events.fall_of_the_tsar) {
      vp++;
      results.events.push('Fall of the Tsar (+1)');
    }
    if (this.game.state.events.fall_of_the_tsar_romania_bonus) {
      vp++;
      results.events.push('Fall of the Tsar (Romania Bonus) (+1)');
    }
    if (this.game.state.events.fourteen_points) {
      vp--;
      results.events.push('Fourteen Points (-1)');
    }
    if (this.game.state.events.convoy) {
      vp--;
      results.events.push('Convoy (-1)');
    }
    if (this.game.state.events.italian_nonentry > 0) {
      vp += this.game.state.events.italian_nonentry;
    }
    if (this.game.state.events.zimmerman_telegram) {
      vp--;
      results.events.push('Zimmerman Telegram (-1)');
    }
    if (this.game.state.events.blockade > 1) {
      vp -= this.game.state.events.blockade - 1;
      results.events.push('Blockade (-' + (this.game.state.events.blockade - 1) + ')');
    }

    if (this.game.state.mo.vp_bonus > 0) {
      vp += this.game.state.mo.vp_bonus;
      let mo = 'Mandatory Offensives (';
      if (this.game.state.mo.vp_bonus >= 0) {
        mo += '+';
      }
      mo += this.game.state.mo.vp_bonus;
      mo += ')';
      results.events.push(mo);
    }
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] === 'turn') {
        ...
        for (let z = this.game.deck[0].hand.length - 1; z >= 0; z--) {
          if (this.game.deck[0].hand[z] === 'pass') {
            this.game.deck[0].hand.splice(z, 1);
          }
        }
        for (let z = this.game.deck[1].hand.length - 1; z >= 0; z--) {
          if (this.game.deck[1].hand[z] === 'pass') {
            this.game.deck[1].hand.splice(z, 1);
          }
        }
      ...
      if (mv[0] === 'show_overlay') {
        ...
        if (mv[1] === 'welcome') {
          ...
          if (faction === 'central') {
            player = this.returnPlayerOfFaction('central');
          }
          if (faction === 'allies') {
            player = this.returnPlayerOfFaction('allies');
          }
          if (this.game.player === player) {
            this.welcome_overlay.render(faction);
            this.game.queue.push('hide_overlay\twelcome');
            if (faction === 'central') {
              this.game.queue.push('ACKNOWLEDGE\tYou are the Central Powers');
            }
            if (faction === 'allies') {
              this.game.queue.push('ACKNOWLEDGE\tYou are the Allied Powers');
            }
          }
        }
      ...
      if (mv[0] == 'draw_strategy_card_phase') {
        ...
        if (
          ...
          this.game.state.general_records_track.central_war_status >= 4 &&
          this.game.state.central_limited_war_cards_added == false
        ) {
          this.game.state.central_limited_war_cards_added = true;

          let discarded_cards = {};
          for (let key in this.game.deck[0].discards) {
            if (!this.game.deck[0].removed[key]) {
              discarded_cards[key] = all_cards[key];
            }
          }
        ...
        if (
          this.game.state.turn > 1 &&
          this.game.state.general_records_track.allies_war_status >= 4 &&
          this.game.state.allies_limited_war_cards_added == false
        ) {
          this.game.state.allies_limited_war_cards_added = true;

          let discarded_cards = {};
          for (let key in this.game.deck[1].discards) {
            if (!this.game.deck[1].removed[key]) {
              discarded_cards[key] = all_cards[key];
            }
          }
        ...
        //
        if (
          this.game.state.general_records_track.central_war_status >= 11 &&
          this.game.state.central_total_war_cards_added == false
        ) {
        ...
        //
        if (
          this.game.state.general_records_track.allies_war_status >= 11 &&
          this.game.state.allies_total_war_cards_added == false
        ) {
      ...
      if (mv[0] == 'deal_strategy_cards') {
        ...
        if (parseInt(this.game.state.cards_left['allies']) > 0) {
          allies_cards_available = parseInt(this.game.state.cards_left['allies']);
        }
        if (parseInt(this.game.state.cards_left['central']) > 0) {
          central_cards_available = parseInt(this.game.state.cards_left['central']);
        }
      ...
      if (mv[0] == 'replacement_phase') {
        ...
        if (this.game.state.events.zeppelin_raids == 1) {
          ...
          if (!this.game.state.rp['allies']['BR']) {
            this.game.state.rp['allies']['BR'] = 0;
          }
          this.game.state.rp['allies']['BR'] -= 4;
          if (this.game.state.rp['allies']['BR'] < 0) {
            this.game.state.rp['allies']['BR'] = 0;
          }
        }
        ...
        if (this.game.state.events.uboats_unleashed == 1 && this.game.state.events.convoy != 1) {
          ...
          if (this.game.state.rp['allies']['BR'] > 1) {
            this.game.state.rp['allies']['BR']--;
          }
        }
        ...
        //
        if (
          this.game.state.events.walter_rathenau == 1 &&
          this.game.state.events.independent_air_force != 1
        ) {
          if (!this.game.state.rp['central']['GE']) {
            this.game.state.rp['central']['GE'] = 0;
          }
        ...
        if (this.game.state.events.central_total_war_cards_added) {
          if (this.game.spaces['sedan'].control == 'central') {
            ...
            for (let key in this.game.spaces) {
              if (
                this.game.spaces[key].country == 'france' ||
                this.game.spaces[key].country == 'belgium'
              ) {
      ...
      if (mv[0] == 'player_play_replacements') {
        ...
        //
        let count = 0;
        for (let key in this.game.state.rp[faction]) {
          count += parseInt(this.game.state.rp[faction][key]);
        }
        if (count == 0) {
          return 1;
        }
      ...
      if (mv[0] == 'war_status_phase') {
        ...
        //
        if (
          this.game.state.events.italy != 1 &&
          this.game.state.central_total_war_cards_added == true
        ) {
          if (!this.game.state.events.italian_nonentry) {
            this.game.state.events.italian_nonentry = 0;
          }
        ...
        if (this.game.state.events.blockade > 0) {
          if (this.game.state.round == 4) {
            this.game.state.events.blockade++;
          }
          if (this.game.state.round == 9) {
            this.game.state.events.blockade++;
          }
          if (this.game.state.round == 14) {
            this.game.state.events.blockade++;
          }
          if (this.game.state.round == 19) {
            this.game.state.events.blockade++;
          }
        }
          ...
          if (space.besieged == true) {
            if (space.fort > 0) {
              if (space.units.length > 0) {
                ...
                if (
                  this.returnPowerOfUnit(space.units[0]) != space.control &&
                  units_num >= space.fort
                ) {
                  ...
                  if (roll > space.fort) {
                    space.fort = -1;
                    this.updateLog(
                      this.returnSpaceNameForLog(space.key) + ' fort destroyed (roll: ' + roll + ')'
                    );
                    this.updateStatus(
                      this.returnSpaceNameForLog(space.key) + ' fort destroyed (roll: ' + roll + ')'
                    );
                    ...
                    this.updateStatus(
                      this.returnSpaceNameForLog(space.key) +
                        ' fort resists siege (roll: ' +
                        roll +
                        ')'
                    );
                    this.updateLog(
        ...
        if (this.game.player == player) {
          ...
          if (player == 1) {
            ...
            if (can_discard == false) {
              this.addMove(
                'SETVAR\tstate\tcards_left\t' + this.returnFactionOfPlayer() + '\t' + num
              );
              this.endTurn();
              return;
            }
          ...
          this.attachCardboxEvents((action) => {
            ...
            if (action === 'discard') {
              this.addMove(
                'SETVAR\tstate\tcards_left\t' + this.returnFactionOfPlayer() + '\t' + (num - 1)
              );
      ...
      if (mv[0] == 'attrition_phase') {
        ...
        for (let key in spaces) {
          if (this.game.state.events.turkey) {
            if (spaces[key].country == 'turkey') {
              spaces[key].control = 'central';
            }
          }
          if (this.game.state.events.romania) {
            if (spaces[key].country == 'romania') {
              spaces[key].control = 'allies';
            }
          }
          if (this.game.state.events.bulgaria) {
            if (spaces[key].country == 'bulgaria') {
              spaces[key].control = 'central';
            }
          }
          if (this.game.state.events.greece) {
            if (spaces[key].country == 'greece') {
              spaces[key].control = 'allies';
            }
          }
          if (this.game.state.events.albania) {
            if (spaces[key].country == 'albania') {
              spaces[key].control = 'allies';
            }
          }
          if (this.game.state.events.turkey) {
            if (spaces[key].country == 'persia') {
              spaces[key].control = 'allies';
            }
          }
          if (this.game.state.events.turkey) {
            if (spaces[key].country == 'kuwait') {
              spaces[key].control = 'allies';
            }
          }
          if (this.game.state.events.turkey) {
            if (spaces[key].country == 'egypt') {
              spaces[key].control = 'allies';
            }
          }
          if (this.game.state.events.italy) {
            if (spaces[key].country == 'italy') {
              spaces[key].control = 'allies';
            }
          }
          if (this.game.state.events.turkey && key == 'kermanshah') {
            spaces[key].country = 'central';
          }
        }
        ...
        for (let key in this.game.spaces) {
          if (
            this.game.spaces[key].units.length > 0 &&
            key != 'arbox' &&
            key != 'crbox' &&
            key != 'aeubox' &&
            key != 'ceubox'
            ...
            if (power == 'central' || power == 'allies') {
              if (!this.checkSupplyStatus(ckey, key)) {
                ...
                for (let z = 0; z < this.game.spaces[key].units.length; z++) {
                  if (
                    this.game.units[this.game.spaces[key].units[z].key].checkSupplyStatus(this, key)
                  ) {
        ...
        for (let z = 0; z < units_to_eliminate.length; z++) {
          ...
          this.updateLog(
            name + ' eliminated from ' + this.returnSpaceNameForLog(key) + ' (out-of-supply)'
          );
      ...
      if (mv[0] == 'mandated_offensive_phase') {
        ...
        if (central == 1) {
          ...
          if (
            (this.game.spaces['vienna'].control == 'allies' ||
              this.game.spaces['vienna'].besieged == 1) &&
            (this.game.spaces['budapest'].control == 'allies' ||
              this.game.spaces['budapest'].besieged == 1)
          ) {
            central = 1;
          }
        }
        if (central == 2) {
          ...
          if (this.game.state.events.italy != 1) {
            this.game.state.mandated_offensives.central = 'AH';
          }
          if (
            (this.game.spaces['vienna'].control == 'allies' ||
        ...
        if (central == 3) {
          ...
          if (
            this.game.spaces['constantinople'].control == 'allies' ||
            this.game.spaces['constantinople'].besieged == 1
          ) {
            central = 4;
          }
        }
        if (central == 4) {
          ...
          if (
            this.game.spaces['berlin'].control == 'allies' ||
            this.game.spaces['berlin'].besieged == 1
          ) {
            central = 5;
          }
        }
        ...
        if (allies == 1) {
          ...
          if (
            this.game.spaces['paris'].control == 'central' ||
            this.game.spaces['paris'].besieged == 1
          ) {
            allies = 3;
          }
        }
        ...
        if (allies == 4) {
          ...
          if (
            this.game.spaces['rome'].control == 'central' ||
            this.game.spaces['rome'].besieged == 1
          ) {
            allies = 6;
          }
        }
        ...
        while (shift_required && allies <= 6) {
          ...
          for (let z = 0; z < c.length; z++) {
            if (
              this.game.spaces[c[z]].besieged == 0 &&
              this.game.spaces[c[z]].control != 'allies'
            ) {
            } else {
        ...
        while (shift_required && central <= 6) {
          ...
          for (let z = 0; z < c.length; z++) {
            if (
              this.game.spaces[c[z]].besieged == 0 &&
              this.game.spaces[c[z]].control != 'central'
            ) {
            } else {
              shift_required = false;
            }
      ...
      if (mv[0] == 'play') {
        ...
        //
        if (
          paths_self.game.spaces['breslau'].control == 'allies' &&
          paths_self.game.spaces['essen'].control == 'allies'
        ) {
      ...
      if (mv[0] == 'add_unit_to_space') {
        ...
        if (mv[3]) {
          player_to_ignore = parseInt(mv[3]);
        }
      ...
      if (mv[0] === 'ws') {
        ...
        //
        if (
          this.game.state.general_records_track.combined_war_status >= 30 &&
          this.game.state.us_commitment_track == 1
        ) {
      ...
      if (mv[0] === 'rp') {
        ...
        for (let key in c.rp) {
          if (faction == 'central') {
            if (!this.game.state.rp['central'][key]) {
              this.game.state.rp['central'][key] = 0;
            }
            this.game.state.rp['central'][key] += parseInt(c.rp[key]);
          }
          if (faction == 'allies') {
            if (!this.game.state.rp['allies'][key]) {
              this.game.state.rp['allies'][key] = 0;
            }
            this.game.state.rp['allies'][key] += parseInt(c.rp[key]);
          }
        }
        if (faction == 'allies' && this.game.state.events.over_there) {
          if (!this.game.state.rp['allies']['A']) {
            this.game.state.rp['allies']['A'] = 0;
          }
        ...
        this.updateLog(
          this.returnFactionName(faction) + ' plays ' + this.popup(card) + ' for Replacement Points'
        );
        this.game.queue.splice(qe, 1);
        return 1;
      }
      ...
      if (mv[0] === 'player_play_combat') {
        ...
        let options = this.returnSpacesWithFilter((key) => {
          if (this.game.spaces[key].activated_for_combat == 1) {
            for (let z = 0; z < this.game.spaces[key].units.length; z++) {
              if (this.game.spaces[key].units[z].attacked == 0) {
                return 1;
              }
            }
            return 0;
      ...
      if (mv[0] === 'combat') {
        ...
        if (this.game.spaces[key].units.length == 0 && this.game.spaces[key].fort > 0) {
          this.game.state.combat.unoccupied_fort = 1;
        }
        ...
        //
        this.updateLog(
          this.returnFactionName(this.game.state.combat.attacking_faction) +
            ' attacks ' +
            this.returnSpaceNameForLog(key)
        );
        ...
        //
        if (
          this.game.state.events.great_retreat == 1 &&
          ...
          if (this.game.state.mandated_offensives.allies === 'AH IT') {
            ...
            if (this.game.state.mandated_offensives.allies === 'TU') {
              ...
              if (this.game.state.mandated_offensives.allies === 'GE') {
                ...
                let sp = this.game.spaces[this.game.state.combat.key];
                for (let z = 0; z < sp.units.length; z++) {
                  if (sp.units[z].ckey == 'US') {
                    valid_target = true;
                  }
                  if (sp.units[z].ckey == 'BR') {
                    valid_target = true;
                  }
                  if (sp.units[z].ckey == 'FR') {
                    valid_target = true;
                  }
                  if (sp.units[z].ckey == 'BE') {
                    valid_target = true;
                  }
                }
                if (
        ...
        for (let z = 0; z < this.game.spaces[this.game.state.combat.key].units.length; z++) {
          let u = this.game.spaces[this.game.state.combat.key].units[z];
          if (u.corps) {
            u.eligible_for_withdrawal_bonus = 1;
          }
        }
      ...
      if (mv[0] == 'calculate_combat_card_modifications') {
        ...
        if (
          this.game.state.combat.attacker_power == 'allies' &&
          this.game.state.events.brusilov_offensive == 1
        ) {
          let attacker_units = this.returnAttackerUnits();
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].ckey == 'RU') {
              this.game.state.combat.attacker_drm += 1;
              i = attacker_units.length;
            }
          }
        }
      ...
      if (mv[0] == 'defender_select_combat_cards') {
        ...
        if (
          this.game.player != this.returnPlayerOfFaction(this.game.state.combat.attacking_faction)
        ) {
      ...
      if (mv[0] == 'combat_card') {
        ...
        if (faction == 'central') {
          if (!this.game.state.cc_central_active.includes(card)) {
            this.game.state.cc_central_active.push(card);
          }
          if (!this.game.state.cc_central_on_table.includes(card)) {
            this.game.state.cc_central_on_table.push(card);
          }
          if (!this.game.state.cc_central_played_this_round.includes(card)) {
            this.game.state.cc_central_played_this_round.push(card);
          }
          for (let i = 0; i < this.game.deck[0].hand.length; i++) {
            if (this.game.deck[0].hand[i] == card) {
              this.game.deck[0].hand.splice(i, 1);
        ...
        if (faction == 'allies') {
          if (!this.game.state.cc_allies_active.includes(card)) {
            this.game.state.cc_allies_active.push(card);
          }
          if (!this.game.state.cc_allies_active.includes(card)) {
            this.game.state.cc_allies_on_table.push(card);
          }
          if (!this.game.state.cc_allies_active.includes(card)) {
      ...
      if (mv[0] == 'combat_recalculate_loss_factor') {
        ...
        if (this.game.state.events.withdrawal == 1 && faction == 'attacker') {
          ...
          for (let z = 0; z < this.game.spaces[this.game.state.combat.key].units.length; z++) {
            let u = this.game.spaces[this.game.state.combat.key].units[z];
            if (u.eligible_for_withdrawal_bonus == 1 && u.destroyed == 1) {
              u.eligible_for_withdrawal_bonus = 0;
            }
          }
        }
        ...
        if (faction == 'attacker') {
          ...
          try {
            ...
            for (let z = 0; z < this.game.logs.length; z++) {
              this.game.logs[z] = this.game.logs[z].replace(
                /<span class="combat_14 defender_cp">\?<\/span>/g,
                `<span class="combat_14 defender_cp">${this.game.state.combat.defender_cp}</span>`
              );
              this.game.logs[z] = this.game.logs[z].replace(
                /<span class="combat_14 attacker_loss_factor">\?<\/span>/g,
                `<span class="combat_14 attacker_loss_factor">${this.game.state.combat.attacker_loss_factor}</span>`
              );
            }
            document.querySelectorAll(qs1).forEach((el) => {
              setTimeout(() => {
                el.innerHTML = this.returnAttackerLossFactor();
              }, 500);
            });
            document.querySelectorAll(qs2).forEach((el) => {
              setTimeout(() => {
                el.innerHTML = this.returnDefenderCombatPower();
              }, 500);
            });
          } catch (err) {
        ...
        if (faction == 'defender') {
          ...
          try {
            ...
            for (let z = 0; z < this.game.logs.length; z++) {
              this.game.logs[z] = this.game.logs[z].replace(
                /<span class="combat_14 attacker_cp">\?<\/span>/g,
                `<span class="combat_14 attacker_cp">${this.game.state.combat.attacker_cp}</span>`
              );
              this.game.logs[z] = this.game.logs[z].replace(
                /<span class="combat_14 defender_loss_factor">\?<\/span>/g,
                `<span class="combat_14 defender_loss_factor">${this.game.state.combat.defender_loss_factor}</span>`
              );
            }
            document.querySelectorAll(qs1).forEach((el) => {
              setTimeout(() => {
                el.innerHTML = this.returnDefenderLossFactor();
              }, 500);
            });
            document.querySelectorAll(qs2).forEach((el) => {
              setTimeout(() => {
                el.innerHTML = this.returnAttackerCombatPower();
              }, 500);
            });
          } catch (err) {
            console.log('error updated attacker loss factor: ' + JSON.stringify(err));
        ...
        if (
          this.game.state.combat.attacker_loss_factor > this.game.state.combat.defender_loss_factor
        ) {
      ...
      if (mv[0] == 'combat_determine_outcome') {
        ...
        if (this.game.state.combat.unoccupied_fort == 1) {
          ...
          for (let i = 0; i < this.game.spaces[this.game.state.combat.key].units.length; i++) {
            ...
            if (this.game.state.events.yanks_and_tanks == 1 && unit.ckey == 'US') {
              defender_drm += 2;
            }
            if (unit.key.indexOf('army') > 0) {
              defender_table = 'army';
        ...
        if (['portsaid', 'cairo', 'gaza', 'beersheba'].includes(this.game.state.combat.key)) {
          ...
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].spacekey == 'sinai') {
              attacking_from_sinai = true;
            }
          }
          for (let i = 0; i < attacker_units.length; i++) {
            if (attacker_units[i].spacekey != 'sinai') {
              attacking_from_sinai = false;
            }
          }
          if (attacking_from_sinai == true) {
            if (attacker_power == 'allies' && this.game.state.events.sinai_pipeline == 1) {
            } else {
        ...
        if (attacker_drm > 0) {
          this.updateLog(
            `Attacker <span class="combat_${this.game.state.combat.step} attacker_cp">${this.game.state.combat.attacker_cp}</span>: <span class="combat_${this.game.state.combat.step} attacker_roll">${this.game.state.combat.attacker_roll}</span> [+<span class="combat_${this.game.state.combat.step} attacker_drm">${this.game.state.combat.attacker_drm}</span>] ==> <span class="combat_${this.game.state.combat.step} defender_loss_factor">${this.game.state.combat.defender_loss_factor}</span> hits`
          );
        } else {
          this.updateLog(
            `Attacker <span class="combat_${this.game.state.combat.step} attacker_cp">${this.game.state.combat.attacker_cp}</span>: <span class="combat_${this.game.state.combat.step} attacker_roll">${this.game.state.combat.attacker_roll}</span> ==> <span class="combat_${this.game.state.combat.step} defender_loss_factor">${this.game.state.combat.defender_loss_factor}</span> hits`
          );
        }
        if (this.game.state.combat.defender_drm > 0) {
          this.updateLog(
            `Defender <span class="combat_${this.game.state.combat.step} defender_cp">${this.game.state.combat.defender_cp}</span>: <span class="combat_${this.game.state.combat.step} defender_roll">${this.game.state.combat.defender_roll}</span> [+<span class="combat_${this.game.state.combat.step} defender_drm">${this.game.state.combat.defender_drm}</span>] ==> <span class="combat_${this.game.state.combat.step} attacker_loss_factor">${this.game.state.combat.attacker_loss_factor}</span> hits`
          );
        } else {
          this.updateLog(
            `Defender <span class="combat_${this.game.state.combat.step} defender_cp">${this.game.state.combat.defender_cp}</span>: <span class="combat_${this.game.state.combat.step} defender_roll">${this.game.state.combat.defender_roll}</span> ==> <span class="combat_${this.game.state.combat.step} attacker_loss_factor">${this.game.state.combat.attacker_loss_factor}</span> hits`
          );
        }
        ...
        for (
          let z = 0;
          z < new DOMParser().parseFromString(xhtml, 'text/html').body.textContent.length;
          z++
        ) {
          yhtml += '!';
        }
      ...
      if (mv[0] === 'combat_assign_hits') {
        ...
        if (this.game.state.combat.unoccupied_fort == 1) {
          if (
            ...
            this.game.spaces[this.game.state.combat.key].fort
          ) {
      ...
      if (mv[0] === 'combat_defender_retreat') {
        ...
        //
        if (
          this.game.state.events.withdrawal == 1 &&
          this.game.state.events.withdrawal_bonus_used == 0
        ) {
          ...
          for (let z = 0; z < this.game.spaces[this.game.state.combat.key].units.length; z++) {
            ...
            if (unit.damaged && unit.eligible_for_withdrawal_bonus) {
              unit.damaged = 0;
              try {
                salert(unit.name + ' restored with Withdrawal bonus...');
              } catch (err) {}
        ...
        // withdrawal prevents
        if (this.game.state.withdrawal == 1) {
          this.game.state.combat.can_defender_cancel = false;
        }
        ...
        //
        try {
          this.loss_overlay.showRetreatNotice();
        } catch (err) {}
      ...
      if (mv[0] === 'repair') {
        ...
        let player_to_ignore = 0;
        if (mv[4]) {
          player_to_ignore = parseInt(mv[4]);
        }
        ...
        if (player_to_ignore != this.game.player) {
          ...
          for (let z = 0; z < this.game.spaces[spacekey].units.length; z++) {
            if (!this.game.spaces[spacekey].units[z].destroyed) {
              if (damaged == 1) {
                if (
                  this.game.spaces[spacekey].units[z].damaged == true &&
                  key === this.game.spaces[spacekey].units[z].key
                ) {
                ...
                if (
                  this.game.spaces[spacekey].units[z].damaged == false &&
                  key === this.game.spaces[spacekey].units[z].key
                ) {
                  unit = this.game.spaces[spacekey].units[z];
                  unit_idx = z;
      ...
      if (mv[0] === 'add') {
        ...
        if (player_to_ignore != this.game.player) {
          ...
          if (attacked) {
            this.game.spaces[spacekey].units[this.game.spaces[spacekey].units.length - 1].attacked =
              1;
            this.game.spaces[spacekey].units[
              this.game.spaces[spacekey].units.length - 1
            ].damaged_this_combat = true;
          }
          //
...
****/
        ...
        if (idx !== null) {
          if (loss_factor) {
            ...
            for (let i = 0; i < this.game.state.entrenchments.length; i++) {
              if (this.game.state.entrenchments[i].spacekey == key) {
                already_entrenching = true;
              }
            }
            if (!already_entrenching) {
              ...
              //
              this.game.state.entrenchments.unshift({
                spacekey: key,
          ...
          if (!this.game.spaces[key].trench) {
            this.game.spaces[key].trench = 0;
          }
          if (this.game.spaces[key].trench == 0) {
            this.updateLog(
              this.returnFactionName(faction) + ' entrenches in ' + this.returnSpaceNameForLog(key)
            );
          } else {
            this.updateLog(
              this.returnFactionName(faction) +
                ' entrenches deeper in ' +
                this.returnSpaceNameForLog(key)
            );
          }
          this.game.spaces[key].trench++;
          if (this.game.spaces[key].trench > 2) {
            this.game.spaces[key].trench = 2;
          }
        }
          ...
          for (let i = 0; i < this.game.state.entrenchments.length; i++) {
            ...
            if (e.finished != 1) {
              ...
              if (this.game.state.entrenchments[i].loss_factor >= roll) {
                if (roll_modified) {
                  this.updateLog(
                    this.returnFactionName(this.game.spaces[e.spacekey].control) +
                      ' entrenches in ' +
                      this.returnSpaceNameForLog(e.spacekey) +
                      ' (' +
                      (roll + 1) +
                      ' - 1)'
                  );
      ...
      if (mv[0] === 'move') {
        ...
        if (this.game.spaces[destinationkey].units.length > 0) {
          if (
            this.returnPowerOfUnit(this.game.spaces[destinationkey].units[0]) !=
            this.game.spaces[destinationkey].control
          ) {
            if (this.game.spaces[destinationkey].fort > 0) {
              ...
              if (this.game.spaces[destinationkey].besieged != 1) {
                if (this.game.spaces[destinationkey].trench > 0) {
                  this.game.spaces[destinationkey].trench--;
                }
              ...
              if (control_space == 1) {
                this.game.spaces[destinationkey].control = this.returnPowerOfUnit(
                  this.game.spaces[destinationkey].units[0]
                );
              }
  ...
  returnPowerOfPlayer() {
    return this.returnFactionOfPlayer();
  }
  ...
  playerSelectAttackerCombatCards() {
    ...
    for (let z = ccs.length - 1; z >= 0; z--) {
      if (ccs[z] == this.game.state.active_card) {
        ccs.splice(z, 1);
      }
    }
    ...
    this.attachCardboxEvents((card) => {
      if (cards[card]) {
        if (!cards[card].canEvent(this, 'attacker')) {
          let c = confirm(
            'Do you wish to play this combat card, even though it will have no effect on the current battle?'
          );
          if (!c) {
            return;
          }
        ...
        if (unit.corps) {
          ...
          if (options.length == 0) {
            ...
            for (let z = 0; z < options.length; z++) {
              if (!choices.includes(options[z])) {
                choices.push(options[z]);
              }
            }
          ...
          if (spacekeys.length > 1) {
            paths_self.playerSelectSpaceWithFilter(
              `Destination for ${unit.name}`,
              (spacekey) => {
                if (spacekeys.includes(spacekey)) {
                  return 1;
                }
                return 0;
              },
  ...
  playerPlayAdvance() {
    ...
    for (let i = 0; i < attacker_units.length; i++) {
      ...
      if (unit.ckey == 'RU') {
        are_attackers_russian = true;
      }
      if (!unit.damaged) {
        can_player_advance = true;
      }
    }
    if (space.fort) {
      //
    ...
    if (
      this.game.state.turn == 1 &&
      are_attackers_russian == true &&
      this.game.spaces[this.game.state.combat.key].country == 'germany'
    ) {
      can_player_advance = false;
    }
  ...
  playerHandleAdvance() {
    ...
    if (attacker_loss_factor - defender_loss_factor == 1) {
      spaces_to_retreat = 1;
    }
        ...
        (key) => {
          ...
          for (let i = attacker_units.length - 1, j = 0; j <= 2 && i >= 0; i--) {
            ...
            if (!unit.damaged && !unit.damaged_this_combat) {
              ...
              paths_self.prependMove(
                `move\t${faction}\t${skey}\t${uidx}\t${key}\t${paths_self.game.player}`
              );
              j++;
            }
        ...
        for (let i = 0; i < source.neighbours.length; i++) {
          for (let z = 0; z < destination.neighbours.length; z++) {
            if (source.neighbours[i] == destination.neighbours[z]) {
              if (!roptions.includes(source.neighbours[i])) {
                roptions.push(source.neighbours[i]);
              }
            }
          }
        }
    ...
    for (let z = roptions.length - 1; z >= 0; z--) {
      ...
      if (s.fort && this.game.state.combat.unoccupied_fort == 1) {
        spliceout = true;
      }
      if (s.units.length > 0) {
        spliceout = true;
  ...
  playerSpendReplacementPoints(faction = 'central') {
    ...
    continue_func = () => {
        ...
        for (let z = 0; z < paths_self.game.spaces[key].units.length; z++) {
          if (key == 'arbox' && faction == 'allies') {
            if (paths_self.doReplacementPointsExistForUnit(paths_self.game.spaces[key].units[z])) {
              ...
              if (paths_self.game.spaces[key].units[z].damaged) {
                ...
                can_repair_unit_in_reserves_array.push({
                  ckey: paths_self.game.spaces[key].units[z].ckey,
                  country: paths_self.game.spaces[key].units[z].country,
                  key: key,
                  idx: z,
                  name: paths_self.game.spaces[key].units[z].name
                });
              }
            }
          }
          if (key == 'aeubox' && faction == 'allies') {
            if (paths_self.doReplacementPointsExistForUnit(paths_self.game.spaces[key].units[z])) {
              ...
              can_uneliminate_unit_array.push({
                ckey: paths_self.game.spaces[key].units[z].ckey,
                country: paths_self.game.spaces[key].units[z].country,
                key: key,
                idx: z,
          ...
          if (
            ...
            if (paths_self.doReplacementPointsExistForUnit(paths_self.game.spaces[key].units[z])) {
              if (
                paths_self.game.spaces[key].units[z].damaged &&
                paths_self.returnPowerOfUnit(paths_self.game.spaces[key].units[z]) == 'central'
              ) {
                can_repair_unit_on_board = true;
                can_repair_unit_on_board_array.push({
                  ckey: paths_self.game.spaces[key].units[z].ckey,
                  country: paths_self.game.spaces[key].units[z].country,
                  key: key,
                  idx: z,
          ...
          if (
            ...
            if (paths_self.doReplacementPointsExistForUnit(paths_self.game.spaces[key].units[z])) {
              if (
                paths_self.game.spaces[key].units[z].damaged &&
                paths_self.returnPowerOfUnit(paths_self.game.spaces[key].units[z]) == 'allies'
              ) {
                can_repair_unit_on_board = true;
                can_repair_unit_on_board_array.push({
                  ckey: paths_self.game.spaces[key].units[z].ckey,
      ...
      let options = [];
      if (can_uneliminate_unit) {
        options.push(`<li class="option" id="uneliminate">rebuild eliminated unit</li>`);
      }
      if (can_repair_unit_on_board) {
        options.push(`<li class="option" id="repair_board">repair unit on board</li>`);
      }
      if (can_repair_unit_in_reserves) {
        options.push(`<li class="option" id="repair_reserves">repair unit in reserves</li>`);
      }
      if (can_deploy_unit_in_reserves) {
        options.push(`<li class="option" id="deploy">deploy unit from reserves</li>`);
      }
      ...
      this.game.state.replacements.can_repair_unit_in_reserves_array =
        can_repair_unit_in_reserves_array;
      this.game.state.replacements.can_deploy_unit_in_reserves = can_deploy_unit_in_reserves;
      this.game.state.replacements.can_deploy_unit_in_reserves_array =
        can_deploy_unit_in_reserves_array;
  ...
  playerPlayPostCombatRetreat() {
    ...
    if (space.terrain == 'forest') {
      can_defender_cancel_retreat = true;
    }
    if (space.terrain == 'mountain') {
      can_defender_cancel_retreat = true;
    }
  ...
  playerHandleRetreat() {
    ...
    let spaces_within_hops = paths_self.returnSpacesWithinHops(
      ...
      spaces_to_retreat,
      (spacekey) => {
        if (spacekey == this.game.state.combat.key) {
          return 1;
        }
    ...
    let retreat_function = (unit_idx, retreat_function) => {
      ...
      for (let z = 0; z < spaces_within_hops.length; z++) {
        ...
        if (
          paths_self.game.spaces[s].control ==
            paths_self.returnFactionOfPlayer(paths_self.game.player) &&
          paths_self.game.spaces[s].units.length < 3
        ) {
      ...
      paths_self.playerSelectSpaceWithFilter(
        ...
        (destination) => {
          if (spaces_within_hops.includes(destination)) {
            if (
              paths_self.game.spaces[destination].control ==
              paths_self.returnFactionOfPlayer(paths_self.game.player)
            ) {
              return 1;
            } else {
              if (enemy_spaces_permitted) {
                return 1;
              }
            }
          }
        ...
        (key) => {
          ...
          if (source.units[unit_idx].key === 'mn_corps') {
            ...
            paths_self.prependMove(
              `retreat\t${faction}\t${sourcekey}\t${unit_idx}\t${key}\t${paths_self.game.player}`
            );
            paths_self.displaySpace(key);
            if (unit_idx <= 0) {
              paths_self.endTurn();
    ...
    this.attachCardboxEvents((action) => {
      ...
      if (action === 'yes') {
        ...
        for (let i = 0; i < this.game.state.combat.attacker.length; i++) {
          ...
          if (!eligible_spaces.includes(unit.unit_sourcekey)) {
            eligible_spaces.push(unit.unit_sourcekey);
          }
        }
        ...
        for (let i = 0; i < eligible_spaces.length; i++) {
          if (eligible_spaces[i] === best_option_spacekey) {
            action = i;
          }
        }
  ...
  playerPlayCard(faction, card) {
    ...
    if (only_play_as_event != true) {
      ...
      try {
        can_event_card = c.canEvent(this, faction);
      } catch (err) {}
    }
    if (can_event_card) {
    ...
    this.updateStatusWithOptions(
      `${this.returnFactionName(faction)} - playing ${this.popup(card)}`,
      html,
      true
    );
      ...
      for (let i = 0; i < options.length; i++) {
        ...
        for (let z = 0; z < paths_self.game.spaces[options[i]].units.length; z++) {
          if (paths_self.game.spaces[options[i]].units[z].attacked != 1) {
            if (paths_self.game.spaces[options[i]].units[z].ckey != 'GE') {
              non_german_units = true;
            }
            units_to_attack++;
          }
        }
      ...
      paths_self.playerSelectSpaceWithFilter(
        ...
        (key) => {
          ...
          if (paths_self.game.spaces[key].terrain == 'desert') {
            if (paths_self.game.state.turn == 6) {
              return 0;
            }
            if (paths_self.game.state.turn == 10) {
              return 0;
            }
            if (paths_self.game.state.turn == 14) {
              return 0;
            }
            if (paths_self.game.state.turn == 18) {
              return 0;
            }
          }
          ...
          if (
            paths_self.game.state.events.oberost != 1 &&
            paths_self.game.state.general_records_track.central_war_status < 4
          ) {
            if (faction == 'central') {
              if (
                paths_self.game.spaces[key].country == 'russia' &&
                paths_self.game.spaces[key].fort > 0
              ) {
                if (non_german_units == false) {
                  return 0;
                } else {
                  let attack_ok = false;
          ...
          if (
            paths_self.game.spaces[key].fort > 0 &&
            paths_self.game.spaces[key].units.length == 0
          ) {
            for (let z = 0; z < paths_self.game.spaces[key].neighbours.length; z++) {
              if (paths_self.game.spaces[key].activated_for_combat == 1) {
                if (paths_self.game.spaces[key].control != faction) {
                  return 1;
                }
              }
            }
          ...
          if (
            ...
            if (paths_self.game.spaces[key].units.length > 0) {
              power = paths_self.returnPowerOfUnit(paths_self.game.spaces[key].units[0]);
            }
            if (power != faction) {
              for (let key2 in paths_self.game.state.attacks) {
                if (paths_self.game.state.attacks[key2].includes(key)) {
                  return 0;
                }
              }
              for (let i = 0; i < paths_self.game.spaces[key].neighbours.length; i++) {
                ...
                if (
                  paths_self.game.spaces[n].oos != 1 &&
                  paths_self.game.spaces[n].activated_for_combat == 1
                ) {
                  for (let z = 0; z < paths_self.game.spaces[n].units.length; z++) {
                    if (paths_self.game.spaces[n].units[z].attacked != 1) {
                      return 1;
                    }
                  }
        ...
        (idx) => {
          ...
          if (selected.length > 0) {
            ...
            for (let z = 0; z < selected.length; z++) {
              if (paths_self.app.crypto.stringToBase64(JSON.stringify(idx)) === selected[z]) {
  ...
  playerPlayMovement(faction) {
    ...
    let paths_self = this;
    let options = this.returnSpacesWithFilter((key) => {
      if (key == 'ceubox' || key == 'crbox' || key == 'aeubox' || key == 'arbox') {
        return 0;
      }
      if (this.game.spaces[key].activated_for_movement == 1) {
        return 1;
    ...
    let moveEverythingInterface = function (sourcekey, currentkey) {
      ...
      if (unit.damaged) {
        total_moves = unit.rmovement;
      }

      for (let zz = 1; zz < active_units.length; zz++) {
        if (unit.damaged) {
          if (unit.rmovement < total_moves) {
            total_moves = unit.rmovement;
            unit = active_units[zz];
          }
        } else {
          if (unit.movement < total_moves) {
            total_moves = unit.movement;
            unit = active_units[zz];
          }
        }
      }
      if (active_unit_moves == 0) {
        active_unit_moves = total_moves;
      }
      ...
      let spaces_within_hops = paths_self.returnSpacesWithinHops(
        ...
        (spacekey) => {
          ...
          if (paths_self.game.spaces[spacekey].units.length > 0) {
            if (
              paths_self.returnPowerOfUnit(paths_self.game.spaces[spacekey].units[0]) != faction
            ) {
              return 0;
            }
          }
      ...
      if (paths_self.isSpaceOnNearEastMap(currentkey)) {
        is_currentkey_on_near_east_map = true;
      }
      ...
      if (
        faction == 'central' &&
        paths_self.game.state.events.race_to_the_sea != 1 &&
        (currentkey == 'amiens' || currentkey == 'ostend' || currentkey == 'calais')
      ) {
        stop_move_option = [];
      }
      ...
      paths_self.playerSelectSpaceWithFilter(
        ...
        (destination) => {
          if (
            ...
            if (destination == 'amiens') {
              return 0;
            }
            if (destination == 'ostend') {
              return 0;
            }
            if (destination == 'calais') {
              return 0;
            }
          }
          if (
            ...
            if (
              paths_self.game.spaces['cambrai'].units.length > 0 &&
              paths_self.game.spaces['cambrai'].control == 'allies' &&
              destination == 'ostend'
            ) {
              return 0;
            }
          ...
          //
          if (
            paths_self.game.state.turn == 1 &&
            paths_self.game.spaces[currentkey].country == 'russia'
          ) {
            if (faction == 'allies') {
          ...
          if (paths_self.game.state.events[country] != 1) {
            return 0;
          }
        ...
        (key2) => {
          ...
          //
          if (
            paths_self.game.spaces[key2].units.length + active_units.length > 3 &&
            paths_self.game.spaces[key2].activated_for_movement != 1
          ) {
          ...
          // the fort’s LF
          //
          let can_group_move_into_fort = false;
          let is_the_destination_a_fort = false;
          if (
            paths_self.game.spaces[key2].fort > 1 &&
            paths_self.game.spaces[key2].control != paths_self.returnFactionOfPlayer()
          ) {
            let num_corps = 0;
            let num_armies = 0;
            // count stuff already there...
            for (let zz = 0; zz < paths_self.game.spaces[key2].units.length; zz++) {
              if (paths_self.game.spaces[key2].units[zz].corps) {
                num_corps++;
              }
              if (paths_self.game.spaces[key2].units[zz].army) {
                num_armies++;
              }
            }
            for (let zz = 0; zz < active_units.length; zz++) {
              if (active_units.army) {
                num_armies++;
              }
              if (active_units.corps) {
                num_corps++;
              }
            }
            if (num_armies > 0 || num_corps >= paths_self.game.spaces[key2].fort) {
              can_group_move_into_fort = true;
            }
          }
          for (let z = 0; z < paths_self.game.spaces[key2].units.length; z++) {
            if (paths_self.game.spaces[key2].units[0].army == true) {
              if (
                paths_self.returnPowerOfUnit(paths_self.game.spaces[key2].units[0]) ==
                paths_self.returnFactionOfPlayer()
              ) {
                can_group_move_into_fort = true;
              }
            }
          ...
          if (paths_self.game.spaces[currentkey].neighbours.includes(key2)) {
            is_one_hop_move = true;
          }
          ...
          if (!is_one_hop_move) {
            ...
            if (
              faction == 'central' &&
              paths_self.game.state.events.race_to_the_sea != 1 &&
              (key2 == 'amiens' || key2 == 'ostend' || key2 == 'calais')
            ) {
          ...
          for (let zz = active_units.length - 1; zz >= 0; zz--) {
            if (
              paths_self.isSpaceOnNearEastMap(key2) &&
              paths_self.game.state.does_movement_start_outside_near_east
            ) {
              paths_self.trackMovementIntoNearEast(faction, active_units[zz]);
            }
            if (
              !paths_self.isSpaceOnNearEastMap(key2) &&
              paths_self.game.state.does_movement_start_inside_near_east
            ) {
              paths_self.trackMovementIntoNearEast(faction, active_units[zz]);
            }

            paths_self.moveUnit(currentkey, active_units[zz].idx, key2);
            paths_self.game.spaces[key2].units[
              paths_self.game.spaces[key2].units.length - 1
            ].moved = 1;
            paths_self.prependMove(
              `move\t${faction}\t${currentkey}\t${active_units[zz].idx}\t${key2}\t${paths_self.game.player}`
            );
          }
    ...
    let mainInterface = function (options) {
      ...
      paths_self.playerSelectSpaceWithFilter(
        ...
        (key) => {
          if (paths_self.game.spaces[key].activated_for_movement == 1 && options.includes(key)) {
    ...
    let unitActionInterface = function (key, idx, options) {
      ...
      if (unit.damaged) {
        active_unit_moves = unit.rmovement;
      }
      ...
      if (paths_self.game.spaces[key].oos != 1) {
        ...
        if (paths_self.game.state.events.entrench == 1) {
          ...
          for (let z = 0; z < paths_self.game.state.entrenchments.length; z++) {
            if (paths_self.game.state.entrenchments[z].spacekey == key) {
              can_entrench_here = false;
            }
          }
      ...
      paths_self.attachCardboxEvents((action) => {
        ...
        if (action === 'entrench') {
          ...
          paths_self.game.state.entrenchments.push({
            spacekey: sourcekey,
            loss_factor: lf,
            finished: 0
          });
    ...
    let continueMoveInterface = function (sourcekey, currentkey, idx, options) {
      ...
      for (let z = 0; z < paths_self.game.spaces[currentkey].units.length; z++) {
        if (JSON.stringify(paths_self.game.spaces[currentkey].units[z]) === JSON.stringify(unit)) {
          idx = z;
        }
      }
      ...
      paths_self.playerSelectSpaceWithFilter(
        ...
        (destination) => {
          ...
          //
          if (
            (currentkey == 'gallipoli' || currentkey == 'adrianople') &&
            destination == 'monastir'
          ) {
            if (paths_self.game.state.events.bulgaria != 1) {
              return 0;
            }
          }
          ...
          //
          if (
            (currentkey == 'plevna' || currentkey == 'varna') &&
            (destination == 'ismail' ||
              destination == 'kronstadt' ||
              destination == 'hermannstadt' ||
              destination == 'timisvar')
          ) {
            if (paths_self.game.state.events.romania != 1) {
              return 0;
            }
          }
        ...
        (key2) => {
          ...
          if (
            ...
            is_the_destination_a_fort = true;
          }
          if (unit.army == 1) {
            is_the_unit_an_army = true;
          }
          ...
          //
          let select_and_add_extra_armies = (
            units_remaining = 1,
            select_and_add_extra_armies,
            bonus_moves = []
          ) => {
            //
            ...
            let spaces_within_hops = paths_self.returnSpacesAndHopsWithinHops(
              ...
              (spacekey) => {
                ...
                if (paths_self.game.spaces[spacekey].fort > 0) {
                  if (paths_self.game.spaces[spacekey].units.length > 0) {
                    if (
                      paths_self.returnPowerOfUnit(paths_self.game.spaces[spacekey].units[0]) ==
                      faction
                    ) {
                      return 1;
                    }
            ...
            for (let z = 0; z < spaces_within_hops.length; z++) {
              ...
              if (paths_self.game.spaces[skey].activated_for_movement == 1) {
                for (let i = 0; i < paths_self.game.spaces[skey].units.length; i++) {
                  if (
                    skey != currentkey ||
                    JSON.stringify(paths_self.game.spaces[skey].units[i]) !== JSON.stringify(unit)
                  ) {
                    let u = paths_self.game.spaces[skey].units[i];
                    if (!u.moved) {
                      let is_in_range = false;
                      if (
                        (u.damaged && u.rmovement <= shop) ||
                        (!u.damaged && u.movement <= shop)
                      ) {
                        if (u.corps == 1) {
                          count++;
                        }
            ...
            let spaces_in_distance = paths_self.returnSpacesWithinHops(key2, 4, () => {
              return 1;
            });
            ...
            paths_self.playerSelectUnitWithFilter(
              ...
              (spacekey, u) => {
                if (!spaces_in_distance.includes(spacekey)) {
                  return 0;
                }
              ...
              (bspacekey, bunit_idx) => {
                ...
                bonus_moves.push(
                  `move\t${faction}\t${bspacekey}\t${bunit_idx}\t${key2}\t${paths_self.game.player}`
                );
                ...
                if (units_remaining > 0) {
                  ...
                  sorting_obj[currentkey][0].move =
                    `move\t${faction}\t${currentkey}\t${idx}\t${key2}\t${paths_self.game.player}`;
                  ...
                  for (let key in sorting_obj) {
                    ...
                    if (highest > -1) {
                      sorted_obj[key].push({});
                      sorted_obj[key][sorted_obj[key].length - 1].idx =
                        sorting_obj[key][highest_idx].idx;
                      sorted_obj[key][sorted_obj[key].length - 1].move =
                        sorting_obj[key][highest_idx].move;
                    }
                  }
                  ...
                  for (let z = 0; z < paths_self.game.spaces[sourcekey].units.length; z++) {
                    if (paths_self.game.spaces[sourcekey].units[z].moved != 1) {
                      mint = true;
                    }
                  }
          ...
          if (
            ...
            paths_self.trackMovementIntoNearEast(
              faction,
              paths_self.game.spaces[currentkey].units[idx]
            );
          }
          if (
            !paths_self.isSpaceOnNearEastMap(key2) &&
          ...
          paths_self.moveUnit(currentkey, idx, key2);
          paths_self.game.spaces[key2].units[paths_self.game.spaces[key2].units.length - 1].moved =
            1;
          paths_self.prependMove(
            `move\t${faction}\t${currentkey}\t${idx}\t${key2}\t${paths_self.game.player}`
          );
    ...
    let moveInterface = function (key, options, subsequent_movement = 0) {
      ...
      if (units.length == 1) {
        ...
        if (subsequent_movement == 1) {
          msg = 'Move Another?';
          extra_options = [
            { key: 'all', value: '[move together]' },
            { key: 'none', value: '[stand down]' }
          ];
        }
          ...
          (idx) => {
            ...
            if (idx == 'all') {
              ...
              for (let zz = 0; zz < paths_self.game.spaces[key].units.length; zz++) {
                ...
                if (paths_self.game.spaces[key].units[zz].moved != 1) {
  ...
  playerPlayOps(faction, card, cost, skipend = 0) {
    ...
    let targets = this.returnNumberOfSpacesWithFilter((key) => {
      if (cost < this.returnActivationCost(faction, key)) {
        return 0;
      }
      let space = this.game.spaces[key];
    ...
    this.bindBackButtonFunction(() => {
      for (let key in this.game.spaces) {
        if (
          this.game.spaces[key].activated_for_movement == 1 ||
          this.game.spaces[key].activated_for_combat == 1
        ) {
          this.game.spaces[key].activated_for_movement = 0;
          this.game.spaces[key].activated_for_combat = 0;
          this.displaySpace(key);
      ...
      let movement_func = (movement_func) => {
        this.playerSelectSpaceWithFilter(
          ...
          (key) => {
            ...
            let space = this.game.spaces[key];
            if (key === 'ceubox' || key === 'aeubox' || key === 'crbox' || key === 'arbox') {
              return 0;
            }
            if (space.oos) {
              return 0;
            ...
            if (space.control == 'neutral' && space.country != 'romania') {
              return 0;
            }
        ...
        this.playerSelectSpaceWithFilter(
          ...
          (key) => {
            ...
            if (faction === 'allies' && this.isSpaceOnNearEastMap(key)) {
              ...
              if (this.game.state.has_player_activated_ne_space_this_turn == 0) {
                i_can_fight_in_near_east = true;
              }
            ...
            for (let i = 0; i < space.units.length; i++) {
              if (this.returnPowerOfUnit(space.units[i]) === faction) {
                for (let z = 0; z < space.neighbours.length; z++) {
                  ...
                  if (
                    this.game.spaces[space.neighbours[z]].control != faction &&
                    this.game.spaces[space.neighbours[z]].fort > 0
                  ) {
                    ...
                    if (does_space_have_non_german_units == false) {
                      if (
                        paths_self.game.state.events.oberost != 1 &&
                        paths_self.game.state.general_records_track.central_war_status < 4 &&
                        this.game.spaces[space.neighbours[z]].country == 'russia'
                      ) {
                      } else {
                  ...
                  if (
                    this.game.spaces[space.neighbours[z]].control != faction &&
                    this.game.spaces[space.neighbours[z]].units.length > 0
                  ) {
                    return 1;
                  }
                  if (
                    ...
                    if (this.game.spaces[space.neighbours[z]].units.length > 0) {
                      if (
                        this.returnFactionOfUnit(this.game.spaces[space.neighbours[z]].units[0]) !=
                        faction
                      ) {
                        return 1;
                      }
      ...
      for (let z = 0; z < this.game.spaces[key].units.length; z++) {
        if (filter_fnct(key, this.game.spaces[key].units[z]) == 1) {
          ...
          html +=
            ...
            key +
            ' - ' +
            this.game.spaces[key].units[z].name +
            '</li>';
        }
      }
                ...
                for (let z = 0; z < paths_self.game.spaces[clicked_key].units.length; z++) {
                  if (filter_fnct(clicked_key, paths_self.game.spaces[clicked_key].units[z]) == 1) {
                    h +=
                      ...
                      clicked_key +
                      ' - ' +
                      this.game.spaces[clicked_key].units[z].name +
                      '</li>';
                  }
                }
                h += '</ul>';
  ...
  playerRedeployUnit(faction, card, value = 0, spacekey = '', unit_idx = 0) {
    ...
    let destinations = paths_self.returnSpacesConnectedToSpaceForStrategicRedeployment(
      faction,
      spacekey,
      unit
    );
    ...
    this.playerSelectSpaceWithFilter(
      ...
      (key) => {
        ...
        if (spacekey == 'aeubox' && (key == 'crbox' || key == 'ceubox' || key == 'arbox')) {
          return 0;
        }
        if (spacekey == 'ceubox' && (key == 'crbox' || key == 'arbox' || key == 'aeubox')) {
          return 0;
        }
        if (spacekey == 'arbox' && (key == 'crbox' || key == 'ceubox' || key == 'aeubox')) {
          return 0;
        }
        if (spacekey == 'crbox' && (key == 'arbox' || key == 'ceubox' || key == 'aeubox')) {
          return 0;
        }
        ...
        if (key == 'arbox' || key == 'crbox') {
          ...
          if (
            this.game.spaces['vienna'].control == 'central' ||
            this.game.spaces['vienna'].besieged == 1 ||
            this.game.spaces['budapest'].control == 'central' ||
            this.game.spaces['budapest'].besieged == 1
          ) {
            if (unit.ckey == 'AH') {
              return 0;
        ...
        //
        if (unit.ckey == 'RU') {
          if (paths_self.game.spaces[key].country != 'russia') {
            return 0;
          }
        }
        ...
        if (unit.ckey == 'SB') {
          if (
            paths_self.game.spaces[key].country == 'serbia' &&
            paths_self.game.spaces['belgrade'].control == 'allies'
          ) {
            if (paths_self.game.spaces[key].control == 'allies') {
              return 1;
            }
        ...
        if (this.isSpaceOnNearEastMap(key)) {
          if (paths_self.game.state.does_movement_start_outside_near_east) {
            if (
              !this.canPlayerDeployUnitIntoNearEast(
                faction,
                paths_self.game.spaces[spacekey].units[unit_idx]
              )
            ) {
              return 0;
            }
          ...
          if (paths_self.game.state.does_movement_start_inside_near_east) {
            if (
              !this.canPlayerDeployUnitOutOfNearEast(
                faction,
                paths_self.game.spaces[spacekey].units[unit_idx]
              )
            ) {
        ...
        if (spacekey == 'arbox' || spacekey == 'crbox') {
          ...
          if (!viable_sources.includes(key)) {
            return 0;
          }
        }

        if (key == 'aeubox' || key == 'ceubox' || key == 'arbox' || key == 'crbox') {
          return 1;
        }
      ...
      (key) => {
        ...
        if (this.isSpaceOnNearEastMap(key)) {
          ...
          paths_self.game.state.does_movement_end_inside_near_east = 1;
          paths_self.trackDeploymentIntoNearEast(
            faction,
            paths_self.game.spaces[spacekey].units[unit_idx]
          );
        } else {
  ...
  playerPlaceUnitInSpacekey(spacekeys = [], units = [], mycallback = null) {
    ...
    let finish_fnct = (spacekey) => {
      ...
      this.addMove(
        `add\t${spacekey}\t${this.game.units[units[unit_idx]].key}\t${this.game.player}`
      );
      this.displaySpace(spacekey);
      unit_idx++;
  ...
  playerPlaceUnitOnBoard(country = '', units = [], mycallback = null) {
      ...
      filter_fnct = (spacekey) => {
        if (countries.includes(spacekey)) {
          if (this.game.spaces[spacekey].control == 'allies') {
            if (this.checkSupplyStatus('russia', spacekey)) {
              return 1;
            }
          }
        }
    ...
    if (country == 'romania') {
      ...
      filter_fnct = (spacekey) => {
        if (countries.includes(spacekey)) {
          if (this.game.spaces[spacekey].control == 'allies') {
            if (this.checkSupplyStatus('romania', spacekey)) {
              return 1;
            }
          }
        }
    ...
    if (country == 'bulgaria') {
      ...
      filter_fnct = (spacekey) => {
        if (countries.includes(spacekey)) {
          if (this.game.spaces[spacekey].control == 'central') {
            if (this.checkSupplyStatus('bulgaria', spacekey)) {
              return 1;
            }
          }
        }
    ...
    if (country == 'france') {
      ...
      filter_fnct = (spacekey) => {
        if (countries.includes(spacekey)) {
          if (this.game.spaces[spacekey].control == 'allies') {
            if (this.checkSupplyStatus('france', spacekey)) {
              return 1;
            }
          }
        }
    ...
    if (country == 'austria') {
      ...
      filter_fnct = (spacekey) => {
        if (countries.includes(spacekey)) {
          if (this.game.spaces[spacekey].control == 'central') {
            if (this.checkSupplyStatus('austria', spacekey)) {
              return 1;
            }
          }
        }
  ...
  returnFactionOfUnit(unit) {
    return this.returnPowerOfUnit(unit);
  }
  ...
  returnReserveBoxDeploymentOptions(spacekey, unit_idx = 0) {
    ...
    if (this.game.spaces[spacekey].units.length <= unit_idx) {
    ...
    if (
      (this.game.spaces['vienna'].besieged != 0 ||
        this.game.spaces['vienna'].control == 'allies') &&
      (this.game.spaces['budapest'].besieged != 0 ||
        this.game.spaces['budapest'].control == 'allies') &&
      unit.corps &&
      spacekey == 'vrbox'
    ) {
    ...
    if (unit.ckey == 'GE') {
      fellow_nationalities = ['GE'];
    }
    if (unit.ckey == 'AH') {
      fellow_nationalities = ['AH'];
    }
    if (unit.ckey == 'TU') {
      fellow_nationalities = ['TU'];
    }
    if (unit.ckey == 'BG') {
      fellow_nationalities = ['BG'];
    }
    if (unit.ckey == 'AOI') {
      fellow_nationalities = ['AOI'];
    }
    if (unit.ckey == 'BU') {
      fellow_nationalities = ['BU'];
    }
    if (unit.ckey == 'SN') {
      fellow_nationalities = ['SN'];
    }
    if (unit.ckey == 'YLD') {
      fellow_nationalities = ['YLD'];
    }

    if (unit.ckey == 'FR') {
      fellow_nationalities = ['FR'];
    }
    if (unit.ckey == 'RU') {
      fellow_nationalities = ['RU'];
    }
    if (unit.ckey == 'BR') {
      fellow_nationalities = ['BR', 'CND', 'AUS'];
    }
    if (unit.ckey == 'IT') {
      fellow_nationalities = ['IT'];
    }
    if (unit.ckey == 'US') {
      fellow_nationalities = ['US'];
    }
    if (unit.ckey == 'ANA') {
      fellow_nationalities = ['ANA'];
    }
    if (unit.ckey == 'AUS') {
      fellow_nationalities = ['AUS'];
    }
    if (unit.ckey == 'BEF') {
      fellow_nationalities = ['BEF'];
    }
    if (unit.ckey == 'CAU') {
      fellow_nationalities = ['CAU'];
    }
    if (unit.ckey == 'CND') {
      fellow_nationalities = ['BR', 'CND', 'AUS'];
    }
    if (unit.ckey == 'CZL') {
      fellow_nationalities = ['CZL'];
    }
    if (unit.ckey == 'GR') {
      fellow_nationalities = ['GR'];
    }
    if (unit.ckey == 'MEF') {
      fellow_nationalities = ['MEF'];
    }
    if (unit.ckey == 'MN') {
      fellow_nationalities = ['MN'];
    }
    if (unit.ckey == 'NE') {
      fellow_nationalities = ['NE'];
    }
    if (unit.ckey == 'OA') {
      fellow_nationalities = ['OA'];
    }
    if (unit.ckey == 'POL') {
      fellow_nationalities = ['POL'];
    }
    if (unit.ckey == 'PT') {
      fellow_nationalities = ['PT'];
    }
    if (unit.ckey == 'RD') {
      fellow_nationalities = ['RD'];
    }
    if (unit.ckey == 'SB') {
      fellow_nationalities = ['SB'];
    }
    ...
    if (unit.ckey == 'SB') {
      if (this.game.spaces['salonika'].control == 'allies') {
        viable_sources.push('salonika');
      }
    }
    ...
    if (allied.includes(unit.ckey)) {
      if (unit.corps) {
        if (this.game.spaces['london'].control == 'allies') {
          viable_sources.push('london');
        }
        if (this.game.spaces['paris'].control == 'allies') {
          viable_sources.push('paris');
        }
        if (this.game.spaces['belgrade'].control == 'allies') {
          viable_sources.push('belgrade');
        }
        if (this.game.spaces['moscow'].control == 'allies') {
          viable_sources.push('moscow');
        }
        if (this.game.spaces['caucasus'].control == 'allies') {
          viable_sources.push('caucasus');
        }
        if (this.game.spaces['kharkov'].control == 'allies') {
          viable_sources.push('kharkkov');
        }
        if (this.game.spaces['petrograd'].control == 'allies') {
          viable_sources.push('petrograd');
        }
        if (this.game.state.events.italy) {
          if (this.game.spaces['rome'].control == 'allies') {
            viable_sources.push('rome');
          }
        }
        if (this.game.state.events.romania) {
          if (this.game.spaces['bucharest'].control == 'allies') {
            viable_sources.push('bucharest');
          }
        }
      }
    }
    ...
    if (central.includes(unit.ckey)) {
      if (unit.corps) {
        if (this.game.spaces['berlin'].control == 'central') {
          viable_sources.push('berlin');
        }
        if (this.game.spaces['essen'].control == 'central') {
          viable_sources.push('essen');
        }
        if (this.game.spaces['breslau'].control == 'central') {
          viable_sources.push('breslau');
        }
        if (this.game.spaces['vienna'].control == 'central') {
          viable_sources.push('vienna');
        }
        if (this.game.spaces['budapest'].control == 'central') {
          viable_sources.push('budapest');
        }
        if (this.game.spaces['constantinople'].control == 'central') {
          viable_sources.push('constantinople');
        }
      }
    }
  ...
  importUnit(key, obj) {
    ...
    if (this.game.units[key]) {
      if (obj.checkSupplyStatus) {
        ...
        this.game.units[key].checkSupplyStatus = (paths_self, spacekey) => {
          return 0;
        };
      }
      return;
    ...
    if (!obj.damaged_this_combat) {
      obj.damaged_this_combat = false;
    }
    if (!obj.destroyed) {
      obj.destroyed = false;
    }
    ...
    if (!obj.checkSupplyStatus) {
      obj.checkSupplyStatus = (paths_self, spacekey) => {
        return 0;
      };
    }

    if (key.indexOf('army') > -1) {
      obj.army = 1;
    } else {
      obj.corps = 1;
    }
  ...
  moveUnit(sourcekey, sourceidx, destinationkey) {
    ...
    //
    if (unit.key == 'bef_army' || unit.key == 'bef_corps') {
    ...
    if (destinationkey == 'aeubox' || destinationkey == 'ceubox') {
      ...
      if (eliminate_rather_than_move) {
        ...
        this.updateLog(
          unit.name +
            ' moves from ' +
            this.returnSpaceNameForLog(sourcekey) +
            ' to ' +
            this.returnSpaceNameForLog(destinationkey)
        );
      }
    }
  ...
  doReplacementPointsExistForUnit(unit = null) {
    ...
    if (unit.ckey === 'BE') {
      ...
      if (this.game.spaces['antwerp'].control == 'allies') {
        ptp = true;
      }
      if (this.game.spaces['brussels'].control == 'allies') {
        ptp = true;
      }
      if (this.game.spaces['ostend'].control == 'allies') {
        ptp = true;
      }
      if (this.game.spaces['liege'].control == 'allies') {
        ptp = true;
      }
      if (ptp) {
        if (rp['A'] > 0 || rp['BE'] > 0) {
    ...
    if (unit.ckey === 'SB') {
      ...
      if (this.game.spaces['valjevo'].control == 'allies') {
        ptp = true;
      }
      if (this.game.spaces['nis'].control == 'allies') {
        ptp = true;
      }
      if (this.game.spaces['skopje'].control == 'allies') {
        ptp = true;
      }
      if (this.game.spaces['monastir'].control == 'allies') {
        ptp = true;
      }
      if (ptp) {
        if (rp['A'] > 0 || rp['SB'] > 0) {
    ...
    for (let z = 0; z < capitals.length; z++) {
      ...
      if (z + 1 < capitals.length) {
        is_capital_besieged = false;
      }
    }

    if (is_capital_besieged == true) {
      return 0;
    }
    if (rp[unit.ckey] > 0) {
      return 1;

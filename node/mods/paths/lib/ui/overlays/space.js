...
class SpaceOverlay {
  ...
  render(spacekey = '') {
    ...
    for (let z = 0; z < space.units.length; z++) {
      html += this.mod.returnUnitImage(space.units[z]);
    }
    ...
    if (!this.mod.checkSupplyStatus(space.control, spacekey)) {
      document.querySelector('.space-overlay .status').innerHTML = 'out-of-supply';
    }

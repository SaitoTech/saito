    ...
    try {
      ...
      for (let i = 1; i <= this.game.players.length; i++) {
        this.playerbox.addClass(
          ...
          i,
          'game-playerbox'
        ); /* add to root object so it can be inherited everywhere.*/
      ...
      if (this.game.state.lastroll.length) {
        this.playerbox.updateGraphics(
          `<div class="last-roll">${this.returnDiceImage(this.game.state.lastroll[0])}${this.returnDiceImage(this.game.state.lastroll[1])}</div>`,
          this.game.state.playerTurn
        );
      }

...
const App = ({ app, mod }) => {
  ...
  useEffect(() => {
    mod.handlePeerTransaction = function (blk, tx) {
      if (
        tx.msg.request === 'message-request' &&
        tx.isTo(mod.publicKey) &&
        !tx.isFrom(mod.publicKey)
      ) {
  ...
  const renderBlockInfo = (block) => {
    ...
    return (
      <div className="block-container">
        ...
        <p>
          <strong>Previous Block Hash:</strong> <span>{block.previousBlockHash}</span>
        </p>
        <p>
          <strong>Block Type</strong> <span>{block.block_type}</span>
        </p>
        <p>
          <strong>Number of Transactions:</strong> <span>{block.transactions.length}</span>
        </p>
        <ul>
          {block.transactions.map((tx, index) => (
            <li key={index}>
              ...
              <p>
                <strong>Hash:</strong> {tx.signature}
                <span></span>
              </p>
              <p>
              ...
              <p>
                <strong>From:</strong>{' '}
                <span>{tx.from.map((slip) => slip.publicKey).join(', ')}</span>
              </p>
              <p>
                <strong>To:</strong> <span>{tx.to.map((slip) => slip.publicKey).join(', ')}</span>
              </p>
              <p>
                <strong>Content:</strong> <span>{tx.content}</span>
              </p>
              <p>
                <strong>Module:</strong> <span>{tx.module}</span>
              </p>
              <p>
                <strong>Request:</strong> <span>{tx.request}</span>
              </p>
            </li>
          ))}
        </ul>
    ...
    <div className="app-container">
      <div className="user-box">
        ...
        <div className="identicon">
          <img src={app.keychain.returnIdenticon(mod.publicKey)} />
        </div>
      ...
      <div className="message-send-container">
        ...
        <Button
          app={app}
          onClick={() => createMessageTransaction('transaction')}
          text="Send transaction onchain"
        />
      </div>

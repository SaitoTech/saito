...
class Popup extends ModTemplate {
		...
		return new Promise((resolve, reject) => {
			...
			request.onupgradeneeded = (event) => {
				...
				if (!this.localDB.objectStoreNames.contains('vocabulary')) {
					const objectStore = this.localDB.createObjectStore('vocabulary', {
						keyPath: 'id',
						autoIncrement: true
					});
	...
	async onPeerServiceUp(app, peer, service = {}) {
		...
		if (this.app.BROWSER == 1) {
			...
			save_display_mode = function (mode = '') {
				if (mode === 'simplified') {
	...
	updatePreference(field1, value1) {
		...
		if (field1 == 'display') {
			if (value1 == 'simplified') {
				this.app.options.popup.display.simplified = 1;
			}
			if (value1 == 'traditional') {
	...
	async loadQuestion() {
		return new Promise((resolve, reject) => {
			let question_types = ['multiple_choice_english', 'multiple_choice_pinyin'];

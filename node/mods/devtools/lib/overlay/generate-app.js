  ...
  attachEvents() {
    try {
      ...
      document.querySelector('#saito-app-generate-btn').onclick = async (e) => {
        document.querySelector('#saito-app-generate-btn').innerHTML =
          'Generating app, please wait...';
        document.querySelector('#saito-app-generate-btn').classList.add('active');

        await this_self.mod.sendSubmitModuleTransaction(
          this_self.zip_file,
          this_self.mod_details.slug,
          async function (res) {
            ...
            let newtx = await this_self.app.wallet.createUnsignedTransaction(
              this_self.publicKey,
              BigInt(0),
              BigInt(0)
            );
            ...
            this_self.mod.download(
              jsonData,
              `${this_self.mod_details.slug}.saito`,
              'text/plain',
              function () {

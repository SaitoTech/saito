...
class Scripting extends ModTemplate {
  ...
  canonicalize(x) {
    ...
    if (typeof x === 'object') {
      ...
      const parts = keys.map((k) => JSON.stringify(k) + ':' + this.canonicalize(x[k]));
      return '{' + parts.join(',') + '}';
    }
  ...
  _parseTokens(tokens) {
    ...
    const parsePrimary = () => {
      ...
      if (tok.type === 'WORD') {
        ...
        while (peek() && peek().type === 'WORD' && !isLogicalWord(peek().value)) {
          const k = next();
    ...
    if (node.type === 'opcode_call') {
      ...
      if (node.positionalArgs && node.positionalArgs.length > 0) {
        if (!schema.allowExpressionArgs) {
          throw new Error(`Opcode '${opName}' does not accept nested expressions.`);
        }
        scriptFields.__args = node.positionalArgs.map((c) => this._applyOpcodeSchemas(c));
      }
  ...
  generateWitnessFromScript(access_script) {
    try {
      let script = typeof access_script === 'string' ? JSON.parse(access_script) : access_script;
  ...
  evaluateCondition(context, condition, vars = {}) {
    ...
    const lhs = field
      .split('.')
      .reduce((obj, key) => (obj !== undefined && obj !== null ? obj[key] : undefined), context);

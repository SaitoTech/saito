...
inside vars.__opcodes.sumfields under a controlled key.
`,
  ...
  execute(app, script, witness, vars, tx, blk) {
    ...
    try {
      ...
      // Resolve operands
      // ---------------------------------------------
      const left  = app.browser.resolveVarReference(vars, script.a);
      const right = app.browser.resolveVarReference(vars, script.b);
      ...
      if (!vars.__opcodes.sumfields) { vars.__opcodes.sumfields = {}; }

      vars.__opcodes.sumfields[key] = sum;

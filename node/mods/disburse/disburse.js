...
class Disburse extends ModTemplate {
  ...
  async render() {
    ...
    document.querySelector('.balance-value').innerHTML =
      this.app.browser.formatNumberToLocale(this_wallet_balance);
    //
    //	Manual input
    //
  ...
  parseData(raw_data, this_wallet_balance) {
    /*    expected data, list of 'wallet \t value \n'    */
    try {
      ...
      if (rows.length > 0) {
        ...
        if (dataArray.length == rows.length && this_wallet_balance >= total_out) {
          ...
          document.getElementById('total_out').innerHTML =
            this.app.browser.formatNumberToLocale(total_out);
          document.getElementById('sender_balance').innerHTML =
            this.app.browser.formatNumberToLocale(this_wallet_balance);
          document.getElementById('final_balance').innerHTML =
            this.app.browser.formatNumberToLocale(this_wallet_balance - total_out);

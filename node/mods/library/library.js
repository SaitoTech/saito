...
class Library extends ModTemplate {
  ...
  async handlePeerTransaction(app, tx = null, peer, mycallback) {
    ...
    if (message.request === 'library collection') {
      ...
      if (mycallback) {
        let x = JSON.parse(
          JSON.stringify(this.library[message.data.collection].peers[this.publicKey])
        );
        for (let key in x) {
          x.random = '';
        } // do not share decryption key is all
  ...
  addItemToCollection(item, collection, peer = 'localhost') {
    ...
    for (let i = 0; i < this.library[collection.name].peers[peer].length; i++) {
      if (
        this.library[collection.name].peers[peer][i].title === item.title &&

...
class DreamControls {
  constructor(app, mod, container = '') {
    ...
    this.video = new SaitoVideoBox(app, mod, 'local', 'video-preview');
  ...
  toggleAudio() {
    ...
    //Update UI
    try {
      document.querySelector('.dream-controls .audio-control').classList.toggle('disabled');
      document
        .querySelector('.dream-controls .audio-control i')
        .classList.toggle('fa-microphone-slash');
      document.querySelector('.dream-controls .audio-control i').classList.toggle('fa-microphone');
    } catch (err) {
  ...
  toggleVideo() {
    ...
    //Update UI
    try {
      document.querySelector('.dream-controls .video-control').classList.toggle('disabled');
      document.querySelector('.dream-controls .video-control i').classList.toggle('fa-video-slash');
      document.querySelector('.dream-controls .video-control i').classList.toggle('fa-video');
    } catch (err) {
      console.warn('Stun UI error', err);
    }
  }
  ...
  toggleScreen() {
    ...
    //Update UI
    try {
      document.querySelector('.screen-control').classList.toggle('disabled');
      document.querySelector('.screen-control i').classList.toggle('fa-tablet-screen-button');
      document.querySelector('.screen-control i').classList.toggle('fa-tablet-button');
    } catch (err) {
      console.warn('Stun UI error', err);
    }
  }

...
class DreamControls {
  ...
  addDreamControlsItem(item, id, index) {
    ...
    const existingIcon = document.querySelector(
      `#dream-controls .control-panel .control-list i.${iconClass}`
    );
    if (existingIcon) {
  ...
  attachEvents() {
    ...
    if (document.querySelector('.dream-controls .members-control')) {
      document.querySelector('.dream-controls .members-control').onclick = () => {
        this.overlay.show(
          `<div class="limbo-floating-overlay"><div class="saito-modal-content hide-scrollbar"></div></div>`
        );
  ...
  insertActions() {
    ...
    for (const mod of this.app.modules.mods) {
      let item = mod.respondTo('limbo-actions', {
        group_name:
          this.mod.dreams[this.mod.dreamer].identifier ||
          this.app.keychain.returnUsername(this.mod.dreamer) + "'s Space",

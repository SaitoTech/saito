...
class PurchaseLifecycle {
  ...
  findByNftId(nft_id = '') {
    ...
    return (
      this.purchases.find(
        (p) => p.nft_id === id && p.phase !== PHASE.COMPLETE && p.phase !== PHASE.DISMISSED
      ) || null
    );
  }
  ...
  returnActivePurchase() {
    const pending = this.purchases.find(
      (p) =>
        p.phase === PHASE.SUBMITTED || p.phase === PHASE.CONFIRMING || p.phase === PHASE.FULFILLING
    );
    if (pending) {
      return pending;
    }
  ...
  async checkWalletForArrival({ sync = true } = {}) {
    ...
    try {
      ...
      for (const purchase of this.purchases) {
        if (purchase.phase === PHASE.COMPLETE || purchase.phase === PHASE.DISMISSED) {
          continue;
        }

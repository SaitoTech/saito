...
class Settings {
  ...
  attachEvents() {
    ...
    Array.from(document.querySelectorAll("input[name='confirm_moves']")).forEach((radio) => {
      radio.addEventListener('change', (e) => {
        if (e.currentTarget.value != this.mod.loadGamePreference('settlers_confirm_moves')) {
          this.mod.saveGamePreference('settlers_confirm_moves', parseInt(e.currentTarget.value));
        }
      });
    });

    Array.from(document.querySelectorAll("input[name='overlays']")).forEach((radio) => {
      radio.addEventListener('change', (e) => {
        if (e.currentTarget.value != this.mod.loadGamePreference('settlers_overlays')) {
    ...
    Array.from(document.querySelectorAll("input[name='play_mode']")).forEach((radio) => {
      radio.addEventListener('change', (e) => {
        if (e.currentTarget.value != this.mod.loadGamePreference('settlers_play_mode')) {

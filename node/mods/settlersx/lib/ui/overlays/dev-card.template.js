module.exports = (app, mod, dev_card) => {
  ...
  for (let x = 0; x < mod.game.state.players[mod.game.player - 1].devcards.length; x++) {
    ...
    cards += `
 	           <div class="settlers-dev-card ${card_disable ? 'settlers-card-disabled' : ''}" id="${x}" role="button" tabindex="0" aria-disabled="${card_disable}" aria-label="Play ${card.card}">
 	             	<img src="${card.img}">
  ...
  for (let x = 0; x < mod.game.deck[0].hand.length; x++) {
    ...
    cards += `
            <div class="settlers-dev-card settlers-card-disabled" role="button" tabindex="0" aria-disabled="true" aria-label="${card.card}, available after this turn">
              <img src="${card.img}">
              <div class="settlers-dev-card-title">${card.card}</div>
              <div class="settlers-dev-card-text">${card.subtitle}</div>
            </div>

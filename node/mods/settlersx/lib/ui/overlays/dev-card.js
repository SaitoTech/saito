  ...
  attachEvents() {
    ...
    document.querySelectorAll('.settlers-dev-card').forEach((card) => {
      card.onclick = (e) => {
        ...
        let card =
          this_dev_card.mod.game.state.players[this_dev_card.mod.game.player - 1].devcards[
            selection
          ];
        ...
        switch (cardobj.action) {
          ...
          case 4:
            ...
            this_dev_card.mod.addMove('player_build_road\t' + this_dev_card.mod.game.player);
            this_dev_card.mod.addMove('player_build_road\t' + this_dev_card.mod.game.player);
            this_dev_card.mod.addMove(
          ...
          default:
            //victory point
            this_dev_card.mod.addMove(`vp\t${this_dev_card.mod.game.player}\t${cardobj.card}`);
            this_dev_card.mod.endTurn();
        }

        this_dev_card.mod.game.state.players[this_dev_card.mod.game.player - 1].devcards.splice(
          parseInt(selection),
          1
        );
        this_dev_card.mod.game.state.canPlayCard = false; //No more cards this turn
        ...
        if (
          this_dev_card.mod.game.deck[0].hand.length == 0 &&

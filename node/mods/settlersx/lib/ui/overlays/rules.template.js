module.exports = (rules, theme = '') => {
  return `
  <div class="rules-overlay saito-overlay-size ${theme}">
  <h4>Overview<h4>
  ...
  <p>${rules.mod.vp.name} are important because the first player to accumulate ${rules.mod.game?.options?.game_length || 10} ${rules.mod.vp.name} is declared the winner of the game. 
    ...
    There are also two special achievements: Longest Road is worth ${rules.mod.longest.value} ${rules.mod.vp.name} and Largest Army is worth ${rules.mod.largest.value} ${rules.mod.vp.name}.
  </p>
  <p> The player with the longest contiguous ${rules.mod.r.name} of at least ${rules.mod.longest.min} is awarded the "${rules.mod.longest.name}" badge. 
  ...
  ${rules.mod.game?.players?.length === 2 ? `<h4>Two-player expedition</h4>
  <p>The island has three deserts. Longest Road requires six segments; both achievement badges award one victory point.
  When the score gap exceeds one point, Robin Hood protects the trailing player from theft and blocked production.
  That player chooses where Robin Hood moves.</p>` : ''}
  <h4>FAQ</h4>
  <dl>

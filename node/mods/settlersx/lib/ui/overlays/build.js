...
class BuildOverlay {
  ...
  returnCardImg(resource, disabled = false) {
    ...
    }" src="/settlersx/img/cards/${resource}.svg">`;
  }
    ...
    document.querySelectorAll('.settlers-item-row').forEach((item) => {
      item.onclick = (e) => {
        ...
        if (!disabled) {
          ...
          document.querySelectorAll('.settlers-item-row').forEach((row) => {
            row.onclick = (e) => {};
          });

          if (id === '0') {
            this_self.mod.addMove(`player_build_road\t${this_self.mod.game.player}\t0\t1`);
          }
          if (id === '1') {
            this_self.mod.addMove(`player_build_city\t${this_self.mod.game.player}\t1`);
          }
          if (id === '2') {
            this_self.mod.addMove('player_upgrade_city\t' + this_self.mod.game.player);
          }
          if (id === '3') {

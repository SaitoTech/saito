...
class YearOfPlentyOverlay {
  ...
  attachEvents() {
    ...
    document.querySelectorAll('.settlers-select-options img').forEach((card) => {
      card.onclick = (e) => {
        ...
        if (this_self.cardsToGain.length == 2) {
  ...
  updateCards() {
    ...
    for (let i = 0; i < this.cardsToGain.length; i++) {
      html += `<img id="${i}" role="button" tabindex="0" aria-label="Remove selected ${this.cardsToGain[i]}" alt="${this.cardsToGain[i]}" src="${this.mod.returnCardImage(this.cardsToGain[i])}" >`;
    }

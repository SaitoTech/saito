...
class SettlersState {
  ...
  */
  doesPlayerHaveResources(player, requirement) {
    ...
    returnPortIcon(res){
        if (res == "any"){
            return `<img class="icon" src="/settlersx/img/icons/any-port.svg">`;
        }
        for (let i of this.resources){
            if (i.name == res){
                if (i.icon){
    ...
    randomizeTileImage(resObj){
        let tileDir = "/settlersx/img/sectors/";
        let x = Math.ceil(Math.random()*resObj.ict); 
        return "/settlersx/img/resources/"+resObj.name+".svg";
    }

...
class Graffiti extends ModTemplate {
  ...
  async loadTilesFromDatabase() {
    ...
    for (const row of rows) {
      ...
      this.boardState.setTile(
        { i: row.i, j: row.j, rgbaColor: rowRgbaColor, ordinal: row.ordinal },
        'confirmed'
      );
    }
  }
  ...
  async onConfirmation(blk, tx, conf) {
    ...
    try {
      if (conf == 0) {
        if (
          txmsg.module === this.name &&
          (txmsg.request === 'draw tiles' || txmsg.request === 'paint')
        ) {
    ...
    if (message?.request === 'graffiti: serve board') {
      if (this.serveBoard) {
        if (this.isValidSize(message?.data)) {
          const confirmedBoard = this.boardState.getConfirmedBoard(
            message.data[0],
            message.data[1]
          );
  ...
  async receiveDrawingTransaction(tx) {
    ...
    const tileArray = this.isValidTileArrayData(txData)
      ? this.tileArray_fromTileArrayData(txData)
      : this.isValidLegacyTileArray(txData)
        ? this.tileArray_fromLegacyTileArray(txData)
        : undefined;
    if (!this.isValidTileArray(tileArray)) {
      return;
    }
  ...
  async processTileQueue() {
    ...
    while (this.tilesProcessQueue.length > 0) {
      if (!this.app.BROWSER) {
        console.log(
          `[Graffiti] /!\\ ${this.tilesProcessQueue.length} tile batches left to be processed.`
        );
      }
      ...
      for (let j = 0; j < board[i].length; j++) {
        if (board[i][j] !== null) {
          tileArray.push({
            i: i,
            j: j,
            rgbaColor: board[i][j].rgbaColor,
            ordinal: board[i][j].ordinal
          });
        }
      }
    }
  ...
  async setEffectiveTiles(effectiveTileArray) {
    if (this.app.BROWSER) {
      ...
      window.userInterface.undrawPendingTiles(
        effectiveTileArray.filter((effectiveTile) => !effectiveTile.pendingExists)
      );
    } else {
  ...
  async setTilesInDatabase(tileArray) {
    const maxNbValuesPerStatement = 32766,
      nbSqlColumns = 7;
    ...
    for (let n = 0; n < nbStatements; n++) {
      ...
      for (let k = kMin; k < kMax; k++) {
        sqlValues.push(
          `($i${k}, $j${k}, $red${k}, $green${k}, $blue${k}, $alpha${k}, $ordinal${k})`
        );
      ...
      const sqlStatement =
        'REPLACE INTO tiles (i, j, red, green, blue, alpha, ordinal) VALUES\n' +
        sqlValues.join(',\n');
    ...
    const ordinalsCode =
      ...
      ordinals
        .map((ordinal) => {
          ...
          const ordinalPositionsCode = positions
            .map((position) => {
    ...
    for (let i = 0; i < width; i++) {
      for (let j = 0; j < height; j++) {
        ...
        const { red, green, blue, alpha } =
          tileRgbaColor !== null ? tileRgbaColor : { red: 0, green: 0, blue: 0, alpha: 0 };
    ...
    try {
      ...
      const read = (nbBits) => {
        ...
        if (tail > bitString.length) {
          throw new Error(
            `tail (${tail}) > bitString.length (${bitString.length}) (head: ${head}, nbBits: ${nbBits})`
          );
        }
          ...
          for (let j = 0; j < height; j++) {
            if (board[i][j] !== null) {
              const {
                ...
                b: blue,
                a: alpha
              } = this.Jimp.intToRGBA(boardImage.getPixelColor(i, j));
  ...
  isValidBoard(data) {
    return (
      Array.isArray(data) &&
      this.isValidWidth(data.length) &&
      data.every(
        (arrayElement) =>
          Array.isArray(arrayElement) &&
          this.isValidHeight(arrayElement.length) &&
          arrayElement.every(
            (arrayArrayElement) =>
              arrayArrayElement === null ||
              (this.isValidRgbaColor(arrayArrayElement?.rgbaColor) &&
                this.isValidOrdinal(arrayArrayElement?.ordinal))
          )
      )
    );
  }
  ...
  isValidTileArray(data) {
    return (
      ...
      data.every(
        (arrayElement) =>
          this.areValidCoordinates(arrayElement?.i, arrayElement?.j) &&
          this.isValidRgbaColor(arrayElement?.rgbaColor)
      )
    );
  }
  ...
  isValidLegacyTileArray(data) {
    return (
      ...
      data.every(
        (arrayElement) =>
          this.areValidCoordinates(arrayElement?.i, arrayElement?.j) &&
          this.isValidHexColor(arrayElement?.color)
      )
    );
  }
  ...
  isValidTileArrayData(data) {
    return (
      ...
      data.every(
        (arrayElement) =>
          Array.isArray(arrayElement) &&
          ((arrayElement.length === 6 && this.isValidRgbaComponent(arrayElement[5])) ||
            arrayElement.length === 5) &&
  ...
  isValidRow(data) {
    return (
      this.areValidCoordinates(data?.i, data?.j) &&
  ...
  areValidCoordinates(data1, data2) {
    return [
      [data1, 'i'],
      [data2, 'j']
    ].every((args) => this.isValidCoordinate(...args));
  }

  isValidCoordinate(data, coordinateName) {
    return this.isValidIntegerBetweenBounds(
      data,
      0,
      { i: this.boardWidth, j: this.boardHeight }[coordinateName] - 1
    );
  }
  ...
  isValidSize(data) {
    return (
      Array.isArray(data) &&
      data.length === 2 &&
      this.isValidWidth(data[0]) &&
      this.isValidHeight(data[1])
    );
  }
  ...
  isValidOrdinal(data) {
    return (
      typeof data === 'number' && data < (this.currentTimestamp() + 1) * 10 ** this.txOrderPrecision
    );
  }
  ...
  mixedRgbaColor(rgbaColor1, rgbaColor2) {
    const [R1, G1, B1, a1] = [
      ...
      rgbaColor1.green,
      rgbaColor1.blue,
      rgbaColor1.alpha / 255
    ];
    const [R2, G2, B2, a2] = [
      rgbaColor2.red,
      rgbaColor2.green,
      rgbaColor2.blue,
      rgbaColor2.alpha / 255
    ];
  ...
  bitString_fromBuffer(buffer) {
    return Array.from(buffer)
      .map((byte) => this.bitString_fromByte(byte))
      .join('');
  }

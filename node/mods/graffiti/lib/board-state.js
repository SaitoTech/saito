class BoardState {
  ...
  prettyPrint(nbColumns, nbRows) {
    ...
    const cellWidth =
      1 + statusHeaderLength + 1 + nbHexaColorCharacters + 1 + nbOrdinalDigitsToPrint + 1;
        ...
        for (let i = 0; i < nbColumns; i++) {
          ...
          if (this.state[i][j][status] !== null) {
            line +=
              ' ' +
              alignedText(
                nameFromRgbaColor(this.tileStateGrid[i][j][status].rgbaColor),
                'left',
                nbHexaColorCharacters
              ) +
              ' ' +
  ...
  setTile(tile, status, ordinal = undefined) {
    ...
    switch (status) {
      ...
      case 'pending':
        if (
          this.tileStateGrid[i][j].confirmed === null ||
          ordinal > this.tileStateGrid[i][j].confirmed.ordinal
        ) {
          if (this.tileStateGrid[i][j].pending === null) {

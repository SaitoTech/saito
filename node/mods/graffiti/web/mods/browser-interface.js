...
class BrowserInterface {
  ...
  renderTopBarButton(name, feature, segmentName) {
    ...
    feature.button.title =
      this.label_fromName(name) + (feature.shortcuts ? ` (${feature.shortcuts[0]})` : '');
  ...
  setCurrent_tool(toolName) {
    ...
    for (const toolButton of this.topBarSegments.tools.children) {
      [toolButton.style.background, toolButton.style.borderTop] =
        toolButton === this.tools[this.currentToolName].button
          ? [
              'var(--selected-button-background-color)',
              '1.5px solid var(--selected-button-bordertop-color)'
            ]
          : ['none', 'none'];
    }

    this.selectionLayer.canvas.style.visibility =
      toolName === 'select-layer' ? 'visible' : 'hidden';
  ...
  adjustSettingClones_fromPosition(new_position, setting) {
    const widthOrHeight = (coordinateName) =>
      ({ x: 'clientWidth', y: 'clientHeight' })[coordinateName];

    const namesOfCoordinatesToChange = this.settingCoordinateNames(setting);
    const new_percentageValues = Object.fromEntries(
      namesOfCoordinatesToChange.map((nameOfCoordinateToChange) => {
        const new_percentageValue =
          (new_position[nameOfCoordinateToChange] /
            setting.elements.adjuster[widthOrHeight(nameOfCoordinateToChange)]) *
          100;
        const percentageStep = this.settingPercentageStep(setting);
        const corrected_new_percentageValue =
          Math.round(new_percentageValue / percentageStep) * percentageStep;
        return [
          this.leftTopProperty_fromCoordinateName(nameOfCoordinateToChange),
          corrected_new_percentageValue
        ];
      })
    );
  ...
  settingPercentageStep(setting) {
    return Object.values(this.hsvSettings).includes(setting)
      ? 1
      : 100 / (setting.maxValue - setting.minValue);
  }

  adjustSettingClones_byUnit(direction, sign, setting) {
    const propertyToChange = { horizontal: 'left', vertical: 'top' }[direction];
    const old_coordinateToChange_percentageValue = parseFloat(
      setting.elements.handle.style[propertyToChange]
    );
    const new_percentageValues = Object.fromEntries([
      [
        propertyToChange,
        old_coordinateToChange_percentageValue + sign * this.settingPercentageStep(setting)
      ]
    ]);
  ...
  adjustSettingClones_fromPercentageValues(new_percentageValues, setting) {
    const corrected_new_percentageValues = Object.fromEntries(
      Object.entries(new_percentageValues).map(([propertyToChange, new_percentageValue]) => [
        propertyToChange,
        Utils.boundedPercentageValue(new_percentageValue)
      ])
    );
    this.setCurrent_settingClonesState_fromPercentageValues(
      corrected_new_percentageValues,
      setting
    );
  }

  settingPositionProperties(setting) {
    return this.settingCoordinateNames(setting).map((coordinateName) =>
      this.leftTopProperty_fromCoordinateName(coordinateName)
    );
  }
  ...
  setCurrent_colorSettingState_fromPercentageValues(
    percentageValuesToChange,
    nameOfHsvSettingToChange
  ) {
    this.setCurrent_hsvSettingHandle_percentageValues(
      percentageValuesToChange,
      nameOfHsvSettingToChange
    );
      ...
      this.hsvSettingsNames().map((hsvSettingName) => [
        ...
        Object.fromEntries(
          this.settingPositionProperties(this.hsvSettings[hsvSettingName]).map((property) => [
            property,
            hsvSettingName === nameOfHsvSettingToChange &&
            Object.keys(percentageValuesToChange).includes(property)
    ...
    const hsvaColor = Object.fromEntries(
      Object.entries(percentageValues).map(([hsvComponentName, percentageValue]) => [
        hsvComponentName,
        this.hsvComponent_fromPercentageValue(hsvComponentName, percentageValue)
      ])
    );
  ...
  setCurrent_colorState_fromHsva(hsvaColor) {
    ...
    const fullRgbaColor = ColorConverter.rgba_fromHsva({
      hue: hsvaColor.hue,
      saturation: 1,
      value: 1,
      alpha: hsvaColor.alpha
    });
    this.hsvSettings.saturationAndValue.elements.handle.style.background =
      ColorConverter.rgbCSS_fromRgba(rgbaColor);
    this.hsvSettings.saturationAndValue.elements.spectrumHueLayer.style.background =
      ColorConverter.rgbCSS_fromRgba(fullRgbaColor);
    this.hsvSettings.hue.elements.handle.style.background =
      ColorConverter.rgbCSS_fromRgba(fullRgbaColor);
  }
  ...
  percentageValue_fromHsvComponent(hsvComponentName, hsvComponent) {
    ...
    const corrected_normalizedHsvComponent =
      hsvComponentName === 'value' ? 1 - normalizedHsvComponent : normalizedHsvComponent;
  ...
  hsvComponent_fromPercentageValue(hsvComponentName, percentageValue) {
    ...
    const corrected_normalizedPercentageValue =
      hsvComponentName === 'value' ? 1 - normalizedPercentageValue : normalizedPercentageValue;
    const hsvComponent =
      corrected_normalizedPercentageValue * this.hsvComponentMaxValue(hsvComponentName);
    const corrected_hsvComponent =
      hsvComponentName === 'hue' ? hsvComponent % this.hsvComponentMaxValue('hue') : hsvComponent;
    return corrected_hsvComponent;
  }

  setCurrent_colorSettingState_fromHsva(hsvaColor) {
    const percentageValue = (hsvComponentName) =>
      this.percentageValue_fromHsvComponent(hsvComponentName, hsvaColor[hsvComponentName]);
    ...
    const saturationAndValueSettingHandle_percentageValues = {
      left: percentageValue('saturation'),
      top: percentageValue('value')
    };
    this.setCurrent_hsvSettingHandle_percentageValues(
      saturationAndValueSettingHandle_percentageValues,
      'saturationAndValue'
    );
  ...
  setCurrent_toolSettingState_fromValue(toolSettingValue, toolSetting) {
    const toolSettingHandle_percentagePosition = {
      left: Utils.round(
        ((toolSettingValue - toolSetting.minValue) /
          (toolSetting.maxValue - toolSetting.minValue)) *
          100,
        2
      )
    };
    this.setCurrent_toolSettingState(
      toolSettingHandle_percentagePosition,
      toolSettingValue,
      toolSetting
    );
  }
  ...
  setCurrent_toolSettingState_fromPercentageValues(percentageValues, toolSetting) {
    const toolSettingValue = Math.round(
      toolSetting.minValue +
        (percentageValues.left / 100) * (toolSetting.maxValue - toolSetting.minValue)
    );
  ...
  correctBoardPosition() {
    ...
    if (this.boardApparentLeft > maxLeft) {
      this.boardApparentLeft = Math.floor(maxLeft);
    }
    if (this.boardApparentTop > maxTop) {
      this.boardApparentTop = Math.floor(maxTop);
    }
    if (this.boardApparentLeft < minLeft) {
      this.boardApparentLeft = Math.ceil(minLeft);
    }
    if (this.boardApparentTop < minTop) {
      this.boardApparentTop = Math.ceil(minTop);
    }
  }
  ...
  handleResize_forWindow() {
    this.boardApparentLeft +=
      Math.round(this.displayArea.clientWidth / 2) - Math.round(this.displayAreaWidth / 2);
    this.boardApparentTop +=
      Math.round(this.displayArea.clientHeight / 2) - Math.round(this.displayAreaHeight / 2);
  ...
  updateDisplaySize() {
    ...
    this.minBoardApparentWidth = Math.min(
      this.displayAreaWidth,
      (this.boardWidth / this.boardHeight) * this.displayAreaHeight
    );
    this.minBoardApparentHeight = Math.min(
      this.displayAreaHeight,
      (this.boardHeight / this.boardWidth) * this.displayAreaWidth
    );
  ...
  renderBoard() {
    ...
    for (const layerName of ['background', 'confirmed', 'pending']) {
      this.baseLayers[layerName] = new BoardLayer(
        this.boardWidth,
        this.boardHeight,
        this.board.childElementCount + 1,
        layerName
      );
  ...
  setScale(inputZoomFactor) {
    ...
    const [new_scale, roundingFunction] =
      inputScale < this.minScale
        ? [this.minScale, Math.ceil]
        : inputScale > this.maxScale
          ? [this.maxScale, Math.floor]
          : [inputScale, Math.round];

    const corrected_new_scale =
      Math.max(1, roundingFunction(this.gcd_boardDimensions * new_scale)) /
      this.gcd_boardDimensions;
  ...
  handleWheel_forDisplayArea(event) {
    ...
    if (this.zoomable) {
      ...
      if (this.isInsideBoard(position)) {
        const corrected_zoomFactor = this.setScale(
          event.deltaY < 0 ? this.zoomAbsoluteFactor : 1 / this.zoomAbsoluteFactor
        );
        this.boardApparentLeft = Math.round(
          position.x + corrected_zoomFactor * (this.boardApparentLeft - position.x)
        );
        this.boardApparentTop = Math.round(
          position.y + corrected_zoomFactor * (this.boardApparentTop - position.y)
        );
  ...
  isInsideLayer(position, layer) {
    return layer === null
      ? false
      : position.i >= layer.layerBox.left &&
          position.i < layer.layerBox.right &&
          position.j >= layer.layerBox.top &&
          position.j < layer.layerBox.bottom;
  }
  ...
  isNearlyInside_someLayerAboveSelectedLayer(position) {
    return this.layersAbove(this.selectedLayer).some((layer) =>
      this.isNearlyInside(position, layer)
    );
  }

  isInside_someLayerAboveSelectedLayer(position) {
    return this.layersAbove(this.selectedLayer).some((layer) =>
      this.isInsideLayer(position, layer)
    );
  }
  ...
  handleMousedown_forDisplayArea(event) {
    if (event.button === 0) {
      ...
      this.selectionLayer.lastMousedownNearlyInsideSelectedLayer = this.isNearlyInside(
        position,
        this.selectedLayer
      );
      if (this.selectedLayer !== null) {
        if (!this.isNearlyInside_someLayerAboveSelectedLayer(position)) {
          this.updateSelectionLayerCursorAndResizingSides(
            'mousedown',
            position,
            position,
            true,
          ...
          if (this.currentToolName === 'brush') {
            this.draftShape(
              new Line({
                ...
                rgbaColor: this.currentRgbaColor,
          ...
          if (['rectangle', 'ellipse', 'line'].includes(this.currentToolName)) {
            ...
            const shape = (() => {
              switch (this.currentToolName) {
                case 'rectangle':
                  return new Rectangle(shapeArgs);
                case 'ellipse':
                  return new Ellipse(shapeArgs);
                case 'line':
                  return new Line(shapeArgs);
              }
            })();
  ...
  isInsideBoard(position) {
    return (
      position.x >= this.boardApparentLeft &&
      position.x < this.boardApparentLeft + this.boardApparentWidth &&
      position.y >= this.boardApparentTop &&
      position.y < this.boardApparentTop + this.boardApparentHeight
    );
  }
  ...
  nearestPositionInBoard(targetPosition, otherPosition) {
    ...
    const t = Math.max(
      0,
      targetPosition.x < otherPosition.x
        ? (xmin - targetPosition.x) / (otherPosition.x - targetPosition.x)
        : targetPosition.x > otherPosition.x
          ? (xmax - targetPosition.x) / (otherPosition.x - targetPosition.x)
          : 0,
      targetPosition.y < otherPosition.y
        ? (ymin - targetPosition.y) / (otherPosition.y - targetPosition.y)
        : targetPosition.y > otherPosition.y
          ? (ymax - targetPosition.y) / (otherPosition.y - targetPosition.y)
          : 0
    );
  ...
  isNearlyInside(position, layer, direction = null) {
    return layer === null
      ? false
      : direction === null
        ? this.isNearlyInside(position, layer, 'horizontal') &&
          this.isNearlyInside(position, layer, 'vertical')
        : this.isInsideFarFromBorders(position, layer, direction) ||
          Utils.sides_fromDirection(direction).some((side) =>
            this.isNearBorderLine(position, layer, side)
          );
  }
  ...
  isInsideFarFromBorders(position, layer, direction) {
    ...
    return (
      Math.abs(coordinate - this.middle(layer, direction)) <
      dimension / 2 - this.selectionLayer.borderScope
    );
  }

  boxDimension(layer, dimensionName) {
    return (
      this.border(layer, Utils.positiveSide_fromDimensionName(dimensionName)) -
  ...
  border(layer, side) {
    ...
    const boardApparentPosition = {
      horizontal: this.boardApparentLeft,
      vertical: this.boardApparentTop
    }[direction];
  ...
  distanceToBorderLine(position, layer, side) {
    return Math.abs(
      position[Utils.continuousCoordinateName_fromSide(side)] - this.border(layer, side)
    );
  }
  ...
  isInsideBorderArea(position, layer, side) {
    return (
      this.isNearBorder(position, layer, side) &&
      this.matchSideRelativeToMiddle(position, layer, side)
    );
  }
  ...
  matchNearestBorder(position, layer, side) {
    ...
    const otherCoordinate =
      position[Utils.otherOne(continuousCoordinateName, Utils.CONTINOUS_COORDINATES_NAMES)];

    return Utils.orthogonalSides(side).every(
      (orthogonalSide) =>
        isDirectionHorizontal ===
        isDirectionHorizontal_sign *
          Utils.sign_fromSide(side) *
          (coordinate - this.border(layer, side)) >
          isDirectionHorizontal_sign *
            Utils.sign_fromSide(orthogonalSide) *
            (otherCoordinate - this.border(layer, orthogonalSide))
    );
  }

  isInsideSomeOrthogonalSideArea(position, layer, side) {
    return Utils.orthogonalSides(side).some((orthogonalSide) =>
      this.isNearBorder(position, layer, orthogonalSide)
    );
  }

  updateSelectionLayerCursorAndResizingSides(
    eventType,
    old_position,
    new_position,
    isMouseDown,
    ...
    if (this.selectedLayer !== null) {
      ...
      const isInsideBorderArea = (side) =>
        this.isInsideBorderArea(old_position, this.selectedLayer, side);
      const isInsideSomeOrthogonalSideArea = (side) =>
        this.isInsideSomeOrthogonalSideArea(old_position, this.selectedLayer, side);
      const isInsideFarFromBorders = (direction) =>
        this.isInsideFarFromBorders(old_position, this.selectedLayer, direction);
      const matchNearestBorder = (side) =>
        this.matchNearestBorder(old_position, this.selectedLayer, side);
      const isNearlyInside = (direction) =>
        this.isNearlyInside(old_position, this.selectedLayer, direction);
      const matchSideRelativeToMiddle = (side) =>
        this.matchSideRelativeToMiddle(old_position, this.selectedLayer, side);
      ...
      this.selectionLayer.resizingSides = Utils.objectFrom(Utils.SIDES, (side) =>
        !this.selectionLayer.lastMousedownNearlyInsideSelectedLayer
          ? null
          : !(matchSideRelativeToMiddle(side) && isMouseDown)
            ? null
            : isNearBorder(side) && (!lockAspectRatio || matchNearestBorder(side))
              ? 'master'
              : lockAspectRatio && isInsideSomeOrthogonalSideArea(side)
                ? 'subject'
                : null
      );
      ...
      const cursorTableIndex = (direction) =>
        isNearlyInside(direction)
          ? isMouseDown && this.selectionLayer.lastMousedownNearlyInsideSelectedLayer
            ? 1 +
              (this.selectionLayer.resizingSides[Utils.positiveSide_fromDirection(direction)] !==
              null
                ? 1
                : 0) -
              (this.selectionLayer.resizingSides[Utils.negativeSide_fromDirection(direction)] !==
              null
                ? 1
                : 0)
            : lockAspectRatio &&
                !(isInsideFarFromBorders('horizontal') && isInsideFarFromBorders('vertical'))
              ? 2 *
                Utils.sides_fromDirection(direction)
                  .map(
                    (side) =>
                      isInsideBorderArea(side) ||
                      (isInsideSomeOrthogonalSideArea(side) && matchSideRelativeToMiddle(side))
                  )
                  .indexOf(true)
              : this.selectionLayer.lastMousedownNearlyInsideSelectedLayer ||
                  eventType === 'mouseup' ||
                  (eventType === 'mousemove' && !isMouseDown)
                ? [
      ...
      const new_cursor =
        (!this.isInside_someLayerAboveSelectedLayer(new_position) ||
          (isMouseDown && eventType === 'mousemove')) &&
        cursorTableIndex('horizontal') >= 0 &&
        cursorTableIndex('vertical') >= 0
          ? [
              ['nwse-resize', 'ns-resize', 'nesw-resize'],
              ['ew-resize', grabCursor, 'ew-resize'],
              ['nesw-resize', 'ns-resize', 'nwse-resize']
            ][cursorTableIndex('vertical')][cursorTableIndex('horizontal')]
          : this.selectionLayer.defaultCursor;
  ...
  sameDiscretePosition(position1, position2, center = false) {
    if (center) {
      return position1.i_center === position2.i_center && position1.j_center === position2.j_center;
    } else {
      ...
      if (!this.sameDiscretePosition(old_position, new_position, true)) {
        if (this.currentToolName === 'select-layer' && this.selectedLayer !== null) {
          if (this.selectionLayer.cursor === 'grabbing') {
            ...
            this.selectedLayer.resize(
              old_position,
              new_position,
              this.selectionLayer.resizingSides
            );
          }
          this.updateSelectionLayer();
        }
      ...
      if (this.lastMousedownElement === this.board) {
        ...
        if (this.isInsideBoard(old_position) || this.isInsideBoard(new_position)) {
          ...
          if (!this.sameDiscretePosition(old_positionInBoard, new_positionInBoard)) {
            if (this.currentToolName === 'pixel') {
              this.draftShape(
                new Pixel({ position: new_positionInBoard, rgbaColor: this.currentRgbaColor }),
                false
              );
            }
            if (this.currentToolName === 'brush') {
              this.draftShape(
                new Line({
                  toolName: 'brush',
                  start: old_positionInBoard,
                  end: new_positionInBoard,
                  rgbaColor: this.currentRgbaColor,
            ...
            if (['rectangle', 'ellipse', 'line'].includes(this.currentToolName)) {
              ...
              const shapeArgs = {
                toolName: this.currentToolName,
                start: this.shapeOriginPosition,
                end: new_positionInBoard,
                rgbaColor: this.currentRgbaColor
              };
              if (this.currentToolName === 'line') {
  ...
  handleMouseup_forDisplayArea(event) {
    ...
    if (this.currentToolName === 'select-layer') {
      ...
      if (latestLayerContainingPosition !== this.selectedLayer) {
        if (
          !this.isNearlyInside(position, this.selectedLayer) ||
          this.isInsideSelectedLayer(position)
        ) {
  ...
  attachEvents() {
    ...
    this.main.addEventListener(
      'wheel',
      (event) => {
        this.handleWheel_forMain(event);
      },
      true
    );
    this.main.addEventListener(
      'mousedown',
      (event) => {
        this.handleMousedown_forMain(event);
      },
      true
    );
    this.main.addEventListener(
    ...
    this.displayArea.addEventListener('wheel', (event) => {
      this.handleWheel_forDisplayArea(event);
    });
    this.displayArea.addEventListener('mousedown', (event) => {
      this.handleMousedown_forDisplayArea(event);
    });
    this.displayArea.addEventListener('mousemove', (event) => {
      this.handleMousemove_forDisplayArea(event);
    });
    this.displayArea.addEventListener('mouseup', (event) => {
      this.handleMouseup_forDisplayArea(event);
    });
    this.displayArea.addEventListener('contextmenu', (event) => {
      this.handleContextmenu_forDisplayArea(event);
    });

    this.iterateOverSettings((setting) => {
      setting.elements.adjuster.addEventListener('wheel', (event) => {
        this.handleWheel_forAdjuster(event, setting);
      });
      setting.elements.adjuster.addEventListener('mousedown', (event) => {
        this.handleMousedown_forAdjuster(event, setting);
      });
      setting.elements.handle.addEventListener('mousedown', () => {
        this.handleMousedown_forSettingHandle(setting);
      });
      setting.elements.handle.addEventListener('mouseup', () => {
        this.handleMouseup_forSettingHandle(setting);
      });
    });
    this.main.addEventListener('mousemove', (event) => {
      this.iterateOverSettings((setting) => {
        this.handleMousemove_forAdjuster(event, setting);
      });
    });

    this.controls['clear-draft'].button.addEventListener('mouseup', () => {
      this.handleMouseup_forClearDraftButton();
    });
    this.controls['submit'].button.addEventListener('mouseup', () => {
      this.handleMouseup_forSubmitButton();
    });

    this.stacks = { undo: this.draftLayers, redo: [] };
    this.undoRedoIntervalIDs = {};
    for (const undoOrRedo of ['undo', 'redo']) {
      this.controls[undoOrRedo].button.onmousedown = () => {
        this.handleMousedown_forUndoOrRedoButton(undoOrRedo);
      };
      this.controls[undoOrRedo].button.onmouseup = () => {
        this.handleMouseup_forUndoOrRedoButton(undoOrRedo);
      };
      this.controls[undoOrRedo].button.onmouseleave = () => {
        this.handleMouseleave_forUndoOrRedoButton(undoOrRedo);
      };
    }

    for (const [toolName, tool] of Object.entries(this.tools)) {
      tool.button.addEventListener('mouseup', () => {
        this.setCurrent_tool(toolName);
      });
    }
    this.tools['image'].button.addEventListener('mouseup', () => {
      this.tools['image'].input.click();
    });
    this.tools['image'].input.onchange = () => {
      this.handleOnchange_forImageButtonInput();
    };

    this.isColorPickerActive = false;
    this.colorPicker.button.addEventListener('mouseup', () => {
      this.handleMouseup_forColorPickerButton();
    });
    this.iterateOverButtons((button) => {
      ...
      const buttonBackgrounds = {
        mouseenter: 'var(--hovered-button-background-color)',
        mouseleave: 'none',
        mousedown: 'var(--selected-button-background-color)',
        mouseup: 'none'
      };
      for (const [eventtype, propertyValue] of Object.entries(buttonBackgrounds)) {
        button.addEventListener(eventtype, () => {
          if (
            button !== this.tools[this.currentToolName].button &&
            (button !== this.controls['submit'].button ||
              this.controls['submit'].button.disabled) &&
    ...
    document.addEventListener('keydown', (event) => {
      this.handleKeydown_forDocument(event);
    });
    document.addEventListener('keyup', (event) => {
      this.handleKeyup_forDocument(event);
    });
  }
    ...
    if (files.length === 0) {
      ...
      imageToDraw.onload = () => {
        const scale = Math.min(
          1,
          this.boardWidth / imageToDraw.width,
          this.boardHeight / imageToDraw.height
        );
      ...
      imageToDraw.onerror = () => {
        console.error('Error loading image to draw.');
      };
    }
  }
  ...
  clearSelectionLayer() {
    this.selectionLayer.ctx.clearRect(
      0,
      0,
      this.selectionLayer.canvas.width,
      this.selectionLayer.canvas.height
    );
  }
  ...
  drawLayerSelectionBox(layer) {
    const left =
      Math.round(this.boardApparentLeft + this.currentScale * layer.unionHullBox.left) + 0.5;
    const right =
      Math.round(this.boardApparentLeft + this.currentScale * layer.unionHullBox.right) + 0.5;
    const top =
      Math.round(this.boardApparentTop + this.currentScale * layer.unionHullBox.top) + 0.5;
    const bottom =
      Math.round(this.boardApparentTop + this.currentScale * layer.unionHullBox.bottom) + 0.5;
  ...
  handleKeydown_forDocument(event) {
    ...
    if (this.pressedKeys['n'] && eventKey === '*') {
      ...
      const callbacks = {
        ...
        undo: () => {
          this.startUndoingOrRedoing('undo');
        },
        redo: () => {
          this.startUndoingOrRedoing('redo');
        },
        submit: () => {
          this.handleSubmission();
        }
      ...
      for (const [controlName, control] of Object.entries(this.controls)) {
        if (
          control.shortcuts.some((shortcut) => {
            const keys = this.keys_fromShortcut(shortcut);
            return (
              keys.length === this.nbPressedKeys &&
  ...
  disableControl(controlName) {
    ...
    if (controlName === 'submit') {
      this.controls['submit'].button.style.background =
        this.hoveredElement === this.controls['submit'].button
          ? 'var(--hovered-button-background-color)'
          : 'none';
    }
  }
        ...
        if (i >= shapeBox.corners.left.i && i <= shapeBox.corners.right.i) {
          ...
          const jBound = (baseCorner) => {
            return Math.round(
              baseCorner.j +
                (i < baseCorner.i
                  ? slope(baseCorner, shapeBox.corners.left)
                  : i > baseCorner.i
                    ? slope(baseCorner, shapeBox.corners.right)
                    : 0) *
                  (i - baseCorner.i)
            );
          };
          jIntervals.push({
            min: jBound(shapeBox.corners.top),
            max: jBound(shapeBox.corners.bottom)
          });
        }
      }
  ...
  async submitDraftedTiles() {
    ...
    for (const layer of this.draftLayers) {
      const layerData = layer.ctx.getImageData(
        layer.layerBox.left,
        layer.layerBox.top,
        layer.layerBox.width,
        layer.layerBox.height
      ).data;
      const layerRgbaColors = {};
      this.iterateOverUnionOfShapeBoxesInsideBoard(layer, (i, j) => {
        const layerDataIndex =
          4 * ((j - layer.layerBox.top) * layer.layerBox.width + (i - layer.layerBox.left));
      ...
      for (const [positionStr, rgbaColorToAdd] of Object.entries(layerRgbaColors)) {
        const old_rgbaColor = rgbaColorsToSend[positionStr] ?? {
          red: 0,
          green: 0,
          blue: 0,
          alpha: 0
    ...
    if (tilesToSend.length !== 0) {
      ...
      const effectiveTileArray = tilesToSend.map((tile) =>
        this.boardState.setTile(tile, 'pending', ordinal)
      );
  ...
  baseRgbaColor(i, j) {
    const pendingRgbaColor = this.boardState.getTileRgbaColor(i, j, 'pending') ?? {
      red: 0,
      green: 0,
      blue: 0,
      alpha: 0
    };
    const confirmedRgbaColor = this.boardState.getTileRgbaColor(i, j, 'confirmed') ?? {
      red: 0,
      green: 0,
      blue: 0,
      alpha: 0
    ...
    return this.mod.mixedRgbaColor(
      this.mod.mixedRgbaColor(this.blankTileRgbaColor, confirmedRgbaColor),
      pendingRgbaColor
    );
  }

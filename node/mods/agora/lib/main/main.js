...
const AgoraMainTemplate = require('./main.template');
...
class AgoraMain {
	constructor(app, mod, container = 'body') {
		...
		this.app.connection.on('agora-render-auction-list-request', async () => {
			await this.render();
		});

		this.app.connection.on('agora-update-auction-list-request', () => {
			console.log('inside  agora-update-auction-list-request /////////////////');
			this.updateAuctionList();
		});

		this.app.connection.on('agora-build-auction-list-request', async () => {
			console.log('inside  agora-buildAuctionList-auction-list-request /////////////////');
	...
	async render() {
		...
		if (!document.querySelector('.saito-container')) {
			this.app.browser.addElementToDom(AgoraMainTemplate(this.app, this.mod, this));
		} else {
			this.app.browser.replaceElementBySelector(
				AgoraMainTemplate(this.app, this.mod, this),
				'.saito-container'
			);
		}
	...
	async buildAuctionList() {
		// empty list
		if (document.querySelector('.agora-table-list')) {
			document.querySelector('.agora-table-list').innerHTML = ``;
		}
		let empty_msg = document.querySelector('#agora-empty');
		let title = document.querySelector('#agora-table-title');
		...
		if (this.mod.auction_list.length > 0) {
			...
			for (let i = 0; i < this.mod.auction_list.length; i++) {
				...
				const nft = new Nft(this.app, this.mod, '.agora-table-list');
	...
	updateAuctionList() {
		this.mod.sendRetreiveRecordsTransaction((records) => {
			...
			this.app.connection.emit('agora-build-auction-list-request');
		});
	}

...
class PandemicOriginalSkin {
  ...
  queryPlayer(role) {
    ...
    if (role === 'scientist') {
      ...
      player.desc = 'The Scientist may research a vaccine with only 4 cards instead of 5';
      player.type = 2;
    }
    if (role === 'medic') {
      player.name = 'Medic';
  ...
  returnCities() {
    ...
    cities['chicago'] = {
      top: 490,
      left: 340,
      neighbours: ['sanfrancisco', 'losangeles', 'mexicocity', 'atlanta', 'montreal'],
      name: 'Chicago',
      virus: 'blue'
    };
    cities['montreal'] = {
    ...
    cities['istanbul'] = {
      top: 560,
      left: 1385,
      neighbours: ['stpetersburg', 'milan', 'algiers', 'cairo', 'baghdad', 'moscow'],
      name: 'Istanbul',
      virus: 'black'
    };
    cities['moscow'] = {
    ...
    cities['bangkok'] = {
      top: 880,
      left: 2005,
      neighbours: ['kolkata', 'hongkong', 'chennai', 'jakarta', 'hochiminhcity'],
      name: 'Bangkok',
      virus: 'red'
    };
    cities['jakarta'] = {
    ...
    cities['manila'] = {
      top: 1000,
      left: 2305,
      neighbours: ['sydney', 'hongkong', 'hochiminhcity', 'sanfrancisco', 'taipei'],
      name: 'Manila',
      virus: 'red'
    };
    ...
    cities['hongkong'] = {
      top: 790,
      left: 2115,
      neighbours: ['hochiminhcity', 'shanghai', 'bangkok', 'kolkata', 'taipei', 'manila'],
      name: 'Hong Kong',
      virus: 'red'
    };
    cities['taipei'] = {
  ...
  displayDecks() {
    ...
    for (let i = 0; i < this.mod.game.deck[0].discards.length; i++) {
      html += `<img style="bottom:${2 * i}px; right:${
        ...
        this.mod.game.deck[0].cards[this.mod.game.deck[0].discards[i]].img
      }" />`;
    }
    ...
    for (let i = 0; i < this.mod.game.deck[1].discards.length; i++) {
      html += `<img style="bottom:${2 * i}px; right:${
        ...
        this.mod.game.deck[1].cards[this.mod.game.deck[1].discards[i]].img
      }" />`;
    }
    ...
    try {
      for (let i = 0; i < station_list.length; i++) {
        ...
        if (!document.querySelector(divname)) {
          this.app.browser.addElementToElement(
            `<div id="station_${i + 1}" class="research_station"></div>`,
            document.getElementById('gameboard')
          );
        }

...
class PandemicRetroSkin extends PandemicOriginalSkin {
  ...
  queryPlayer(role) {
    ...
    if (role === 'scientist') {
      ...
      player.desc = 'The Experimenter may research a vaccine with only 4 cards instead of 5';
      player.type = 2;
    }
    if (role === 'medic') {
  ...
  shouldMap(city) {
    ...
    if (
      Math.sqrt(Math.pow(city.x - city.left - 40, 2) + Math.pow(city.y - city.top - 40, 2)) > 20
    ) {
      return true;
    }
  ...
  returnCities() {
    ...
    cities['istanbul'] = {
      top: 480,
      left: 1500,
      neighbours: ['stpetersburg', 'milan', 'cairo', 'baghdad', 'moscow', 'essen'],
      name: 'Istanbul',
      virus: 'black',
      x: 1503,
      y: 483
    ...
    cities['hongkong'] = {
      top: 675,
      left: 2200,
      neighbours: ['hochiminhcity', 'shanghai', 'bangkok', 'taipei', 'manila'],
      name: 'Hong Kong',
      virus: 'red',
      x: 2222,
      y: 680
    ...
    try {
      this.app.browser.addElementToElement(
        `<div class="infection_highlight${
          dontplace ? ' blocked' : ''
        }" style="top: ${this.mod.scale(this.cities[city].top + 40)}px; left: ${this.mod.scale(

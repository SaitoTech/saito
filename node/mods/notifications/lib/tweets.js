...
class Tweets {
  ...
  render() {
    if (!document.querySelector('.tweets')) {
      this.app.browser.addElementToSelector(TweetsTemplate(this), '.notifications-center');
    } else {
      this.app.browser.replaceElementBySelector(TweetsTemplate(this), '.tweets');
    }
  ...
  drawThreadConnectors() {
    ...
    const tweet_els = Array.from(panel_el.querySelectorAll('.tweet:not(.is_embedded)'));

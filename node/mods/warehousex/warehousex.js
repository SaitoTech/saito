...
class Warehousex extends ModTemplate {
	...
	async initialize(app) {
		...
		// Calculate the stats for the last full calendar day
		...
		const checkTime = () => {
			...
			// Calculate how long until midnight again
			let temp1 = new Date(Date.now() + 24 * 60 * 60 * 1000);
			tomorrow = new Date(temp1.getFullYear(), temp1.getMonth(), temp1.getDate());

			let time_until_midnight = tomorrow.getTime() - now.getTime();

			console.log(`WarehouseX: Try again after ${time_until_midnight / 1000} seconds`);

			setTimeout(checkTime, time_until_midnight + 5000);
		};
	...
	async calculateStats(today, tweet = true) {
		...
		try {
			...
			console.log(
				`WarehouseX: ${tx_ct} txs and ${unique_user_count} unique users in the 24 hours prior to ${today}`
			);

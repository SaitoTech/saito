  ...
  attachEvents() {
    ...
    if (document.getElementById('admin')) {
      document.getElementById('admin').onclick = (e) => {
        ...
        contactList.render(
          thisobj.chat_group.members.filter((x) => thisobj.chat_group.member_ids[x] == 1)
        );
      };
    }
    ...
    if (document.getElementById('leave')) {
      document.getElementById('leave').onclick = async (e) => {
        await this.mod.sendRemoveMemberTransaction(thisobj.chat_group, this.mod.publicKey);
    ...
    if (document.getElementById('remove')) {
      document.getElementById('remove').onclick = async (e) => {
        ...
        contactList.callback = async (person) => {
          await this.mod.sendRemoveMemberTransaction(thisobj.chat_group, person);
    ...
    if (document.getElementById('view')) {
      document.getElementById('view').onclick = async (e) => {
        ...
        console.log(
          ...
          JSON.parse(JSON.stringify(thisobj.chat_group.member_ids))
        );
        ...
        Array.from(document.querySelectorAll('.remove_user')).forEach((member) => {
          member.onclick = async (e) => {
            ...
            if (person) {
              await thisobj.mod.sendRemoveMemberTransaction(chat, person);

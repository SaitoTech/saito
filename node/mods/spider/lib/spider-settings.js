...
class Settings {
  ...
  attachEvents() {
    ...
    Array.from(document.querySelectorAll("input[name='spider-difficulty']")).forEach((radio) => {
      radio.addEventListener('change', (e) => {
        if (e.currentTarget.value !== this.mod.loadGamePreference('spider-difficulty')) {
    ...
    Array.from(document.querySelectorAll("input[name='play_mode']")).forEach((radio) => {
      radio.addEventListener('change', (e) => {
        ...
        if (e.currentTarget.value !== this.mod.loadGamePreference('spider-play-mode')) {

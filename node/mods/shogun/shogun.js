...
class Shogun extends GameTemplate {
      ...
      if (mv[0] == 'turn') {
        ...
        if (this.game.player == player) {
          ...
          this.updateStatusAndListCards(
            `Waiting for ${this.formatPlayer(player)} to take their turn`
          );
      ...
      */
        ...
        if (this.gameOver()) {
          ...
          for (let p = 1; p <= this.game.players.length; p++) {
            ...
            this.game.queue.push(
              `SAFEDEAL\t${p}\t${p}\t${Object.keys(this.game.deck[p - 1].discards).length + this.game.deck[p - 1].crypt.length}`
            );
          }
      ...
      if (mv[0] == 'buy') {
        ...
        */
        ...
        if (player !== this.game.player) {
          this.updateStatus(
            `${this.formatPlayer(player)} bought a ${this.cardToText(card_to_buy)}`
          );
          animationDestination = '.purchase_zone';
          delay = delay * 2;
        }

        let element = this.createGameElement(
      ...
      if (mv[0] == 'play') {
        ...
        if (this.game.player !== player) {
          ...
          $(
            `<div class="card showcard" id="${card_to_play}">${this.returnCardImage(card_to_play)}</div>`
          )
            .hide()
      ...
      if (mv[0] == 'spend') {
        ...
        if (this.game.player == player) {
          this.animationSequence.push({
            callback: this.moveGameElement,
            params: [
              this.copyGameElement(`#controls #${card}:not(.copied_elem)`),
              '#discardpile',
              {
                callback: () => {
          ...
          if (optional) {
            this.bindBackButtonFunction(() => {
              if (card_list.length > 0) {
                we_self.addMove(
                  `NOTIFY\t${this.formatPlayer(player)} moved ${we_self.cardsToText(card_list)} to the their ${target}.`
                );
              }
              we_self.endTurn();
        ...
        if (this.game.player == victim) {
          ...
          if (victim != player) {
            if (this.hasCardInHand('moat')) {
              ...
              this.addMove(
                `NOTIFY\t${this.formatPlayer(victim)} is protected from ${this.cardToText(card_to_play)} by a ${this.cardToText('moat')}.`
              );
            } else {
              switch (card_to_play) {
                case 'bandit':
      ...
      if (mv[0] == 'bandit') {
        ...
        for (let t of trash) {
          ...
          this.game.queue.push(
            `NOTIFY\t${this.formatPlayer(mv[2])} trashes a ${this.cardToText(this.game.pool[victim - 1].cards[t])} because of the ${this.cardToText('bandit')}`
          );
        }
        for (let t of discard) {
          this.game.queue.push(`DISCARD\t${victim}\t${t}`);
          this.game.queue.push(
            `NOTIFY\t${this.formatPlayer(mv[2])} discards a ${this.cardToText(this.game.pool[victim - 1].cards[t])} because of the ${this.cardToText('bandit')}`
          );
        }
      ...
      if (mv[0] == 'bureaucrat') {
        ...
        if (vpcards.length > 0) {
          ...
          this.addMove(
            `NOTIFY\t${this.cardToText('bureaucrat')}: ${this.formatPlayer(victim)} puts a ${this.cardToText(vpcards[0])} back on top of the deck.`
          );
        } else {
          this.addMove(
            `augment\t${this.formatPlayer(victim)} does not have a victory card in their hand\t${JSON.stringify(hand)}`
          );
          this.addMove(
            `NOTIFY\t${this.cardToText('bureaucrat')}: ${this.formatPlayer(victim)} does not have a victory card in their hand.`
          );
        }
        this.endTurn();
        return 0;
      ...
      if (mv[0] == 'spy') {
        ...
        let card =
          this.game.pool[victim - 1].hand.length > 0
            ...
            : 'Deck empty';
        let player_display =
          this.game.player === victim ? 'My top card:' : `${this.formatPlayer(victim)}'s top card:`;
        ...
        if (this.game.player == spy) {
          ...
          we_self.attachCardboxEvents(function (action) {
            ...
            if (action == 'deck') {
              ...
              we_self.addMove(
                `NOTIFY\t${we_self.cardToText('spy')}: ${this.formatPlayer(victim)} reveals a ${we_self.cardToText(card)} and puts it back on top of the deck.`
              );
            } else {
              we_self.addMove(`DISCARD\t${mv[1]}\t${we_self.lastCardKey}`);
              we_self.addMove(
                `NOTIFY\t${we_self.cardToText('spy')}: ${this.formatPlayer(victim)} discards a ${we_self.cardToText(card)}.`
              );
            }
          ...
          this.updateStatusAndListCards(
            `${this.cardToText('spy')}: ${this.formatPlayer(spy)} deciding what to do with ${player_display.toLowerCase()}`
          );
        }
        return 0;
      }
      ...
      if (mv[0] == 'thief') {
        ...
        this.addMove(
          `NOTIFY\t${this.cardToText('thief')}: ${this.formatPlayer(victim)} reveals ${this.cardToText(cards[0])} and ${this.cardToText(cards[1])}`
        );
        this.endTurn();
        this.updateStatusAndListCards(
      ...
      if (mv[0] == 'thief2') {
        ...
        if (tcards.length == 0) {
          ...
          this.addMove(
            `NOTIFY\t${this.formatPlayer(victim)} trashes a ${this.cardToText(this.lastCardValue)} because of the ${this.cardToText('thief')}`
          );
          if (tcards[0] == card1) {
            this.removeCardFromPool(card2, victim);
          } else {
          ...
          this.updateStatusAndListCards(
            `Choose a card from ${this.formatPlayer(victim)} to trash:`,
            tcards
          );
        ...
        if (this.game.player == player) {
          if (card) {
            ...
            this.attachCardboxEvents(function (action) {
              if (action == 'yes') {
                we_self.addMove(
                  `NOTIFY\t${this.formatPlayer(player)} decides to keep ${this.formatPlayer(victim)}'s ${this.cardToText(card)}`
                );
        ...
        if (this.game.player == player) {
          ...
          if (this.deck[cname].type.includes('action')) {
            ...
            we_self.attachCardboxEvents(function (action) {
              ...
              we_self.addMove(`DISCARD\t${mv[1]}\t${we_self.lastCardKey}`);
              we_self.addMove(
                `NOTIFY\t${this.formatPlayer(mv[1])} discards a ${we_self.cardToText(cname)}`
              );
              this.endTurn();
            });
          }
  ...
  filterHud(filter = '') {
    document.querySelectorAll('#controls .card').forEach((card) => {
      ...
      if (
        filter === '' ||
        this.deck[cardid].type == filter ||
        this.deck[cardid].type.includes(filter)
      ) {
  ...
  playerTurn() {
    ...
    if (this.game.state.buys > 0) {
      ...
      this.updateStatusAndListCards(
        `You have ${available_coins} coins and ${this.game.state.buys} card${this.game.state.buys > 1 ? 's' : ''} to buy`
      );
      this.shop.buyCard(
        `You have ${available_coins} coins and ${this.game.state.buys} card${this.game.state.buys > 1 ? 's' : ''} to buy`,
        available_coins,
        ...
        (newcard) => {
          if (we_self.deck[newcard].cost <= available_coins) {
            ...
            we_self.addMove(
              `NOTIFY\t${this.formatPlayer(we_self.game.player)} bought a ${we_self.cardToText(newcard)}.`
            );
  ...
  acquireCard(max_value, target = '') {
    ...
    this.shop.buyCard(
      `You may select a card worth up to ${max_value}`,
      max_value,
      false,
      (newcard) => {
        if (we_self.deck[newcard].cost <= max_value) {
          ...
          we_self.addMove(
            `NOTIFY\t${we_self.formatPlayer(we_self.game.player)} acquired a ${we_self.cardToText(newcard)}.`
          );
          we_self.endTurn();
        } else {
          console.error('Daniel bug 1');

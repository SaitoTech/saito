...
class NodeSetup {
  ...
  render() {
    if (!document.querySelector('.node-setup-options')) {
      this.app.browser.addElementToSelector(NodeSetupTemplate(this.mod), this.container);
    } else {
      this.app.browser.replaceElementBySelector(NodeSetupTemplate(this.mod), this.container);
    }
  ...
  attachEvents() {
    ...
    document.querySelectorAll('.node-setup-card').forEach((card) => {
      card.onclick = async () => {
        ...
        // apply configuration
        const updatedOptions = await this.configureOptionsForChoice(currentOptions, choice);

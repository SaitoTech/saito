...
class Board {
  ...
  render(enable = false) {
    ...
    if (!document.querySelector('.poker-board')) {
      this.app.browser.addElementToSelector(BoardTemplate(), '.poker-card-zone');
    }
    ...
    try {
      ...
      for (let i = 0; i < 5 || i < pool_hand.length; i++) {
        if (i < pool_hand.length && i < this.cards_visible) {
          ...
          let card = pool_cards[pool_hand[i]];
          newHTML += `<div class="flipped slot${i + 1} card"><img class="cardFront" src="${poker_self.card_img_dir}/${card.name}"><img class="cardBack" src="/poker/img/cards/red.png"></div>`;
        } else {
          newHTML += `<div class="flipped slot${i + 1} card"><img class="cardBack" src="/poker/img/cards/red.png"></div>`;
        }
      }

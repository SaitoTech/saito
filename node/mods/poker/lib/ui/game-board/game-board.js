...
class GameBoard {
  ...
  displayTable() {
    ...
    let animate_flip =
      this.game_mod.game.state.flipped > 0 && this.game_mod.game.state.flipped > this.cards_visible;
    ...
    try {
      if (document.getElementById('deal')) {
        let newHTML = '';
        for (let i = 0; i < 5 || i < poker_self.game.pool[0].hand.length; i++) {
          let card = {};
      ...
      this.timer = setTimeout(() => {
        ...
        if (animate_flip && this.cards_visible < poker_self.game.pool[0].hand.length) {
          ...
          console.log(
            'Skip animation: ',
            animate_flip,
            this.game_mod.game.state.flipped,
            this.cards_visible,
            poker_self.game.pool[0].hand.length
          );
        }

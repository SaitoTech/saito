...
class Sidebar {
  ...
  render() {
    ...
    if (!document.querySelector('.poker-sidebar')) {
      this.app.browser.addElementToSelector(SidebarTemplate(), '.poker-main');
    }
  ...
  renderPlayerboxes() {
    ...
    for (let i = 0; i < this.playerboxes.length; i++) {
      this.playerboxes[i].render('.poker-sidebar');
    }
  }
  ...
  setActive(player_number, deactivate_others = true) {
    if (deactivate_others) {
      document.querySelectorAll('.poker-playerbox.active').forEach((el) => {

...
class Result {
  ...
  render() {
    ...
    if (!document.querySelector('.poker-result')) {
      this.app.browser.addElementToSelector(ResultTemplate(), '.poker-table');
    }
  }

  show(opts = {}) {
    this.render();
    let host = document.getElementById('poker-result');
    if (!host) {
      return;
    }

    let cards_el = document.getElementById('poker-result-cards');
    let headline_el = document.getElementById('poker-result-headline');
    let hand_el = document.getElementById('poker-result-hand');
  ...
  tickCountdown() {
    let el = document.getElementById('poker-result-countdown');
    if (!el || !this.deadline) {
      return;
    }

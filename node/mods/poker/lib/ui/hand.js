/**
 * Thin Poker adapter around the shared GameCardfan.
 * Keeps the original poker hand rendering path (direct <img class="card">)
 * while mounting into the stable .poker-play slot.
 */
...
class Hand extends GameCardfan {
  ...
  render(cards_html = '') {
    ...
    if (!document.querySelector('.poker-play')) {
      return;
    }
    ...
    if (this.el) {
      this.el.classList.add('poker-hand');

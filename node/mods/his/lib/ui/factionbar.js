...
class FactionBarOverlay {
  ...
  render(faction = '') {
    ...
    if (this.mod.game.state.players.length == 2) {
      return;
    }
    if (this.visible) {
      return;
    ...
    for (let z = 0; z < factions.length; z++) {
      let active_faction = '';
      if (this.mod.returnPlayerCommandingFaction(factions[z]) === this.mod.game.player) {
        active_faction = '*';
      }
      let f = factions[z];
      this.app.browser.addElementToSelector(
        `<div class="factionbar-faction ${f}" id="${f}">${f}${active_faction}</div>`,
        '.factionbar'
      );
    }

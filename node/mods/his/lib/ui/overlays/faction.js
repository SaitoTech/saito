...
class FactionOverlay {
  ...
  render(faction = '') {
    ...
    this.app.browser.addElementToSelector(
      `<div class="faction_sheet_notice ${f.key}" id="faction_sheet_notice"></div>`,
      '.faction_sheet'
    );
    ...
    for (let key in his_self.game.spaces) {
      if (his_self.game.spaces[key].type === 'key') {
        ...
        if (owner == '') {
          owner = his_self.game.spaces[key].home;
        }
    ...
    if (his_self.factions[faction].key === 'papacy') {
      ...
      if (exhtml != '') {
        this.app.browser.addElementToSelector(exhtml, '.faction_sheet');
      }
      ...
      for (let i = 0; i < 12; i++) {
        if (his_self.game.state.saint_peters_cathedral['state'] == i) {
          keyboxen += `<div class="faction_sheet_keytile papacy_construction_status${
            i + 1
          } saint_peters_tile" id="papacy_construction_status_keytile${i + 1}"></div>`;
        }
        if (his_self.game.state.saint_peters_cathedral['vp'] == i) {
          keyboxen += `<div class="faction_sheet_keytile papacy_construction_status${
            7 + i
          } papacy_vp_tile" id="papacy_construction_status_keytile${7 + i}"></div>`;
        }
      }
      for (let i = 1; i <= 7; i++) {
        if (i > controlled_keys) {
    ...
    for (let key in his_self.game.state.newworld) {
      if (
        his_self.game.state.newworld[key].type == 'discovery' &&
        his_self.game.state.newworld[key].claimed == 1 &&
        his_self.game.state.newworld[key].faction == faction
      ) {
        this.app.browser.addElementToSelector(

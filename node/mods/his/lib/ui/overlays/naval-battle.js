...
class NavalBattleOverlay {
  ...
  renderFortification(res = {}) {
    ...
    this.updateInstructions(
      'A Naval Battle imminent in ' + this.mod.returnSpaceName(res.spacekey) + ': Fortification?'
    );
  }
  ...
  assignHitsManually(res = {}, faction = '', hits_to_assign = 1) {
    ...
    if (
      this.mod.game.state.naval_battle.attacker_hits >
        this.mod.game.state.naval_battle.defender_hits &&
      this.mod.returnControllingPower(faction) ==
        this.mod.returnControllingPower(this.mod.game.state.naval_battle.defender_faction)
    ) {
      did_i_lose_and_need_to_assign_last_hit = true;
    }
    if (
      this.mod.game.state.naval_battle.attacker_hits <=
        this.mod.game.state.naval_battle.defender_hits &&
      this.mod.returnControllingPower(faction) ==
        this.mod.returnControllingPower(this.mod.game.state.naval_battle.attacker_faction)
    ) {
      did_i_lose_and_need_to_assign_last_hit = true;
    }
    ...
    document.querySelectorAll(qs2).forEach((el) => {
      ...
      if (unit_owner.indexOf(' / ')) {
        unit_owner = unit_owner.split(' / ')[0];
      }
      if (unit_type == 'corsair') {
        undestroyed_corsairs++;
      }
      if (unit_type == 'squadron') {
        undestroyed_squadrons++;
      }
    });
    ...
    document.querySelectorAll(qs2).forEach((el) => {
      try {
        ...
        if (
          (his_self.returnControllingPower(factionspace) ===
            his_self.returnControllingPower(faction) ||
            his_self.returnControllingPower(unit_owner) ==
              his_self.returnControllingPower(faction)) &&
          his_self.game.player ===
            his_self.returnPlayerCommandingFaction(his_self.returnControllingPower(faction))
        ) {
        ...
        if (can_i_kill_this_guy) {
          ...
          el.onclick = (e) => {
            ...
            //
            if (
              undestroyed_corsairs == 0 &&
              undestroyed_squadrons > 0 &&
              did_i_lose_and_need_to_assign_last_hit == true
            ) {
              if (hits_left == 1) {
            ...
            document.querySelectorAll('.hits_to_assign').forEach((el2) => {
            ...
            this.mod.addMove(
              'naval_battle_destroy_unit\t' + faction + '\t' + spacekey + '\t' + unit_type
            );
            ...
            if (
              ...
              document.querySelectorAll('.hits-assignable').forEach((el3) => {
                el3.onclick = (e) => {};
              });
              this.updateInstructions(
  ...
  render(res = {}, pre_battle = 0) {
    ...
    if (
      this.mod.returnPlayerFactions(this.mod.game.player).includes(res.attacker_faction) ||
      this.mod.returnPlayerCommandingFaction(res.attacker_faction) == this.mod.game.player
    ) {
      am_i_attacker = true;
    }
    if (
      this.mod.returnPlayerFactions(this.mod.game.player).includes(res.defender_faction) ||
      this.mod.returnPlayerCommandingFaction(res.defender_faction) == this.mod.game.player
    ) {
      am_i_defender = true;
    }
    ...
    for (let i = 0; i < res.attacker_units.length; i++) {
      ...
      if (res.attacker_units_owners.length > i) {
        faction_owner = res.attacker_units_owners[i];
      }
      if (res.attacker_units_owners.length <= i) {
        faction_owner = res.attacker_units_owners[res.attacker_units_owners.length - 1];
      }
      let how_many_hits = 1;
      if (unit_type === 'squadron') {
        how_many_hits = 2;
      ...
      for (let z = 0; z < how_many_hits; z++) {
        ...
        if (faction_owner != faction_name) {
          faction_name = faction_name + ' / ' + faction_owner;
        }
    ...
    for (let i = 0; i < res.defender_units.length; i++) {
      ...
      let faction_owner = faction_name;
      if (res.defender_units_owners.length > i) {
        faction_owner = res.defender_units_owners[i];
      }
      if (res.defender_units_owners.length <= i) {
        faction_owner = res.defender_units_owners[res.defender_units_owners.length - 1];
      }
      let how_many_hits = 1;
      if (unit_type === 'squadron') {
        how_many_hits = 2;

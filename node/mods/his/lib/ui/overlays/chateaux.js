...
class ChateauxOverlay {
  ...
  render(faction = 'france') {
    ...
    document.querySelector('.chateaux-overlay .help').innerHTML =
      `${this.mod.returnFactionName(faction)} rolls ${this.roll} (modified: ${modified_roll})`;
    ...
    if (modified_roll >= 8) {
      ...
      this.mod.game.queue.push(
        `ACKNOWLEDGE\t${this.mod.returnFactionName(faction)} rolls on the Chateaux Table`
      );
    }

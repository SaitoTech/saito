...
class AssaultOverlay {
  ...
  assignHitsManually(res = {}, faction = '', hits_to_assign = 1) {
    ...
    if (this.mod.game.player == this.mod.returnPlayerCommandingFaction(res.attacker_faction)) {
      am_i_attacker = true;
    }
    if (this.mod.game.player == this.mod.returnPlayerCommandingFaction(res.defender_faction)) {
      am_i_defender = true;
    }
    ...
    if (side === '.attacker') {
      ...
      //
      if (
        this.mod.game.spaces[res.spacekey].political == 'ottoman' ||
        this.mod.game.spaces[res.spacekey].home == 'ottoman'
      ) {
  ...
  render(res = {}, pre_battle = 0) {
    ...
    if (this.mod.returnPlayerFactions(this.mod.game.player).includes(res.attacker_faction)) {
      am_i_attacker = true;
    }
    if (this.mod.returnPlayerFactions(this.mod.game.player).includes(res.defender_faction)) {
      am_i_defender = true;
    }

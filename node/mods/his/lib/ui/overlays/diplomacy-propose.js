...
class DiplomacyProposeOverlay {
  ...
  render(faction = '') {
    ...
    if (faction != '' && !this.proposal.parties.includes(faction)) {
      this.proposal.parties.push(faction);
    }
          ...
          menu[id].fnct(his_self, faction, (terms) => {
            for (let z = 0; z < terms.length; z++) {
              ...
              for (let y = 0; y < io.length; y++) {
                if (terms[z].indexOf(io[y]) > -1 && !this.proposal.parties.includes(io[y])) {
                  this.proposal.parties.push(io[y]);
                }
                if (
                  terms[z].indexOf('marital') > -1 &&
                  !this.proposal.parties.includes('papacy') &&
                  io[y] == 'papacy'
                ) {
                  this.proposal.parties.push('papacy');
                }
                if (
                  terms[z].indexOf('marriage') > -1 &&
                  !this.proposal.parties.includes('papacy') &&
                  io[y] == 'papacy'
                ) {
                  this.proposal.parties.push('papacy');
  ...
  renderAllProposals(faction = '') {
    ...
    if (any_proposals) {
      this.updateInstructions(
        `${this.mod.returnFactionName(faction)} - Diplomacy Stage - All Proposals`
      );
  ...
  renderCurrentProposal() {
    ...
    if (any_proposals) {
      document.querySelector('.diplomacy-propose-overlay .help').innerHTML =
        'your proposal consists of the following:';
      document.querySelector('.diplomacy-propose-overlay .content').innerHTML = proposals_html;
      document.querySelector('.diplomacy-propose-overlay .content').style.display = 'block';
    } else {
      document.querySelector('.diplomacy-propose-overlay .help').innerHTML =
        'add term to your new proposal';

...
class CouncilOfTrentOverlay {
  ...
  render(stage = 'papacy') {
    ...
    if (stage == 'papacy') {
      ...
      if (this.mod.game.player != papacy_player) {
        document.querySelector('.council-of-trent-overlay .help').innerHTML =
          'Papacy is selecting up to 4 Debaters.';
      } else {
        document.querySelector('.council-of-trent-overlay .help').innerHTML =
          `Papacy - select up to 4 Debaters. (<span class="finish-selecting-debaters">click when done</span>)`;
      }
    }
    if (stage == 'protestant') {
      ...
      if (this.mod.game.player != protestants_player) {
        document.querySelector('.council-of-trent-overlay .help').innerHTML =
          'Protestants are selecting up to 2 Debaters.';
      } else {
        document.querySelector('.council-of-trent-overlay .help').innerHTML =
          `Protestants - select up to 2 Debaters. (<span class="finish-selecting-debaters">click when done</span>)`;
      }
    }
    if (stage == 'results') {
      let papacy_total = 0;
      ...
      if (protestants_hits === papacy_hits) {
        document.querySelector('.council-of-trent-overlay .help').innerHTML +=
          'Council Inconclusive - No Winner';
    ...
    if (
      ...
      (this.mod.game.player == protestants_player && stage == 'protestant')
    ) {
      this.attachEvents(stage);
    }
  }
  ...
  attachEvents(stage = 'papacy') {
    ...
    try {
      document.querySelector('.finish-selecting-debaters').onclick = (e) => {
        ...
        if (this.mod.game.player == papacy_player && stage == 'papacy') {
          this.mod.addMove(
            'council_of_trent_add_debaters\tpapacy\t' + JSON.stringify(this.selected)
          );
          this.mod.endTurn();
        }
        if (this.mod.game.player == protestants_player && stage == 'protestant') {
          this.mod.addMove(
            'council_of_trent_add_debaters\tprotestant\t' + JSON.stringify(this.selected)
          );
          this.mod.endTurn();
        }
      };
    ...
    try {
      document
        .querySelectorAll('.council-of-trent-overlay .debaters .debater-card')
        .forEach((el) => {
          el.onclick = (e) => {

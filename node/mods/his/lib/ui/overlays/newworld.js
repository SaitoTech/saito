...
class NewWorldOverlay {
  ...
  render(stage = '', round = 0) {
    ...
    if (round == 0) {
      round == his_self.game.state.round;
    }
    for (let z = 0; z < his_self.game.state.colonies.length; z++) {
      if (his_self.game.state.colonies[z].round > round) {
        round = his_self.game.state.colonies[z].round;
      }
    }
    for (let z = 0; z < his_self.game.state.conquests.length; z++) {
      if (his_self.game.state.conquests[z].roundd > round) {
        round = his_self.game.state.conquests[z].round;
      }
    }
    for (let z = 0; z < his_self.game.state.explorations.length; z++) {
      if (his_self.game.state.explorations[z].round > round) {
        round = his_self.game.state.explorations[z].round;
      }
    }
          ...
          this.returnRowHTML(
            {
              ...
              name: c.name,
              faction: c.faction,
              total_hits: c.modified_roll,
              bonus_card: bonus_card
            },
            stage
          ),
          '.new-world-overlay .content .colonies'
        );
      } else {
    ...
    for (let i = 0; i < his_self.game.state.conquests.length; i++) {
      ...
      if (
        c.round == round ||
        c.prize.indexOf('Aztec') > -1 ||
        c.prize.indexOf('Maya') > -1 ||
        c.prize.indexOf('Inca') > -1
      ) {
        active_conquests++;
        //
        ...
        if (bonus_card == 1 && c.depleted != 1) {
          his_self.app.browser.addElementToSelector(
            this.returnRowHTML(
              {
                ...
                name: c.prize,
                faction: c.faction,
                conquistador: c.conquistador,
                total_hits: c.bonus_base_roll,
                depleted: 0,
                bonus_card: bonus_card
              },
              stage,
            ...
            '.new-world-overlay .content .conquests'
          );
        } else {
          //
          // conquest depleted
      ...
      if (exp.round == round) {
        ...
        his_self.app.browser.addElementToSelector(
          this.returnRowHTML(
            {
              prize: exp.prize,
              img: exp.explorer_img,
              type: 'exploration',
              name: exp.name,
              faction: exp.faction,
              explorer: exp.explorer,
              total_hits: exp.modified_roll
            },
            stage,
            false
          ),
          '.new-world-overlay .content .explorations'
        );
      }
    }
  ...
  returnRowHTML(obj = {}, stage = '', show_hits_as_goldenrod = true) {
    if (stage === 'results') {
      ...
      if (obj.img != '') {
        img = `background-image:url('${obj.img}')`;
      }
      if (obj.total_hits != '') {
        total_hits = obj.total_hits;
      }
      if (obj.prize != '') {
        prize = obj.prize;
      }
      ...
      if (prize.indexOf('killed') > -1) {
        row_css += ' killed';
      }
      if (prize.indexOf('depleted') > -1) {
        row_css += ' killed';
      }
      if (prize.indexOf('lost at sea') > -1) {
        row_css += ' killed';
      }
      if (obj.bonus_card == 1) {
        goldenrod = 'goldenrod';
      ...
      if (show_hits_as_goldenrod == false) {
        goldenrod = '';
      }

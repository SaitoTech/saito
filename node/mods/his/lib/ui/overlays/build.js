...
class BuildOverlay {
  ...
  render(faction, unit, ops, cost, mycallback) {
    ...
    if (unit === 'mercenary') {
      document.querySelector('.unit-details').style.backgroundImage =
        'linear-gradient(rgba(0, 0, 0, 0.527),rgba(0, 0, 0, 0.5)) , url(/his/img/backgrounds/move/mercenary.jpg)';
    }
    if (unit === 'cavalry') {
      document.querySelector('.unit-details').style.backgroundImage =
        'linear-gradient(rgba(0, 0, 0, 0.527),rgba(0, 0, 0, 0.5)) , url(/his/img/backgrounds/move/cavalry.jpg)';
    }
    if (unit === 'corsair') {
      document.querySelector('.unit-details').style.backgroundImage =
        'linear-gradient(rgba(0, 0, 0, 0.527),rgba(0, 0, 0, 0.5)) , url(/his/img/backgrounds/move/corsair.jpg)';
    }
    if (unit === 'squadron') {
      document.querySelector('.unit-details').style.backgroundImage =
        'linear-gradient(rgba(0, 0, 0, 0.527),rgba(0, 0, 0, 0.5)) , url(/his/img/backgrounds/move/squadron.jpg)';
    }
    this.mod.available_units_overlay.hide();
  ...
  attachEvents(faction, unit, ops, cost, mycallback) {
    document.querySelector('.fewer-units').onclick = (e) => {
      if (this.units == 1) {
        alert('cannot produce less than one unit');
        return;
      }
      this.units--;
    ...
    document.querySelector('.more-units').onclick = (e) => {
      if (this.ops_available < this.cost * (this.units + 1)) {
        alert('not enough OPs to produce more');
        return;
      }
      if (this.max_units <= this.units) {
        alert('at unit production limit...');
        return;
      }
      this.units++;
      this.ops_spent = this.ops_available - this.cost * this.units;

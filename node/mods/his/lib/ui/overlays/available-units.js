...
class AvailableUnitsOverlay {
  ...
  fadeOut(force = false) {
    try {
      if (this.units_already_moved_by_idx.length == 0 || force == true) {
        this.faded_out = true;
        document.querySelector('.movement-overlay .available-units-overlay').style.backgroundColor =
          '#0009';
  ...
  renderMove(mobj, faction, spacekey) {
    let max_formation_size = this.mod.returnMaxFormationSize(mobj.units_to_move, faction, spacekey);
        ...
        if (x.deployed[spacekey]) {
          ...
          if (us['mercenary']) {
            for (let i = 0; i < us['mercenary']['1']; i++) {
              document.querySelector(qs).innerHTML += this.returnTile(
                f,
                this.added,
                'mercenary',
                1
    ...
    document.querySelectorAll('.available-units-overlay .army_tile').forEach((el) => {
      el.onclick = (e) => {
        ...
        for (let z = 0; z < mobj.moved_units.length; z++) {
          if (
            mobj.moved_units[z].type == 'regular' ||
        ...
        if (max_formation_size < num + existing_land_units && adding == true) {
          alert(
            'You cannot move these units without surpassing your max formation capacity of ' +
              max_formation_size
          );
          if (e.currentTarget.classList.contains('nonopaque')) {
            e.currentTarget.classList.remove('nonopaque');
        ...
        if (adding == true) {
          ...
          for (let i = 0; i < num; i++) {
            ...
            for (let z = 0; z < mobj.unmoved_units.length; z++) {
              if (
                mobj.unmoved_units[z].faction == faction &&
                mobj.unmoved_units[z].type == type &&
                moved == false
              ) {
          ...
          for (let z = 0; z < this.units_already_moved_by_idx.length; z++) {
            if (is_army_leader) {
              if (
                this.is_unit_already_moved_by_idx_a_leader[z] == 1 &&
                this.units_already_moved_by_idx[z] === idx
              ) {
                ...
                this.is_unit_already_moved_by_idx_a_leader = [];
                z = 10000;
                for (let ii = mobj.moved_units.length - 1; ii >= 0; ii--) {
                  mobj.unmoved_units.push(mobj.moved_units[ii]);
                }
                mobj.moved_units = [];
                mobj.units_to_move = [];
              ...
              if (
                this.is_unit_already_moved_by_idx_a_leader[z] == 0 &&
                this.units_already_moved_by_idx[z] === idx
              ) {
                this.units_already_moved_by_idx.splice(z, 1);
                this.is_unit_already_moved_by_idx_a_leader.splice(z, 1);
          ...
          for (let i = 0; i < num; i++) {
            ...
            for (let z = 0; z < mobj.moved_units.length && moved == false; z++) {
              if (
                mobj.moved_units[z].faction == faction &&
                mobj.moved_units[z].type == type &&
                moved == false
              ) {
                let mrem = false;
                for (let yy = 0; yy < mobj.units_to_move.length; yy++) {
                  if (
                    mobj.units_to_move[yy].faction == faction &&
                    mobj.units_to_move[yy].type == type &&
                    mrem == false
                  ) {
    ...
    if (leader == false) {
      if (this.units_already_moved_by_idx.includes(added)) {
        for (let z = 0; z < this.units_already_moved_by_idx.length; z++) {
          if (
            this.is_unit_already_moved_by_idx_a_leader[z] == 0 &&
            this.units_already_moved_by_idx[z] === added
          ) {
            muc = 'nonopaque';
          }
        }

  ...
  render(menu, player, faction, ops, attachEventsToOptions = null) {
    ...
    let main_menu = (main_menu) => {
      ...
      for (let i = 0; i < menu.length; i++) {
        ...
        if (!menu[i].check(this.mod, this.mod.game.player, faction, ops)) {
      ...
      if (menu.length > 3) {
        ...
        let special_html = `
				  ...
				  `;
        if (special.length > 0) {
          this.app.browser.addElementToSelector(special_html, `.movemenu`);
          let content = '';
      ...
      for (let i = 0; i < menu.length; i++) {
        ...
        if (cost != 100) {
          if (active_option !== 'inactive') {
            this.app.browser.addElementToSelector(
              this.returnMenuHTML(menu[i].name, menu[i].img, active_option, menu[i].cost),
              `.movemenu`
            );
          }
        }

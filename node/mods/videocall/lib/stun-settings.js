...
class Settings {
  ...
  async loadMediaDevices(currentStream) {
    ...
    const used_devices = currentStream.getTracks().map((track) => track.getSettings().deviceId);
    ...
    if (videoInput) {
      videoInput.addEventListener('change', () =>
        this.mod.streams.updateInputs('video', videoInput.value)
      );
    }

    if (audioInput) {
      audioInput.addEventListener('change', () =>
        this.mod.streams.updateInputs('audio', audioInput.value)
      );
    }
  }

  ...
  return new Promise((resolve, reject) => {
    ...
    sendExplorerPeerRequest(app, EXPLORER_SUBMIT_FEE_TRANSACTION_REQUEST, {
      ...
      callback: (response) => {
        ...
        if (!response?.success) {
          reject(new Error(response?.error || 'Explorer server rejected the transaction.'));
          return;
        }
        resolve(response);
      }
...
async function addFeeTransaction(app, feeSaito, explorerPeer, onStep) {
  ...
  const newtx = await app.wallet.createUnsignedTransaction(publicKey, BigInt(0), feeNolan);
  await newtx.sign();

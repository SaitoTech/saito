...
const { parseSimulationFeeSaito, addFeeTransaction } = require('../simulation-transaction');
...
class Supply {
  ...
  paint() {
    ...
    const error = this.mod.supplyError ? this.app.browser.escapeHTML(this.mod.supplyError) : null;
    ...
    this.app.browser.replaceElementContentBySelector(
      SupplyTemplate({
        loading,
        error,
        produceError: this.produceError ? this.app.browser.escapeHTML(this.produceError) : null,
  ...
  setProducingState(isProducing) {
    ...
    root
      .querySelectorAll('.explorer-supply-admin-controls button.explorer-back-link')
      .forEach((button) => {
  ...
  attachEvents() {
    ...
    if (widthToggle && supplyContainer) {
      const toggleFullWidth = () => {
        ...
        const label = this.fullWidth ? 'Collapse supply dashboard' : 'Expand supply dashboard';

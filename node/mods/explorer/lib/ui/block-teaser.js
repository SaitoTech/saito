...
class BlockTeaser {
	...
	render(container) {
		...
		const error = this.mod.blocksError
			? this.app.browser.escapeHTML(this.mod.blocksError)
			: null;
	...
	attachEvents() {
		document.querySelectorAll('.block-teaser .explorer-feed-item[data-block-hash]').forEach((el) => {

...
class Address {
  ...
  paint() {
    ...
    const error = this.mod.addressError ? this.app.browser.escapeHTML(this.mod.addressError) : null;
  ...
  attachEvents() {
    ...
    root
      .querySelectorAll('.explorer-address-block-link, .explorer-address-tx-link')
      .forEach((link) => {
        link.onclick = (event) => {

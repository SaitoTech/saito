...
class AllTransactions {
	...
	formatTransactions() {
		const app = this.app;
		const formatted = formatTransactionsForTeaser(app, this.rawTransactions, this.rawTransactions.length);
	...
	paint() {
		const transactions = this.loading && !this.rawTransactions.length
			? []
	...
	attachEvents() {
		document.querySelectorAll('.explorer-all-tx-feed .explorer-feed-item[data-tx-signature]').forEach((el) => {
	...
	async loadInitial() {
		...
		try {
			const headerBlocks = await this.requestBlocksPromise(peer, { count: PAGE_SIZE, include_offchain: false });

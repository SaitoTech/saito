...
class Explorer extends ModTemplate {
      ...
      sendExplorerPeerRequest(app, 'request address', {
        ...
        callback: async (response) => {
          ...
          if (!response?.success) {
            ...
            this.addressError =
              response?.error || 'Failed to fetch address activity from Explorer peer.';
            this.addressReady = true;
            await this.refreshActiveView();
            resolve();
            return;
  ...
  renderSimulationToolbar() {
    ...
    el.classList.toggle(
      'is-error',
      Boolean(this.simulationToolbarIsError && this.simulationToolbarMessage)
    );
  }
  ...
  async handlePeerTransaction(app, tx = null, peer, mycallback) {
    ...
    if (produceExplorerBlock) {
      ...
      runManualBlockProductionRequest(app, this, produceExplorerBlock, mycallback, txmsg?.request);
      return 1;
    }

...
class Vote extends ModTemplate {
      ...
      return new Promise(async (resolve, reject) => {
        await this.app.storage.loadTransactions(
          ...
          (txs) => {
            ...
            const electionTxs = txs.filter(
              (tx) => tx.returnMessage().request === 'create election'
            );
            if (electionTxs.length > 0) {
              this.currentElectionId = Math.max(
                ...electionTxs.map((tx) => parseInt(tx.returnMessage().data.electionId) || 0)
              );
  ...
  async sendSubmitVoteTransaction(params = {}, callback = null) {
    ...
    try {
      ...
      const verified = await window.snarkjs.groth16.verify(this.vkey, publicSignals, proof);
      if (!verified) {
        if (callback) {
          callback({
            success: false,
  ...
  async loadVoteLayout() {
    this.app.browser.createReactRoot(
      VoteLayout,
      { app: this.app, mod: this },
      `vote-layout-${Date.now()}`
    );
  }

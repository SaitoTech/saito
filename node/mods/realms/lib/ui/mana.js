...
class Mana {
  ...
  render() {
    ...
    if (document.querySelector(`${this.container} #mana-wheel`)) {
      ...
      this.app.browser.addElementToSelector(ManaTemplate(), this.container);
    }
    ...
    for (let key in mana) {
      if (mana[key] > 0 && swap_in_tapped == 1) {
        swap_in_tapped = 0;
      }
    }
    if (swap_in_tapped) {
      mana = this.mod.returnAvailableMana(this.player, true);
    } // true = include tapped
    ...
    colors.forEach((color) => {
      ...
      if (value > 0) {
        const d = [`M 50 50`, `L ${x1} ${y1}`, `A 50 50 0 ${largeArc} 1 ${x2} ${y2}`, `Z`].join(
          ' '
        );

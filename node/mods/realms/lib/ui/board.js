...
class Board {
  ...
  render() {
    ...
    if (document.querySelector('.gameboard .realms-board')) {
      ...
      this.app.browser.addElementToSelector(BoardTemplate(), '.gameboard');
    }
    ...
    for (let i = 0, slots_needed = 5; i < opponent_cards_on_table.length || i < slots_needed; i++) {
      if (i >= opponent_cards_on_table.length) {
        realms_self.app.browser.addElementToSelector(this.html(''), '.battlefield.opponent');
      } else {
        ...
        if (card.type == 'land') {
          if (cobj.tapped) {
            this.mana_opponent.tapped++;
          } else {
            this.mana_opponent.untapped++;
          }
    ...
    let player_cards_on_table =
      realms_self.game.state.players_info[realms_self.game.player - 1].cards;
    for (let i = 0, slots_needed = 5; i < player_cards_on_table.length || i < slots_needed; i++) {
      if (i >= player_cards_on_table.length) {
        realms_self.app.browser.addElementToSelector(this.html(''), '.battlefield.player');
      } else {
        ...
        if (card.type == 'land') {
          if (cobj.tapped) {
            this.mana_player.tapped++;
          } else {
            this.mana_player.untapped++;
          }
          if (this.mana_depicted == 0) {
            realms_self.app.browser.addElementToSelector(
              this.html('', cobj),
        ...
        if (card.type == 'creature') {
          realms_self.app.browser.addElementToSelector(this.html(key, cobj), '.battlefield.player');
        }
  ...
  attachEvents() {
    ...
    // lands/color-wheel
    //
    document.querySelector(`.player .showcard[data-slot="1"]`).onclick = (e) => {
      this.lands_overlay.render(player);
    };
    document.querySelector(`.opponent .showcard[data-slot="1"]`).onclick = (e) => {
      this.lands_overlay.render(opponent);
    };
  ...
  html(key = '', cobj = null) {
    ...
    if (realms_self.deck[key]) {
      card = realms_self.deck[key];
    }
  ...
  refreshPlayerMana(player_num) {
    if (this.mana_player.player == player_num) {
      this.mana_player.tapped_add_pop = 1;
      this.mana_player.untapped_add_pop = 1;
    }
    if (this.mana_opponent.player == player_num) {
      this.mana_opponent.tapped_add_pop = 1;
      this.mana_opponent.untapped_add_pop = 1;
    }
  }
}

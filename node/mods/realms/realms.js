...
class Realms extends GameTemplate {
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] === 'counter_or_acknowledge') {
        ...
        if (
          ...
          realms_self.game.queue[realms_self.game.queue.length - 1] =
            'HALTED\tWaiting for Game to Continue\t' + realms_self.publicKey;
  ...
  canPlayerPlayCard() {
    ...
    for (let z = 0; z < p.cards.length; z++) {
      ...
      if (
        card.type == 'land' &&
        this.game.state.players_info[this.game.player - 1].land_played == 0
      ) {
        return 1;
      }
      if (this.canPlayerCastSpell(p.cards[z].key, mana)) {
        return 1;
      }
    }
  ...
  returnAvailableMana(player = 0, include_tapped = false) {
    ...
    for (let z = 0; z < p.cards.length; z++) {
      ...
      if (p.cards[z].tapped == false || include_tapped == true) {
        if (card.type == 'land' && card.color == 'black') {
          black_mana++;
        }
        if (card.type == 'land' && card.color == 'red') {
          red_mana++;
        }
        if (card.type == 'land' && card.color == 'green') {
          green_mana++;
        }
        if (card.type == 'land' && card.color == 'blue') {
          blue_mana++;
        }
        if (card.type == 'land' && card.color == 'white') {
          white_mana++;
        }
      }
    }
  ...
  canPlayerCastSpell(cardkey = '', mana = {}) {
    ...
    //
    if (!mana.total) {
      mana = this.returnAvailableMana(this.game.player);
    }
    ...
    //
    let total_needed =
      red_needed + green_needed + black_needed + white_needed + blue_needed + any_needed;

    if (mana.green < green_needed) {
      return 0;
    }
    if (mana.red < red_needed) {
      return 0;
    }
    if (mana.black < black_needed) {
      return 0;
    }
    if (mana.white < white_needed) {
      return 0;
    }
    if (mana.blue < blue_needed) {
      return 0;
    }
    if (mana.total < total_needed) {
      return 0;
    }
    ...
    this.updateStatusAndListCards(
      ...
      function (cardname) {
        ...
        if (card.type == 'land') {
          ...
          this.addMove(
            `counter_or_acknowledge\t${realms_self.returnPlayerUsername(this.game.player)} places ${this.popup(cardname)}\tdeploy_land\t${card}`
          );
        ...
        if (card.type == 'creature') {
          ...
          this.addMove(
            `counter_or_acknowledge\t${realms_self.returnPlayerUsername(this.game.player)} casts ${this.popup(cardname)}\tdeploy_creature\t${card}`
          );
          this.addMove('RESETCONFIRMSNEEDED\tall');
          this.addMove(`discard\t${realms_self.game.player}\t${cardname}`);
          this.endTurn();
        ...
        if (card.type == 'artifact') {
          ...
          this.addMove(`spend\t${realms_self.game.player}\tcreature\t${JSON.stringify(card.cost)}`);
          this.addMove(
            `counter_or_acknowledge\t${realms_self.returnPlayerUsername(this.game.player)} casts ${this.popup(cardname)}\tdeploy_artifact\t${card}`
          );
          this.addMove('RESETCONFIRMSNEEDED\tall');
          this.addMove(`discard\t${realms_self.game.player}\t${cardname}`);
          this.endTurn();
        ...
        if (card.type == 'sorcery') {
          this.deploy(realms_self.game.player, cardname);
          this.addMove(
            `deploy\tsorcery\t${realms_self.game.player}\t${cardname}\t${realms_self.game.player}`
          );
          this.addMove(`spend\t${realms_self.game.player}\tcreature\t${JSON.stringify(card.cost)}`);
          this.addMove(
            `counter_or_acknowledge\t${realms_self.returnPlayerUsername(this.game.player)} casts ${this.popup(cardname)}\tdeploy_sorcery\t${card}`
          );
          this.addMove('RESETCONFIRMSNEEDED\tall');
          this.addMove(`discard\t${realms_self.game.player}\t${cardname}`);
          this.endTurn();
  ...
  returnCardImage(cardname) {
    ...
    if (deck[cardname]) {
      ...
      if (
        card.type === 'land' &&
        this.game.state.players_info[this.game.player - 1].land_played == 1
      ) {
        can_cast = false;
      }
      if (card.type === 'creature' && !this.canPlayerCastSpell(cardname)) {
        can_cast = false;
      }
      if (card.type === 'sorcery' && !this.canPlayerCastSpell(cardname)) {
        can_cast = false;
      }
      if (card.type === 'instant' && !this.canPlayerCastSpell(cardname)) {
        can_cast = false;
      }
  ...
  returnDeck(color = '') {
    ...
    for (let card in deck) {
      if (!deck[card].canEvent) {
        deck[card].canEvent = () => {};
      }
      if (!deck[card].onEvent) {
        deck[card].onEvent = () => {};
      }
      if (color != '') {
  ...
  returnWhiteDeck() {
    return this.returnDeck('white');
  }
  returnRedDeck() {
    return this.returnDeck('red');
  }
  returnBlackDeck() {
    return this.returnDeck('black');
  }
  returnGreenDeck() {
    return this.returnDeck('green');
  }
  returnBlueDeck() {
    return this.returnDeck('blue');
  }
}

      ...
      if (this.useClock) {
        ...
        for (let i = 1; i <= this.game.players.length; i++) {
          this.playerbox.updateIcons(
            `<div class="tool-item item-detail player-icon-${this.roles[i].toLowerCase()}"></div>`,
            i
          );
...
*/
    ...
    if (!this.game.state) {
      ...
      if (this.is_testing == 1) {
        ...
        this.game.options = {};
        if (is_async) {
          this.game.options.async_dealing = 1;
        }
        ...
        if (this.game.options.deck === 'late-war') {
          ...
          if (this.game.options.deck === 'saito') {
            this.removeTwilightCardFromDeck('nato', 'Prerequisites Unmet');
            this.addTwilightCardToDeck('iranianultimatum', 'New Card');
            this.addTwilightCardToDeck('unitedfruit', 'New Card');
      ...
      // deprecated  but included for support. game content is saved...
  ...
  handleGameLoop() {
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] == 'choose_latewar_optional_cards') {
        ...
        //
        if (this.choosecard_overlay.is_visible) {
          return 0;
        }
        ...
        let cardchosen = [];
        let cardoptions = [];
        let x = this.game.state.se.latewar;
        for (let z = 0; z < this.game.state.se.dynamic.length; z++) {
          if (!x.includes(this.game.state.se.dynamic[z])) {
            x.push(this.game.state.se.dynamic[z]);
          }
        }
        if (this.game.state.events.cia == 1) {
          if (x.includes('tsarbomba')) {
        ...
        for (let z = 0; z < x.length; z++) {
          let key = x[z] + '_added';
          if (!this.game.state.events[key]) {
            cardoptions.push(x[z]);
          }
        }
      ...
      if (mv[0] == 'choose_midwar_optional_cards') {
        ...
        let cardchosen = [];
        let cardoptions = [];
        let x = this.game.state.se.midwar;
        for (let z = 0; z < this.game.state.se.dynamic.length; z++) {
          if (!x.includes(this.game.state.se.dynamic[z])) {
            x.push(this.game.state.se.dynamic[z]);
          }
      ...
      if (mv[0] === 'purge') {
        ...
        this.removeTwilightCardFromDeckNextDeal(card);
        return 1;
      }
      ...
      if (mv[0] === 'discard') {
        if (mv[2] === 'china') {
          ...
          this.removeTwilightCardFromHand(mv[2]);
      ...
      if (mv[0] === 'missileenvy') {
        ...
        if (opponent_card == 1) {
          ...
          this.game.queue.push(
            'ops\t' + receiver + '\t' + card + '\t' + this.game.deck[0].cards[card].ops + '\t1'
          ); // 1 => no back button
          this.game.queue.push(
            'NOTIFY\t' +
        ...
        if (this.game.player == sender) {
          this.removeTwilightCardFromHand(card);
        }
      ...
      if (mv[0] === 'quagmire') {
        ...
        this.updateLog(
          `${this.cardToText(sticker)}: ${mv[1].toUpperCase()} discards ${this.cardToText(discard)} and rolls a ${roll}`
        );
      ...
      if (mv[0] === 'ops') {
        ...
        if (mod_ops > orig_ops) {
          ...
          this.updateLog(
            mv[1].toUpperCase() +
              ` plays ${this.game.state.event_name} for ${mv[3]} OPS (-${orig_ops - mod_ops} penalty)`
          );
        } else {
          this.updateLog(
      ...
      if (mv[0] === 'round') {
        ...
        if (this.game.state.round > 1) {
          while (this.game.state.stats.round.length < this.game.state.round - 1) {
            ...
            this.game.state.stats.round[this.game.state.stats.round.length - 1].us_events_ops =
              this.game.state.stats.us_events_ops;
            this.game.state.stats.round[this.game.state.stats.round.length - 1].ussr_events_ops =
              this.game.state.stats.ussr_events_ops;
        ...
        if (this.game.state.round > 1) {
          ...
          this.game.queue.push('reshuffle_and_insert_discards');
  ...
  playHeadlinePostModern(stage, hash = '', xor = '', card = '') {
    ...
    if (stage == 'headline6') {
      ...
      if (
        uscard === 'defectors' ||
        (ussrcard != 'defectors' && this.game.state.defectors_pulled_in_headline == 1)
      ) {
        this.game.turn = [];
    ...
    if (stage == 'headline7') {
      ...
      if (
        ...
        this.updateLog(
          `USSR headlines ${this.cardToText(ussrcard)}, but it is cancelled by ${this.cardToText('defectors')}`
        );
  ...
  async playerTurnCardSelected(card, player) {
    ...
    if (twilight_self.game.state.events.wwby == 1 && twilight_self.game.state.headline == 0) {
      if (player == 'us' && card != 'unintervention') {
        if (twilight_self.playerHoldsCard('unintervention')) {
          let c = await sconfirm(
            `If you don't play ${twilight_self.cardToText('unintervention')}, USSR will gain 3 VP. Still play this card?`
          );
          if (c) {
          } else {
            return;
    ...
    if (ac[card]?.scoring == 1) {
      ...
      if (can_play_event == 1) {
        announcement += '<li class="option" id="event">trigger event</li>';
      }
    ...
    for (let i = twilight_self.game.queue.length - 1; i >= 0; i--) {
      ...
      if (tmpar[0] === 'discard') {
        ...
        twilight_self.updateLog(
          `${tmpar[1].toUpperCase()} second-guesses themselves and takes back their ${twilight_self.cardToText(tmpar[2])}...`
        );
      }
      if (tmpar[0] === 'play') {
  ...
  resolveWeWillBuryYouOnCommit(player, card, committing_as_event = false) {
    if (this.game.state.events.wwby != 1) {
      return;
    }
    if (this.game.state.headline == 1) {
      return;
  ...
  */
  ...
  playerCoupCountry(player, ops, card) {
    ...
    $('.country').on('click', async function () {
      ...
      //
      if (
        twilight_self.game.state.limit_region.indexOf(twilight_self.countries[countryname].region) >
        -1
      ) {
  ...
  playCoup(player, countryname, ops, mycallback = null) {
    ...
    if (winning > 0) {
      ...
      this.updateLog(
        `${player.toUpperCase()}'s coup in ${this.countries[countryname].name} fails, no change in influence`
      );
    }
  ...
  playRealign(player, country) {
    ...
    if (roll_us > roll_ussr) {
      ...
      this.updateLog(
        `Realignment roll is a draw! US [${roll_us - bonus_us} + ${bonus_us}] = USSR [${roll_ussr - bonus_ussr} + ${bonus_ussr}]`
      );
    }
  }
  ...
  displayBoard() {
    try {
      ...
      try {
        ...
        console.log('error updating defcon: ' + JSON.stringify(err));
      }
      try {
        this.updateActionRound();
      } catch (err) {
        console.log('error updating action round: ' + JSON.stringify(err));
      }
      try {
        this.updateSpaceRace();
      } catch (err) {
        console.log('error updating space race: ' + JSON.stringify(err));
      }
      try {
        this.updateVictoryPoints();
      } catch (err) {
        console.log('error updating vp: ' + JSON.stringify(err));
      }
      try {
        this.updateMilitaryOperations();
      } catch (err) {
        console.log('error updating milops: ' + JSON.stringify(err));
      }
      try {
        this.updateRound();
      } catch (err) {
        console.log('error updating round: ' + JSON.stringify(err));
      }
  ...
  removeTwilightCardFromHand(card) {
    if (this.game.player == 0) {
      return;
    }
    for (i = 0; i < this.game.deck[0].hand.length; i++) {
  ...
  returnBattlegroundCountries() {
    ...
    for (let i in this.countries) {
      ...
      if (i == 'taiwan' && this.game.state.events.formosan == 1) {
        bgs.push(i);
      }
    }
    return bgs;
  ...
  modifyOps(ops, card = '', player = '', updatelog = 1) {
    ...
    if (this.game.state.events.brezhnev == 1 && player === 'ussr') {
      if (this.game.log.length > 0) {
        if (this.game.log[0] === 'USSR gets Brezhnev bonus +1') {
          updatelog = 0;
        }
      }
      if (updatelog == 1) {
    ...
    if (this.game.state.events.containment == 1 && player === 'us') {
      if (this.game.log.length > 0) {
        if (this.game.log[0] === 'US gets Containment bonus +1') {
          updatelog = 0;
        }
      }
      if (updatelog == 1) {
    ...
    if (this.game.state.events.redscare_player1 >= 1 && player === 'ussr') {
      if (this.game.log.length > 0) {
        if (this.game.log[0] === 'USSR is affected by Red Purge') {
          updatelog = 0;
        }
        if (this.game.log[0] === 'USSR is really affected by Red Purge') {
          updatelog = 0;
        }
      }
      if (updatelog == 1) {
    ...
    if (this.game.state.events.redscare_player2 >= 1 && player === 'us') {
      if (this.game.log.length > 0) {
        if (this.game.log[0] === 'US is affected by Red Scare') {
          updatelog = 0;
        }
        if (this.game.log[0] === 'US is really affected by Red Scare') {
          updatelog = 0;
        }
      }
      if (updatelog == 1) {
  ...
  */
  ...
  cardToText(cardname, textonly = false) {
    ...
    if (this.game.deck) {
      if (this.game.deck.length < 1) {
        if (ac[cardname]) {
          return ac[cardname].name;
        }
        return '';
      }
    }
    if (!this.game.deck[0]?.cards[cardname] && card != undefined) {
      this.game.deck[0].cards[cardname] = ac[cardname];
    }
    if (card == undefined) {
      return 'Unknown';
    }
  ...
  dynamicDeckManagement() {
    ...
    if (this.game.state.events.double_quagmire && this.game.state.events.kissinger_added != 1) {
      if (!this.game.state.se.dynamic.includes('kissinger')) {
        this.game.state.se.dynamic.push('kissinger');
      }
      this.updateLog(this.cardToText('kissinger') + ' added to bonus cards...');
    ...
    if (this.game.state.events.double_beartrap && this.game.state.events.samotlor_added != 1) {
      this.updateLog(this.cardToText('samotlor') + ' added to bonus cards...');
      if (!this.game.state.se.dynamic.includes('samotlor')) {
        this.game.state.se.dynamic.push('samotlor');
      }
    ...
    if (
      ...
      this.game.state.events.summit_added = 1;
      this.addTwilightCardToDeck('summit', 'Nuclear Treaty in Play');
    }
    ...
    if (
      ...
      if (!this.game.state.se.dynamic.includes('bayofpigs')) {
        this.game.state.se.dynamic.push('bayofpigs');
      }
      this.updateLog(this.cardToText('bayofpigs') + ' added to bonus cards...');
    ...
    if (
      ...
      this.game.state.events.cubanmissile_added = 1;
      this.addTwilightCardToDeck('cubanmissile', 'USSR controls Cuba');
    }
    ...
    if (
      ...
      if (
        ...
        if (!this.game.state.se.dynamic.includes('berlinagreement')) {
          this.game.state.se.dynamic.push('berlinagreement');
        }
        this.updateLog(this.cardToText('berlinagreement') + ' added to bonus cards...');
        this.game.state.events.berlinagreement_added = 1;
      }
    }
    ...
    if (
      ...
      if (!this.game.state.se.dynamic.includes('august1968')) {
        this.game.state.se.dynamic.push('august1968');
      }
      this.updateLog(this.cardToText('august1968') + ' added to bonus cards...');
      this.game.state.events.august1968_added = 1;
    }
    if (
      ...
      if (!this.game.state.se.dynamic.includes('carterdoctrine')) {
        this.game.state.se.dynamic.push('carterdoctrine');
      }
    ...
    if (
      ...
      this.game.countries['panama'].us == 0 &&
    ...
    if (
      ...
      if (!this.game.state.se.dynamic.includes('pinochet')) {
        this.game.state.se.dynamic.push('pinochet');
      }
      this.updateLog(this.cardToText('pinochet') + ' added to bonus cards...');
      this.game.state.events.pinochet_added = 1;
    }
    ...
    if (this.game.state.round == 4) {
      ...
      // remove
      //
      this.removeTwilightCardFromDeckNextDeal('summit', 'Removed');
      if (
        this.game.state.events.iranianultimatum != 1 &&
        this.game.state.events.iranianultimatum_removed != 1
      ) {
        this.game.state.events.iranianultimatum_removed = 1;
        this.removeTwilightCardFromDeckNextDeal('iranianultimatum', 'Removed');
      }
      if (this.game.state.events.fidel != 1) {
        this.removeTwilightCardFromDeckNextDeal('cubanmissile', 'Fidel not evented');
      }
      if (this.game.state.events.cia != 1) {
        this.removeTwilightCardFromDeckNextDeal('lonegunman', 'CIA not evented');
      }
      ...
      // then add
      //
      if (this.game.state.events.cia == 1 && this.game.state.events.cia_added != 1) {
      } else {
    ...
    if (this.game.state.round == 8) {
      ...
      if (this.game.state.space_race_ussr_counter <= this.game.state.space_race_us_counter) {
        ...
        this.removeTwilightCardFromDeckNextDeal('starwars', 'US not ahead in Space Race');
      }
      //
      // Ortega
      //
      if (this.countries['cuba'].us < 1) {
        this.removeTwilightCardFromDeckNextDeal('ortega', 'US has no influence in Cuba');
      }
      //
      // Cambridge
      //
      this.removeTwilightCardFromDeckNextDeal('cambridge', 'Removed from Game');
      //
      //
      //
      ...
      if (this.game.state.events.johnpaul == 1 && this.game.state.events.solidarity_added != 1) {
        ...
        this.removeTwilightCardFromDeckNextDeal('solidarity', 'Prerequisite Unmet');
      }
      ...
      if (this.game.state.events.rustinredsquare_added != 1) {
        this.addTwilightCardToDeck('rustinredsquare', 'Replace Cambridge Five');
      }
      ...
      if (
        ...
        this.addTwilightCardToDeck('revolutionsof1989', 'Replaces KAL007');
      }
  ...
  addTwilightCardToDeck(key = '', reason = '') {
    //
    // remember we added this
    //
    let eventkey = key + '_added';
    if (this.game.state.events[eventkey] == 1) {
      return;
    }
    this.game.state.events[eventkey] = 1;
    ...
    //
    if (!this.game.saito_cards_added.includes(key)) {
      this.updateLog(this.cardToText(key) + ' added to deck...');
      this.game.saito_cards_added.push(key);
      this.game.saito_cards_added_reason.push(reason);
    }
  }

  removeTwilightCardFromDeckNextDeal(key = '', reason = '') {
    if (this.game.options.deck != 'saito') {
      return;
    }
    ...
    //
    // remove from deck if card exists
    //
    if (this.game.deck.length > 0) {
      if (this.game.deck[0].cards[key]) {
        this.removeCardFromDeck(1, key);
      }
    }
  ...
  removeTwilightCardFromDeck(key = '', reason = '') {
    if (this.game.options.deck != 'saito') {
      return;
    }
  ...
  playEvent(player, card) {
    ...
    if (card == 'awacs') {
      // SAITO COMMUNITY
      this.removeTwilightCardFromDeckNextDeal('muslimrevolution', 'Muslim Revolution cancelled');
    ...
    if (card == 'blockade') {
      ...
      if (this.game.player == 2) {
        ...
        if (cards_to_discard.length == 0) {
          ...
          this.endTurn();
          this.updateStatus(
            `<div class='status-message' id='status-message'>${twilight_self.cardToText('blockade')} played: no cards available to discard.</div>`
          );
          return 0;
        }
        ...
        this.updateStatusWithOptions(
          ...
          function (action) {
            if (action == 'discard') {
              twilight_self.updateStatusAndListCards(
                ...
                function (card) {
                  ...
                  twilight_self.addMove(
                    `NOTIFY\tUS discarded ${twilight_self.cardToText(card)} to resolve ${twilight_self.cardToText('blockade')}`
                  );
                  twilight_self.endTurn();
                  return 0;
                }
            ...
            if (action == 'remove') {
              ...
              twilight_self.endTurn();
              twilight_self.updateStatus(
                `<div class='status-message' id='status-message'>${twilight_self.cardToText('blockade')}: lose all influence in West Germany.</div>`
              );
              return 0;
            }
          }
        ...
        this.updateStatus(
          `<div class='status-message' id='status-message'>US is responding to ${this.cardToText(card)}</div>`
        );
      }
      return 0;
    }
        ...
        for (var i in twilight_self.countries) {
          if (twilight_self.countries[i].control <= 2) {
            ...
            $(divname).on('click', function () {
              ...
              document.querySelectorAll('.westerneurope').forEach((el) => {
                el.classList.remove('westerneurope');
              });
              twilight_self.displayBoard();
    ...
    if (card == 'che') {
      ...
      if (this.game.state.events.unitedfruit_removed != 1) {
        ...
        this.removeTwilightCardFromDeckNextDeal('unitedfruit', 'Che Evented');
      }
    ...
    if (card == 'cia') {
      ...
      if (this.game.player == 1) {
        ...
        this.endTurn();
        this.updateStatus(
          `<div class='status-message' id='status-message'>US saw your hand and is playing 1OP (${this.cardToText(card)})</div>`
        );
      }
      ...
      if (this.game.player == 1) {
        ...
        if (available_cards.length == 0) {
          ...
          twilight_self.rollDice(available_cards.length, function (roll) {
            ...
            twilight_self.endTurn();
            twilight_self.updateStatus(
              `<div class='status-message' id='status-message'>Sending ${twilight_self.cardToText(newcard)} to US</div>`
            );
          });
        }
      } else {
    ...
    if (card == 'indreds') {
      ...
      if (this.game.player == 2) {
        ...
        if (
          ...
          if (hungary_us < hungary_ussr) {
            total_countries++;
            only_country = 'hungary';
          }
          if (yugo_us < yugo_ussr) {
            total_countries++;
            only_country = 'yugoslavia';
          }
          if (romania_us < romania_ussr) {
            total_countries++;
            only_country = 'romania';
          }
          if (bulgaria_us < bulgaria_ussr) {
            total_countries++;
            only_country = 'bulgaria';
          }
          if (czechoslovakia_us < czechoslovakia_ussr) {
            total_countries++;
            only_country = 'czechoslovakia';
          }
          ...
          if (total_countries == 1) {
            ...
            if (only_country == 'hungary') {
              diff = hungary_diff;
            }
            if (only_country == 'yugoslavia') {
              diff = yugo_diff;
            }
            if (only_country == 'romania') {
              diff = romania_diff;
            }
            if (only_country == 'bulgaria') {
              diff = bulgaria_diff;
            }
            if (only_country == 'czechoslovakia') {
              diff = czechoslovakia_diff;
            }
    ...
    if (card == 'ironlady') {
      // SAITO COMMUNITY
      this.removeTwilightCardFromDeckNextDeal('socgov', 'The Iron Lady evented');
    ...
    if (card == 'missileenvy') {
      ...
      if (this.game.player == respondant) {
        ...
        if (multiple_cards == 0) {
          ...
          for (let i = 0; i < available_cards.length; i++) {
            if (
              this.modifyOps(ac[available_cards[i]].ops, available_cards[i], opponent) ==
                selected_ops &&
        ...
        this.updateStatus(
          `<div class='status-message' id='status-message'>${this.roles[respondant].toUpperCase()} is returning card for ${this.cardToText(card)}</div>`
        );
      }
      return 0;
    }
    ...
    if (card == 'northseaoil') {
      // SAITO COMMUNITY
      this.removeTwilightCardFromDeckNextDeal('opec', 'OPEC cancelled');
    ...
    if (card == 'olympic') {
      ...
      if (player == opponent) {
        ...
        this.updateStatus(
          `<div class='status-message' id='status-message'>${opponent.toUpperCase()} is deciding whether to boycott the ${this.cardToText(card)}</div>`
        );
      }
    ...
    if (card == 'pershing') {
      ...
      if (this.game.player == 1) {
        var twilight_self = this;
        this.updateStatus(
          `<div class='status-message' id='status-message'>${this.cardToText(card)}: Remove 3 US influence from Western Europe (max 1 per country)</div>`
        );
        ...
        $('.easterneurope').on('click', function () {
          ...
          if (twilight_self.countries[c].place != 1) {
            ...
            twilight_self.updateStatus(
              `<div class='status-message' id='status-message'>${twilight_self.cardToText(card)}: Remove ${ops_to_purge} US influence from Western Europe (max 1 per country)</div>`
            );
          }
        });
      }
    ...
    if (card == 'reformer') {
      ...
      if (this.game.player == 1) {
        ...
        this.updateStatus(
          `<div class="status-message" id="status-message">${twilight_self.cardToText(card)}: Add ${influence_to_add} influence in Europe (max 2 per country)</div>`
        );
    ...
    if (card == 'saltnegotiations') {
      ...
      if (i_played_the_card) {
        ...
        setTimeout(() => {
          document
            .querySelectorAll('.saito-overlay .transparent-card-overlay .card')
            .forEach((el) => {
              el.onclick = (e) => {
...
****/
      ...
      if (this.isControlled('us', 'uk') == 1) {
        ...
        if (this.game.player == 2) {
          ...
          if (this.game.state.events.nato == 1) {
            ...
            this.updateStatus(
              'US is playing Special Relationship. Add 2 Influence to any country in Western Europe.'
            );
          } else {
            this.updateStatus(
              'US is playing Special Relationship. Add 1 Influence to any country adjacent to the UK.'
            );
            placeable.push('canada');
            placeable.push('france');
            placeable.push('norway');
    ...
    if (card === 'teardown') {
      // SAITO COMMUNITY
      if (this.game.options.deck === 'saito') {
        this.removeTwilightCardFromDeckNextDeal('willybrandt');
      }
    ...
    if (card == 'terrorism') {
      ...
      if (target == this.roles[this.game.player]) {
        ...
        twilight_self.addMove(
          `modal\t${twilight_self.cardToText(card)}\t${target.toUpperCase()} discarded ${discard_text}`
        );
        this.endTurn();
      }
    ...
    if (card == 'ussuri') {
      ...
      if (us_cc == 1) {
        ...
        if (this.game.player == 2) {
          ...
          this.updateStatus(
            `<div class='status-message' id='status-message'>${this.cardToText(card)}: US is placing influence in Asia </div>`
          );
        }
        return 0;
      } else {
    ...
    if (card == 'wargames') {
      ...
      if (i_played_the_card) {
        ...
        this.updateStatusWithOptions(
          `${this.cardToText(card)}: Do you want to give your opponent 6 VP and End the Game? (VP ties will be won by opponent)`,
          choicehtml,
          function (action2) {
            if (action2 == 'endgame') {
    ...
    if (card == 'bayofpigs') {
      ...
      if (this.game.player == 1) {
        ...
        this.updateStatus(
          `<div class='status-message' id='status-message'>USSR responding to ${this.cardToText(card)}</div>`
        );
      }

...
class CryptoModule extends ModTemplate {
  ...
  receivePaymentTransaction(tx) {
    ...
    if (expected_payment) {
      ...
      siteMessage(
        `${txmsg.amount} ${this.ticker} inbound from ${this.app.keychain.returnUsername(tx.from[0].publicKey)}`,
        3000
      );
    }
  }
 ...
 * @return {boolean} true/false
 */
CryptoModule.prototype.returnUtxo = async function (state = 'unspent', limit = 1000, order = 'DESC') {
  return true;
};

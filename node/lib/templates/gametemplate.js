...
*/
        ...
        if (!txmsg?.step?.game) {
          ...
          if (this?.treat_all_moves_as_future || this.isFutureMove(tx.from[0].publicKey, txmsg)) {
            ...
            if (this.game.player === 0 && this.game.future.length > 3) {
              console.info('GT [onConfirmation] Observer receives future move', this.gaming_active, this.halted);
      ...
      if (message?.data != undefined) {
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (!this.doesGameExistLocally(gametxmsg.game_id)) {
            console.info(`GT HPT Game does not exist locally. waiting for on-chain to process (${message.request}): `);
            return;
          }
          ...
          if (message.request === 'game relay gamemove') {
            ...
            if (this?.treat_all_moves_as_future || this.isFutureMove(gametx.from[0].publicKey, gametxmsg)) {
  ...
  startClock(player = this.game.target) {
    ...
    this.clock_timers[player] = setInterval(() => {
      ...
      let time_on_clock =
        this.game.clock[player - 1].limit - (t - this.game.time.last_received) - this.game.clock[player - 1].spent;
      if (time_on_clock <= 0) {
    ...
    if (txmsg.request == 'STAKE') {
      ...
      try {
        ...
        for (let i = 0; i < this.game.players.length; i++) {
          ...
          if (sigs[i] !== '') {
            if (this.app.crypto.verifyMessage(msg_to_verify, sigs[i], this.game.players[i])) {
              auths++;
            }
          }
        }

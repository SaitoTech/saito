...
class GameMoves {
          ...
          if (txmsg?.step?.game) {
            if (txmsg.step.game > this.game.step.players[tx.from[0].publicKey]) {
              ...
              for (let zz = 0; zz < arrived.length; zz++) {
                if (JSON.stringify(txmsg) === JSON.stringify(arrived[zz].returnMessage())) {
                  matched = true;
                }
              }
  ...
  isUnprocessedMoveQuickCheck(publickey, step, gameid) {
    ...
    if (!gameobj.step.players) {
      return 0;
    }
    if (gameobj.step.players[publickey] < step) {
      return 1;
    }
    if (gameobj.step.players[publickey] >= step) {
      return -1;
  ...
  */
  async processFutureMoves() {
    ...
    for (let i = 0; i < this.game.future.length; i++) {
      ...
      if (this.isUnprocessedMove(ftx.from[0].publicKey, ftxmsg)) {
        console.info(
          `GT [processFutureMoves] Found FTMSG (${ftxmsg.step.game}): `,
          JSON.parse(JSON.stringify(ftxmsg.turn))
        );
  ...
  async sendGameMoveTransaction(type = 'game', extra = {}, mycallback = null) {
    ...
    if (this.game.sroll == 1) {
      ...
      turn.push('SECUREROLL\t' + this.game.player + '\t' + hash2 + '\t' + hash2_sig);
    }
    ...
    if (game_self.game?.crypto && game_self.game.crypto !== 'SAITO' && game_self.game.crypto !== 'CHIPS') {
      ...
      if (crypto_key !== this.game.keys[this.game.player - 1]) {
        turn.push(
          `CRYPTOKEY\t${this.publicKey}\t${crypto_key}\t${await this.app.crypto.signMessage(crypto_key, privateKey)}`
        );
      }
    }
      ...
      async (txs) => {
        ...
        for (let tx of txs) {
          ...
          if (game_move?.step && game_move.request == 'game') {
            ...
            if (loaded_step > this.game.step.game || loaded_step > this.game.step.players[tx.from[0].publicKey]) {

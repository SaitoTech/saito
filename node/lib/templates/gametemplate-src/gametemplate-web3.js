...
class GameWeb3 {
  ...
  addPaymentToQueue(sender, receiver, amount_to_send) {
    ...
    this.game.queue.push(`RECEIVE\t${sender}\t${receiver}\t${amount_to_send}\t${ts}\t${uh}\t${this.game.crypto}`);
    this.game.queue.push(`SEND\t${sender}\t${receiver}\t${amount_to_send}\t${ts}\t${uh}\t${this.game.crypto}`);
  }

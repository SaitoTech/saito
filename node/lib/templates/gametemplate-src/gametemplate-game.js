...
class GameGame {
  ...
  exportGame() {
    ...
    pom.setAttribute('href', 'data:application/json;utf-8,' + encodeURIComponent(JSON.stringify(g)));
    ...
    if (game_id == null) {
      ...
      if (this.app.options?.games?.length > 0) {
        for (let i = 0; i < this.app.options.games.length; i++) {
          ...
          if (this.name == this.app.options.games[i].module && this.app.options.games[i].timestamp > timeStamp) {
            game_to_open = i;
  ...
  async initializeGameFromAcceptTransaction(tx) {
    ...
    if (!txmsg.players.includes(this.publicKey)) {
      console.error('GT [initializeGameFromAcceptTransaction] -- not our game!');
      return false;
    }
      ...
      if (txmsg.players.length == txmsg.players_sigs.length) {
        for (let i = 0; i < txmsg.players.length; i++) {
          if (!this.app.crypto.verifyMessage(msg_to_verify, txmsg.players_sigs[i], txmsg.players[i])) {
            console.error('GT [initializeGameFromAcceptTransaction] -- PLAYER SIGS do not verify for all players');
            this.halted = 0;
            return false;
          }
        }
  ...
  async receiveStopGameTransaction(resigning_player, txmsg) {
    ...
    if (txmsg.reason == 'cancellation') {
      if (resigning_player == this.publicKey) {
        ...
        this.updateLog(`${this.app.keychain.returnUsername(resigning_player)} cancels the game, no one wins`);
        siteMessage(`${this.app.keychain.returnUsername(resigning_player)} cancels the game, no one wins`, 8000);
      }
    } else {
  ...
  */
  async receiveGameoverTransaction(blk, tx, conf, app) {
    ...
    console.info(
      'GT: Received Gameover Request from ' + tx.from[0].publicKey,

...
class GameGame {
  ...
  exportGame() {
    ...
    pom.setAttribute(
      'href',
      'data:application/json;utf-8,' + encodeURIComponent(JSON.stringify(g))
    );
  ...
  saveGame(game_id) {
    ...
    if (!game_id || game_id !== this.game.id) {
      ...
      console.warn(
        'GT ERR? Save game with wrong id',
        'Parameter: ' + game_id,
  ...
  async initializeGameFromAcceptTransaction(tx) {
    ...
    if (!txmsg.options?.async_dealing) {
      ...
      if (txmsg.players.length == txmsg.players_sigs.length) {
        ...
        console.error(
          'GT [initializeGameFromAcceptTransaction] -- Players and player_sigs different lengths!'
        );
        return false;
      }
    }
    ...
    if (this.game.over == 1) {
      this.halted = 0;
      console.warn(
        'GT [initializeGameFromAcceptTransaction] -- This game is over, cannot initialize from accept tx'
      );
      return false;
    }

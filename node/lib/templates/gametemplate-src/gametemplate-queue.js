...
class GameQueue {
  ...
  async startQueue() {
    ...
    console.debug(
      'GT [startQueue] QUEUE: (' + this.game.step.game + ') ' + JSON.parse(JSON.stringify(this.game.queue))
    );
  ...
  initializeQueueCommands() {
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === 'RESOLVE') {
        ...
        for (let i = 0; i < game_self.game.confirms_needed.length; i++) {
          if (game_self.game.confirms_needed[i] == 1) {
            notice += '<li class="option resolve">' + game_self.returnUsername(game_self.game.players[i]) + '</li>';
      ...
      if (gmv[0] === 'SECUREROLL') {
        ...
        if (game_self.game.sroll_done == 0) {
          if (game_self.game.player != initiating_player) {
            ...
            let hash1_sig = game_self.app.crypto.signMessage(hash1, await game_self.app.wallet.getPrivateKey());
        ...
        if (their_second_hash === '') {
          ...
          if (game_self.game.player == originating_player) {
            ...
            game_self.addMove(
              'SECUREROLL\t' + game_self.game.player + '\t' + game_self.game.sroll_hash + '\t' + hash_sig
            );
            game_self.game.sroll_done = 1;
            game_self.endTurn();
            return 0;
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === 'SIMULTANEOUS_PICK') {
        ...
        if (game_self.game.spick_done == 0) {
          ...
          let hash2_sig = game_self.app.crypto.signMessage(hash2, await game_self.app.wallet.getPrivateKey());
        ...
        if (game_self.game.pool[poolidx - 1].decrypted == decryption_keys_needed) {
          for (let i = 0; i < cardnum; i++) {
            ...
            if (!game_self.game.pool[poolidx - 1].cards[newcard]) {
              console.warn(
                'GT [RESOLVEFLIP] Card decryption error!',
                'Card: ' + newcard,
                'pool:',

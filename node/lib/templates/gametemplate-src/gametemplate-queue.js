  ...
  initializeQueueCommands() {
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === 'RESOLVE') {
        ...
        //
        console.log(
          'confs_needed pre_RESOLVE handling: ' + JSON.stringify(game_self.game.confirms_needed)
        );
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === 'AVAILABLE_CRYPTOS') {
        ...
        if (this.game.player !== playerkey) {
          this.app.wallet.saveAvailableCryptosAssociativeArray(
            this.game.players[playerkey - 1],
            cryptos
          );
        }

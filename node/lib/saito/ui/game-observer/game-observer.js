  ...
  attachEvents() {
    ...
    if (document.getElementById('game-observer-first-btn')) {
      document.getElementById('game-observer-first-btn').onclick = async (e) => {
        ...
        await this.game_mod.initializeObserverMode(this.arcade_mod.returnGame(this.game_mod.game.id));
      ...
      joinBtn.onclick = async () => {
        ...
        if (c) {
          ...
          if (
            ...
            (parseFloat(this.game_mod.game.options.stake) > 0 || parseFloat(this.game_mod.game.options.stake?.min) >= 0)
          ) {

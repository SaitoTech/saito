...
const AuthTemplate = require('./saito-auth.template');
...
/**
 * Saito identity-acquisition overlay.
 *
 ...
 * callers (Faucet, Store, etc.) own those decisions after receiving the result.
 *
 * Callback result:
 *   {
...
class Auth {
	constructor(app, mod) {
		...
		if (app?.browser?.addStylesheet) {
			app.browser.addStylesheet('/saito/css-imports/ui/saito-auth.css');
		}

module.exports = (input_self, activeWindow = null, options = {}) => {
  // null → leave inactive (composer chrome). Explicit window → highlight that mode.
  const active =
    activeWindow === null ? input_self.open_tab || null : activeWindow || 'emoji-window';
  const showClose = !!options.showClose;

  return `
		<div class="selection-box-tabs">
			<div class="saito-box-tab${active === 'emoji-window' ? ' active' : ''}" id="emoji-tab" title="Emoji"><i class="fa-regular fa-face-smile"></i></div>
			<div class="saito-box-tab${active === 'photo-window' ? ' active' : ''}" id="photo-tab" title="Image"><i class="fa-regular fa-image"></i></div>
			<div class="saito-box-tab gif-icon${active === 'gif-window' ? ' active' : ''}" id="gif-tab" title="GIF">
			...
			${
        showClose
          ? `<div class="saito-box-tab selection-box-close" id="selection-box-close" title="Close" role="button" aria-label="Close"><i class="fa-solid fa-xmark"></i></div>`
          : ''
      }
		</div>`;
};

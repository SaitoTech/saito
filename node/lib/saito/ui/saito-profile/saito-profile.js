...
class SaitoProfile {
      ...
      if (Object.keys(this.menu).length > 0) {
        for (let i in this.menu) {
          ...
          this.app.browser.addElementToSelector(
            html,
            `#saito-profile${this.ordinal} .saito-profile-menu`
          );
        }
      }
    }
  ...
  attachEvents() {
    ...
    if (
      document.querySelector(`#saito-profile${this.ordinal} .saito-profile-description.can-edit`)
    ) {
      document.querySelector(
        `#saito-profile${this.ordinal} .saito-profile-description.can-edit`
      ).onclick = async (e) => {

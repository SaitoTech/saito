...
class SaitoContacts {
  ...
  async render(keys = null) {
    if (!document.getElementById('saito-contacts-modal')) {
      ...
      this.app.browser.replaceElementById(SaitoContactsTemplate(this.app, this, keys), 'saito-contacts-modal');
    }
  ...
  attachEvents() {
    if (this.multi_select) {
      Array.from(document.querySelectorAll('#saito-contacts-modal .saito-contact')).forEach((contact) => {
        contact.onclick = (e) => {
          e.stopPropagation();

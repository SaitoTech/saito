  ...
  constructor(app, mod, logo) {
    ...
    app.connection.on('start-stun-call', (event) => {
      ...
      if (controlList) {
        ...
        const audioControl = controlList.querySelector(
    ...
    app.connection.on('stun-disconnect', () => {
      const controlList = document.querySelector('#dream-controls .control-panel .control-list');
      if (controlList) {
        const videoControl = controlList.querySelector(
          '.dream-controls-menu-item i.fas.fa-video, .dream-controls-menu-item i.fas.fa-video-slash'
        );
        const audioControl = controlList.querySelector(
          '.dream-controls-menu-item i.fas.fa-microphone, .dream-controls-menu-item i.fas.fa-microphone-slash'
        );
                ...
                mutation.removedNodes.forEach((node) => {
                  if (
                    ...
                    videos.forEach((video) => {
                      ...
                      if (streamData) {
                        this.streamData = this.streamData.filter(
                          (data) => data.video.id !== video.id
                        );
                      }
                    });
  ...
  async captureGameStream(includeCamera = false) {
            ...
            if (
              ...
              videos.forEach((video) => {
                ...
                const saitoAddressElement = video
                  .closest('div[id^="stream_"]')
                  .querySelector('.saito-address');
                const saitoAddress = saitoAddressElement
                  ? shortenAddress(saitoAddressElement.textContent)
                  : null;
                const fontStyle = saitoAddressElement
                  ? getComputedFontStyle(saitoAddressElement)
                  : null;
                let existingVideoIndex = this.streamData.findIndex(
                  (data) => data.video.id === video.id
                );
                if (existingVideoIndex !== -1) {
                  this.streamData[existingVideoIndex] = {
                    stream,
                    rect,
                    parentID,
                    video,
    ...
    CanvasRenderingContext2D.prototype.createPattern = function (image, repetition) {
      if (image instanceof HTMLCanvasElement && (image.width === 0 || image.height <= 1)) {
        console.warn(
          'Attempted to create pattern with problematic canvas:',
          image.width,
          image.height
        );

...
class GameBoardSizer {
    ...
    try {
      if (!document.getElementById('game_board_sizer')) {
        if (selector) {
          ...
          this.app.browser.prependElementToSelector(GameBoardSizerTemplate(this.maxZoom), this.container);
        }
      }
    } catch (err) {
      console.error(err);
    ...
    if (boardScaler) {
      try {
        if (sizer_self.mod.loadGamePreference(sizer_self.mod.returnSlug() + '-board-scale')) {
          ...
          boardScaler.value = sizer_self.mod.loadGamePreference(sizer_self.mod.returnSlug() + '-board-scale');
  ...
  scaleBoard(targetObject) {
    targetObject.style.transform = `scale(${document.querySelector('#game_board_sizer input').value / 100})`;

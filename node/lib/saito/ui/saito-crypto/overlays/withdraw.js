...
class Withdraw {
  ...
  async loadCryptos() {
    ...
    document.querySelector('.withdraw-info-value.balance').innerHTML = `${this.pc.returnBalance()} ${this.ticker}`;
      ...
      if (document.querySelector('#withdraw-max-btn') != null) {
        document.querySelector('#withdraw-max-btn').onclick = async (e) => {
          if (!document.querySelector('#withdraw-input-amount').disabled) {
            ...
            document.querySelector('#withdraw-input-amount').value = balance_as_float - this_withdraw.fee;
  ...
  validateAddressInput() {
    ...
    if (!valid) {
      document.querySelector('#withdraw-address-error').innerHTML = 'Error: Invalid ' + this.ticker + ' address';

...
class Nft {
  ...
  async render() {
    ...
    if (document.querySelector('.slip-info .metric.balance h3 .metric-amount') != null) {
      document.querySelector('.slip-info .metric.balance h3 .metric-amount').innerHTML =
        balance_str;
    }
  ...
  attachEvents() {
    ...
    document.querySelector('#create_nft').onclick = async (e) => {
      ...
      let depositAmt = this.app.wallet.convertSaitoToNolan(
        document.querySelector('#nfts-deposit').value
      );
      let fee = this.app.wallet.convertSaitoToNolan(document.querySelector('#nfts-fee').value);
      let change = this.app.wallet.convertSaitoToNolan(
        document.querySelector('#nfts-change').value
      );
    ...
    if (document.querySelector('.utxo-selection-button')) {
      document.querySelectorAll('.utxo-selection-button').forEach(function (btn) {
        btn.onclick = async (e) => {
          ...
          document.querySelectorAll('.nft-creator').forEach((el) => {
            el.classList.remove('nft-inactive');
          });
          document.querySelectorAll('.create-button').forEach((el) => {
            el.classList.remove('nft-inactive');
          });
        };
      });

...
class VideoBox {
  ...
  renderPeerList() {
    ...
    const callList = document.querySelector(`.video-call-info[data-id="${this.stream_id}"] .peer-call-list`);
  ...
  updateConnectionMessage(message = 'negotiating peer connection', with_icon = true) {
    ...
    if (video_box.querySelector('#connection-message')) {
      video_box.querySelector('#connection-message').innerHTML = `<p>${message}</p>${icon}`;
    } else {
      video_box.insertAdjacentHTML('beforeend', `<div id="connection-message"> <p>${message}</p>${icon}</div> `);
    }
  }

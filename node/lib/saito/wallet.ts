...
export default class Wallet extends SaitoWallet {
   ...
   * 
   * @param {Buffer} data - The binary data to encrypt
   ...
   */
  public async encryptWithPublicKey(data: Buffer, recipientPublicKey: string): Promise<Buffer> {
    try {
      ...
      const publicKeyHex = this.app.crypto.fromBase58(recipientPublicKey);
      ...
      const sharedPoint = Buffer.from(secp256k1.publicKeyTweakMul(publicKeyBuffer, ephemeralPrivateKey));
      ...
      const encryptedData = this.app.crypto.aesEncrypt(data.toString('base64'), sharedSecret);
   ...
   * @returns {Promise<Buffer>} The decrypted data as a buffer
   * @throws {Error} If decryption fails or invalid data format
   */
  public async decryptWithPrivateKey(encryptedData: Buffer): Promise<Buffer> {
    try {
      ...
      const ephemeralPublicKeyUncompressed = Buffer.from(secp256k1.publicKeyConvert(ephemeralPublicKey, false));

      // Compute shared secret using ECDH
      const sharedPoint = Buffer.from(secp256k1.publicKeyTweakMul(ephemeralPublicKeyUncompressed, privateKeyBuffer));
      ...
      const decryptedBase64 = this.app.crypto.aesDecrypt(encryptedString, sharedSecret);
  ...
  /**
   * Converts a JavaScript string to a Buffer for encryption
   * 
   * @param {string} str - The string to convert
   * @returns {Buffer} The string as a UTF-8 buffer
   */
  ...
  /**
   * Converts a Buffer back to a JavaScript string
   * 
   * @param {Buffer} buffer - The buffer to convert
   * @returns {string} The buffer as a UTF-8 string
   */
  ...
  /**
   * Converts a JavaScript object to a Buffer for encryption
   * 
   * @param {any} obj - The object to convert
   * @returns {Buffer} The object as a JSON UTF-8 buffer
   */
  public objectToBuffer(obj: any): Buffer {
    return Buffer.from(JSON.stringify(obj), 'utf8');
  }

  /**
   * Converts a Buffer back to a JavaScript object
   * 
   * @param {Buffer} buffer - The buffer to convert
   * @returns {any} The parsed JavaScript object
   * @throws {Error} If the buffer contains invalid JSON
   */
  public bufferToObject(buffer: Buffer): any {
    try {
      return JSON.parse(buffer.toString('utf8'));
    } catch (error) {
      throw new Error(`Failed to parse buffer as JSON: ${error.message}`);
    }
  }

  /**
   * Encrypts a JavaScript string with a public key
   * 
   * @param {string} str - The string to encrypt
   * @param {string} recipientPublicKey - The recipient's public key
   * @returns {Promise<Buffer>} The encrypted data
   */
  public async encryptString(str: string, recipientPublicKey: string): Promise<Buffer> {
    const buffer = this.stringToBuffer(str);
    return this.encryptWithPublicKey(buffer, recipientPublicKey);
  }

  /**
   * Decrypts an encrypted buffer back to a JavaScript string
   * 
   * @param {Buffer} encryptedData - The encrypted data
   * @returns {Promise<string>} The decrypted string
   */
  public async decryptToString(encryptedData: Buffer): Promise<string> {
    const decryptedBuffer = await this.decryptWithPrivateKey(encryptedData);
    return this.bufferToString(decryptedBuffer);
  }

  /**
   * Encrypts a JavaScript object with a public key
   * 
   * @param {any} obj - The object to encrypt
   * @param {string} recipientPublicKey - The recipient's public key
   * @returns {Promise<Buffer>} The encrypted data
   */
  public async encryptObject(obj: any, recipientPublicKey: string): Promise<Buffer> {
    const buffer = this.objectToBuffer(obj);
    return this.encryptWithPublicKey(buffer, recipientPublicKey);
  }

  /**
   * Decrypts an encrypted buffer back to a JavaScript object
   * 
   * @param {Buffer} encryptedData - The encrypted data
   * @returns {Promise<any>} The decrypted object
   */
  public async decryptToObject(encryptedData: Buffer): Promise<any> {
    const decryptedBuffer = await this.decryptWithPrivateKey(encryptedData);
    return this.bufferToObject(decryptedBuffer);
  }
}

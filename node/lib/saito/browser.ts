...
*****/
  ...
  saneDateTimeFromTimestamp(timestamp, detailed = true) {
    return this.saneDateFromTimestamp(timestamp, detailed) + '/' + this.saneTimeFromTimestamp(timestamp, detailed);
  }
  ...
  makeDraggable(id_to_move, id_to_drag = '', dockable = false, mycallback = null) {
    ...
    try {
      ...
      element_to_drag.onmousedown = function (e) {
        ...
        if (!e.currentTarget.id || (e.currentTarget.id != id_to_move && e.currentTarget.id != id_to_drag)) {
  ...
  updateAddressHTML(key, id) {
    ...
    try {
      Array.from(document.querySelectorAll(`.saito-address[data-id='${key}']`)).forEach((add) => (add.innerHTML = id));
    } catch (err) {
   ...
   */
  createReactRoot(Component: React.ComponentType<any>, props: Record<string, any> = {}, containerId?: string) {

    ...
    hasSeenTransaction(tx, blk_id = 0) {
        ...
        if (this.processedTxs[hashed_data] !== undefined) {
            if (this.processedTxs[hashed_data]) {
                console.log('prevent processing duplicated on chain transaction : ', tx.from[0]?.publicKey, tx.returnMessage(), this.processedTxs[hashed_data], blk_id);
            }

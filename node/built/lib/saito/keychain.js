...
const modtemplate_1 = __importDefault(require("./../templates/modtemplate"));
...
class Keychain {
    ...
    returnIdenticon(publicKey, img_format = 'svg') {
        ...
        const data = new identicon_js_1.default(this.app.crypto.hash(publicKey), options).toString();
    ...
    returnKeyArchiveNodes(publicKey) {
        const keylist = this.app.options.keys;
    ...
    updateEncryptionByPublicKey(publicKey, aes_publicKey = '', aes_privatekey = '', shared_secret = '') {
        if (!publicKey) {
            return;
        }
        this.addKey(publicKey, {

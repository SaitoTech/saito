...
class Browser {
    ...
    async initialize(app) {
        ...
        try {
            if (screenfull_1.default.isEnabled) {
                screenfull_1.default.on('change', () => {
                    this.app.connection.emit('browser-fullscreen-toggle', screenfull_1.default.isFullscreen);
                });
            }
        ...
        *****/
                document.addEventListener(this.tab_event_name, () => {
                    if (document[this.hidden_tab_property]) {
                        this.setActiveTab(0);
            ...
            if (c_param) {
                if (!(await this.app.wallet.setPreferredCrypto(c_param))) {
                    salert(`Your compile does not contain a ${c_param.toUpperCase()} module. Visit the AppStore or contact us about getting one built!`);
                }
            }
            //
        ...
        this.app.connection.on('peer_connect', function (peerIndex) {
        ...
        this.app.connection.on('peer_disconnect', function (peerIndex) {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
    ...
    async returnInviteLink(email = '') {
        ...
        let url_payload = encodeURIComponent(this.app.crypto.stringToBase64(JSON.stringify(await this.returnInviteObject(email))));
    ...
    isMobileBrowser(user_agent = navigator.userAgent) {
        let check = false;
        (function (user_agent) {
            if (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(user_agent) ||
    ...
    checkForMultipleWindows() {
        ...
        const onLocalStorageEvent = async (e) => {
            ...
            if (e.key == 'page_available' && !this.isMobileBrowser(navigator.userAgent)) {
                ...
                let c = await sconfirm('Your wallet appears to be connected in another Saito tab.\n\nWould you like to connect it here and close the other tab?');
                if (c) {
                    this.multiple_windows_active = 0;
    ...
    async requestFullscreen() {
        if (screenfull_1.default.isEnabled) {
            await screenfull_1.default.toggle();
        }
    }
    ...
    addElementAfterElement(html, elem = document.body) {
        try {
            ...
            console.error('ERROR 582346: error in addElementToElement. Does ' + elem + ' exist? : ' + err);
            console.debug(elem, html);
        }
    }
    ...
    saneDateTimeFromTimestamp(timestamp, detailed = true) {
        return (this.saneDateFromTimestamp(timestamp, detailed) +
            '/' +
    ...
    addDragAndDropFileUploadToElement(id, handleFileDrop = null, click_to_upload = true, read_as_array_buffer = false, read_as_text = false) {
    ...
    makeDraggable(id_to_move, id_to_drag = '', dockable = false, mycallback = null) {
        ...
        try {
            ...
            element_to_drag.onmousedown = function (e) {
                ...
                if (resizeable.indexOf(getComputedStyle(e.target).resize) > -1 ||
                ...
                document.onmousemove = function (e) {
                    ...
                    if (dockable) {
                        ...
                        if (Math.abs(element_to_move.getBoundingClientRect().x +
                        ...
                        if (Math.abs(element_to_move.getBoundingClientRect().y +
    ...
    makeResizeable(target_div, icon_div = null, unique_id = null, direction = 'diagonal', callback = null) {
        ...
        let pullTab = null;
        this.addElementToSelector(`<div class="resize-icon ${direction}" id="resize-icon-${unique_id}"></div>`, icon_div);
    ...
    updateAddressHTML(key, id) {
        ...
        try {
            Array.from(document.querySelectorAll(`.saito-address.treated[data-id='${key}']`)).forEach((add) => (add.innerText = id));
        }
        catch (err) {
            ...
            if (createLinks) {
                text = text.replace(this.urlRegexp(), (url) => {
                    ...
                    return `<a ${url.includes(window.location.host)
                        ...
                        : "target='_blank' rel='noopener noreferrer' "} class="saito-link" href="${!url.includes('http') ? `http://${url1}` : url1}">${url2}</a>`;
                });
                //
    ...
    updateSoftwareVersion(receivedBuildNumber) {
        console.info(`Received build number: ${Number(receivedBuildNumber)}, Current build number: ${this.app.build_number}`);
    ...
    getDecimalSeparator() {
        let locale = this.app.BROWSER && window?.navigator?.language ? window.navigator.language : 'en-US';
     ...
     */
    createReactRoot(Component, props = {}, containerId) {
        const id = containerId || `saito-react-root-${Date.now()}`;
        const container = document.createElement('div');
        container.id = id;
        document.body.appendChild(container);
        const root = (0, react_dom_1.createRoot)(container);

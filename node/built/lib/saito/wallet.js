...
const balance_snapshot_1 = __importDefault(require("saito-js/lib/balance_snapshot"));
const wallet_1 = __importStar(require("saito-js/lib/wallet"));
...
class Wallet extends wallet_1.default {
    async createUnsignedTransactionWithDefaultFee(publicKey = '', amount = BigInt(0), default_fee = this.default_fee) {
        if (publicKey == '') {
            publicKey = await this.getPublicKey();
        }
        return this.createUnsignedTransaction(publicKey, amount, default_fee);
    }
    async createUnsignedTransaction(publicKey = '', amount = BigInt(0), fee = BigInt(0), force_merge = false) {
        if (publicKey == '') {
            publicKey = await this.getPublicKey();
        }
        return saito_1.default.getInstance().createTransaction(publicKey, amount, fee, force_merge);
    }
    async createUnsignedTransactionWithMultiplePayments(keys, amounts, fee = this.default_fee) {
        return saito_1.default.getInstance().createTransactionWithMultiplePayments(keys, amounts, fee);
    }
    async getNFTList() {
        return saito_1.default.getInstance().getNftList();
    }
    async getBalance(ticker = 'SAITO') {
        if (ticker === 'SAITO') {
    ...
    async initialize() {
        ...
        class SaitoCrypto extends CryptoModule {
            ...
            shouldAffixCallbackToModule(modname, tx = null) {
                if (this.app.BROWSER) {
                    if (tx.isTo(this.address) || tx.isFrom(this.address)) {
                        console.log(tx.type, transaction_2.TransactionType.Normal, transaction_2.TransactionType.Bound);
                        if (tx.type == transaction_2.TransactionType.Normal || tx.type == transaction_2.TransactionType.Bound) {
                            let to_amount = 0;
                            let from_amount = 0;
            ...
            async checkHistory(callback) {
                ...
                console.log(`Checking for missed SAITO transactions since ${new Date(this.history_update_ts)}`);
                ...
                this.app.network.sendRequestAsTransaction('memento', {
                    publicKey: this.publicKey,
                    offset: this.history_update_ts
                }, mycallback);
            }
            async sendPayment(amount, to_address, unique_hash = '', memo = '') {
                ...
                let newtx = await this.app.wallet.createUnsignedTransactionWithDefaultFee(to_address, nolan_amount);
                newtx.msg = {
                    module: this.name,
                ...
                console.log('Current balance: ', await this.checkBalance(), '\nExpecting new balance of: ', this.pending_balance);
                return newtx.signature;
            }
            async sendPayments(amounts, to_addresses) {
                ...
                for (let i = 0; i < amounts.length; i += CHUNK_SIZE) {
                    ...
                    let newTx = await this.app.wallet.createUnsignedTransactionWithMultiplePayments(addressesChunk, amountsChunk);
                    await this.app.wallet.signAndEncryptTransaction(newTx);
        ...
        if (this.app.options.wallet != null) {
            ...
            if (this.app.options.wallet.version < this.version) {
                ...
                if (this.app.options.wallet.slips) {
                    let slips = this.app.options.wallet.slips.map((json) => {
                        let slip = new wallet_1.WalletSlip();
                        slip.copyFrom(json);
                        return slip;
                    });
            ...
            for (let i = pending_txs.length - 1, k = 0; i >= 0; i--, k++) {
                try {
                    ...
                    if (!pending_txs[i].from) {
                        ...
                        newtx.deserialize_from_web(this.app, json_bigint_1.default.stringify(pending_txs[i]));
                    ...
                    console.log('caught error: ' + json_bigint_1.default.stringify(err));
                }
            }
    ...
    constructor(wallet) {
        ...
        this.preferred_txs = [];
        this.default_fee = BigInt(0); // in nolan
        this.version = 5.677; //saito-js 0.2.137
        this.nolan_per_saito = 100000000;
     ...
     */
    async sendPayment(ticker, senders = [], receivers = [], amounts = [], unique_hash = '', mycallback = null, saito_public_key = null, memo = '') {
                        ...
                        try {
                            ...
                            if (saito_public_key) {
                                if (ticker !== 'SAITO') {
                                    ...
                                    //
                                    await cryptomod.sendPaymentTransaction(saito_public_key, senders[i], receivers[i], amounts[i], unique_hash, memo);
                                }
                            }
                            if (mycallback) {
     ...
     * @param {String} ticker - Ticker of install crypto module
     */
    async sendPayments(senders = [], receivers = [], amounts = [], timestamp, unique_hash = '', mycallback = null, ticker) {
     ...
     */
    async receivePayment(ticker, senders = [], receivers = [], amounts = [], unique_hash = '', mycallback = null, saito_public_key = null) {
        if (senders.length !== 1 || receivers.length !== 1 || amounts.length !== 1) {
            // We have no code which exercises multiple senders/receivers so can't implement it yet.
    ...
    async isSlipInPendingTransactions(input) {
    ...
    exportWallet() {
        ...
        let newObj = json_bigint_1.default.parse(json_bigint_1.default.stringify(this.app.options));
        delete newObj.games;
        return json_bigint_1.default.stringify(newObj, null, 2);
    }
    /**
     ...
     * @return {Transaction}
     */
    async signAndEncryptTransaction(tx, recipient = '') {
        if (tx == null) {
            return null;
        }
        //
        ...
        try {
            ...
            //  "base64"
            //);
            tx.data = Buffer.from(json_bigint_1.default.stringify(tx.msg), 'utf-8');
        }
        catch (err) {
            console.log('####################');
    ...
    async fetchBalanceSnapshot(key) {
        try {
            ...
            let snapshot = balance_snapshot_1.default.fromString(data);
            if (snapshot) {
                await saito_1.default.getInstance().updateBalanceFrom(snapshot);
            }
        }
        catch (error) {
            console.error(error);
    ...
    isValidPublicKey(key) {
        if (this.app.crypto.isBase58(key)) {
            return saito_1.default.getInstance().isValidPublicKey(key);
        }
        else {
            return false;
        }
    ...
    //
    async addTransactionToPending(tx, save = true) {
        await saito_1.default.getInstance().addPendingTx(tx);
        if (save) {
            await this.saveWallet();
        }
    }
    async onUpgrade(type = '', privatekey = '', decrypted_wallet = null) {
        let publicKey = await this.getPublicKey();
        if (type == 'nuke') {
            await this.resetWallet();
            publicKey = await this.getPublicKey();
            ...
            if (decrypted_wallet != null) {
                try {
                    let wobj = json_bigint_1.default.parse(decrypted_wallet);
                    await this.reset(false);
        ...
        console.log(json_bigint_1.default.parse(json_bigint_1.default.stringify(this.app.options.wallet)));
        await this.saveWallet();
        return true;
    }
    convertSaitoToNolan(amount = '0.0') {
        let nolan = 0;
        let num = (0, decimal_js_1.default)(amount);
        if (Number(amount) > 0) {
            nolan = Number(num.times(this.nolan_per_saito).toFixed(0)); // 100,000,000
        }
        return BigInt(nolan);
    }
    convertNolanToSaito(amount = BigInt(0)) {
        let string = '0.00';
        let num = 0;
    ...
    async disableProducingBlocksByTimer() {
        return saito_1.default.getInstance().disableProducingBlocksByTimer();
    }
    async produceBlockWithGt() {
        return saito_1.default.getInstance().produceBlockWithGt();
    }
    async produceBlockWithoutGt() {
        return saito_1.default.getInstance().produceBlockWithoutGt();
    }
    /**
    ...
    async updateNFTList() {
        ...
        const nfts = typeof raw === 'string' ? json_bigint_1.default.parse(raw) : raw;
        console.log('UPDATE NFT LIST from Rust: ', nfts);
        //
        // snapshot local
        //
        const local = this.app.options.wallet.nfts ?? [];
        //
        // ensure intents bag exists and keep a stable ref
        //
        const intents = ((_a = this.app.options.wallet).nftMergeIntents || (_a.nftMergeIntents = {}));
        ...
        const groupByKey = (arr) => {
            ...
            for (const it of arr) {
                if (!it || typeof it.id !== 'string')
                    continue;
                (g[_a = it.id] ?? (g[_a] = [])).push(it);
            }
            return g;
        };
        const stripSlipLike = (it) => {
            const { slip1, slip2, slip3, tx_sig, ...rest } = it ?? {};
            return rest;
        };
        const signature = (it) => json_bigint_1.default.stringify(stripSlipLike(it));
        const isSlipOnlyChange = (A, B) => {
            const countMap = (arr) => {
                const m = new Map();
                for (const it of arr) {
        ...
        const hasUserMergeIntent = (id) => {
            const ts = intents[id];
            const TTL = 2 * 60000; // 2 minutes
            return !!ts && Date.now() - ts <= TTL;
        };
        const clearMergeIntent = (id) => {
            if (id in intents) {
                delete intents[id];
                intentsMutated = true;
            }
     ...
     *
     */
    async createMintNFTTransaction(num, deposit, tx_msg, fee, receipient_publicKey, nft_type) {
        return saito_1.default.getInstance().createBoundTransaction(num, deposit, tx_msg, fee, receipient_publicKey, nft_type);
    }
    /**
     *
     *  Send an NFT
     ...
     *
     */
    async createSendNFTTransaction(nft, receipient_publicKey) {
        await nft.fetchTransaction();
        return saito_1.default.getInstance().createSendBoundTransaction(BigInt(nft.amount), nft.slip1.utxo_key, nft.slip2.utxo_key, nft.slip3.utxo_key, receipient_publicKey, nft.txmsg);
    }
    /**
     *
     *  Split an NFT
     *
     */
    async createSplitNFTTransaction(nft, leftCount, rightCount) {
        await nft.fetchTransaction();
        return saito_1.default.getInstance().createSplitBoundTransaction(nft.slip1.utxo_key, nft.slip2.utxo_key, nft.slip3.utxo_key, leftCount, rightCount, nft.txmsg);
    }
    /**
     *
     *  Merge an NFT
     *
     */
    async createMergeNFTTransaction(nft) {
        await nft.fetchTransaction();
        return saito_1.default.getInstance().createMergeBoundTransaction(nft.id, nft.txmsg);
    }
    /**
     *
     *  Remove an NFT
     ...
     *
     */
    async createRemoveNFTTransaction(nft) {
        return saito_1.default.getInstance().createRemoveBoundTransaction(nft.slip1.utxo_key, nft.slip2.utxo_key, nft.slip3.utxo_key, nft.txmsg);
    }
    //
    ...
    async onNewBoundTransaction(tx) {
        try {
            console.log('%%% NFT %%%');
            tx.printSlips();
            console.log('%%% %%% %%%');
            if (tx.isTo(this.app.wallet.publicKey)) {
                ...
                if (nft_id)
                    this.app.storage.loadTransactions({ field4: nft_id }, (txs) => {
                        if (txs.length) {

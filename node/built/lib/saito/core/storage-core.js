...
const JSON = __importStar(require("json-bigint"));
const path_1 = __importDefault(require("path"));
const sqlite_1 = require("sqlite");
const sqlite3_1 = __importDefault(require("sqlite3"));
const block_1 = __importDefault(require("../block"));
const slip_1 = __importDefault(require("../slip"));
const slip_2 = require("saito-js/lib/slip");
class StorageCore extends storage_1.default {
    constructor(app, data, dest = 'blocks') {
        super(app);
        this.data_dir = data || path_1.default.join(__dirname, '../../../data');
        this.config_dir = path_1.default.join(__dirname, '../../../config');
        this.dest = dest;
        this.db = [];
        this.dbname = [];
    ...
    async loadBlockFromDisk(filename) {
        try {
            if (fs_extra_1.default.existsSync(filename)) {
                const data = fs_extra_1.default.readFileSync(filename);
    ...
    // }
    //
    async loadBlockByHash(blockHash) {
    ...
    async loadBlockByFilename(filename) {
        try {
            const data = await fs_extra_1.default.readFile(filename);
            const block = new block_1.default();
    ...
    async loadOptions() {
        ...
        // }
        if (fs_extra_1.default.existsSync(`${this.config_dir}/options`)) {
            let optionsfile = '';
            // open options file
            try {
                optionsfile = fs_extra_1.default
    ...
    async loadRuntimeOptions() {
        if (fs_extra_1.default.existsSync(`${this.config_dir}/runtime.config.js`)) {
            //
            // open runtime config file
            //
            try {
                const configfile = fs_extra_1.default.readFileSync(`${this.config_dir}/runtime.config.js`, this.file_encoding_load);
    ...
    saveOptions() {
        ...
        try {
            if (typeof process.env.SAITO_PASS != 'undefined') {
                let secret = this.app.crypto.toBase58(this.app.crypto.stringToHex('BYTHEPRICKINGOFMYTHUMBSSOMETHINGWICKEDTHISWAYCOMES' + process.env.SAITO_PASS));
            ...
            fs_extra_1.default.writeFileSync(`${this.config_dir}/options`, new_wallet_json, null);
            //Update hash
    ...
    //
    returnTokenSupplySlipsFromDisk() {
        let v = [];
        let tokens_issued = 0;
        let filename;
        let contents;
        ...
        contents = fs_extra_1.default.readFileSync(filename);
    ...
    convertIssuanceIntoSlip(line = '') {
        ...
        let type = entries[2];
        let slip = new slip_1.default();
        slip.publicKey = publicKey;
        slip.amount = amount;
        if (type === 'VipOutput') {
            slip.type = slip_2.SlipType.VipOutput;
        }
        if (type === 'Normal') {
            slip.type = slip_2.SlipType.Normal;
        }
        return slip;
    }
    ...
    async saveClientOptions() {
        ...
        //
        try {
            fs_extra_1.default.writeFileSync(`${__dirname}/web/client.options`, JSON.stringify(t));
        }
        catch (err) {
            console.error(err);
        }

...
class Storage {
    ...
    //
    async saveTransaction(tx, obj = {}, peer = null, blk = null) {
        try {
     ...
     *
     */
    async updateTransaction(tx, obj = {}, peer = null, preserve_ts = 0) {
        const message = 'archive';
        let data = {};
        data.request = 'update';
        ...
        if (!obj['updated_at']) {
            if (preserve_ts) {
                obj['updated_at'] = tx.optional.updated_at || tx.timestamp;
            }
        }
        data = Object.assign(data, obj);
        if (peer === 'localhost') {
    ...
    async loadTransactions(obj = {}, mycallback, peer = null, deserialize = 1) {
        ...
        if (peer != null) {
            this.app.network.sendRequestAsTransaction(message, data, (res) => {
                return internal_callback(res);
            }, peer.peerIndex);
        }
        else {
    ...
    watchBuildFile() {
        const checkBuildNumber = async () => {
            const filePath = path_1.default.join(__dirname, '/config/build/build.json');
            fs_1.default.readFile('config/build.json', 'utf8', async (err, data) => {
                if (err) {
        ...
        fs_1.default.watchFile('web/saito/saito.js', { interval: 1000 }, (curr, prev) => {
            checkBuildNumber();
        });
        const filePath = path_1.default.join(__dirname, 'config/build.json');
    }

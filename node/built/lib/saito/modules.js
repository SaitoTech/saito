...
class Mods {
    ...
    async handlePeerTransaction(tx, peer, mycallback = null) {
    ...
    async initialize() {
        ...
        // include events here
        this.app.connection.on('handshake_complete', async (peerIndex) => {
            if (this.app.BROWSER) {
        ...
        this.app.connection.on('peer_disconnect', async (peerIndex, public_key) => {
        ...
        this.app.connection.on('peer_connect', async (peerIndex) => {
    ...
    async onWebSocketServer(webserver) {
        for (let i = 0; i < this.mods.length; i++) {
            ...
            webserver.on('upgrade', (request, socket, head) => {
                const parsedUrl = (0, url_1.parse)(request.url);

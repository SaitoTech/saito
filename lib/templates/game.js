...
Game.prototype.attachEvents = function attachEvents(app) {
  ...
  $('.clicker').on('click', () => {
    ...
    if ($('.hud').css('width') == '60px') {
      $('.hud').animate({width: "15rem", left: '17%'}, 300)
      $('.hud-header').show();
      $('.hud-content').show();
      return;
    }

    if ($('.hud').css('height') != '35px') {
      $('.hud').animate({height: "35px", bottom: "0%"}, 300)
      $('.hud-header').hide();
      $('.hud-content').hide();
      $('.zoombar').hide();
      ...
      let height = $(window).width() > 800 ? "22rem" : "19.5rem"
      // let bottom = $(window).width() > 800 ? '-18%' : '-23%'
      $('.hud').animate({ height }, 300);
    }
  ...
  // $("#sizer, #clicker").on("click", function() {
  //   $("#dragbar").toggleClass("dragbar-tall dragbar-defauly");
  //   $("#hud").toggleClass("tall short");
  //   $("#log").toggleClass("log-small");
  //   $("#status").toggleClass("status-small");
  ...
  $("#pin").on("click", function() {
    $("#pin").toggleClass("fa-thumbtack fa-arrows-alt");
  });
  ...
  //   // HUD Height
  //   //
  //   let log_height = $('.log').css('height');
  //   if (log_height.indexOf("px") > 0) { log_height = log_height.substring(0, log_height.length-2); }
  //   log_height *= 1.05;

  //   let status_height = $('.status').css('height');
  //   if (status_height.indexOf("px") > 0) { status_height = status_height.substring(0, status_height.length-2); }
  //   status_height *= 1.05;

  //   let header_height = $('.hud-header').css('height');
  //   if (header_height.indexOf("px") > 0) { header_height = header_height.substring(0, header_height.length-2); }
  //   header_height *= 1.05;

  //   let max_height = log_height;
  //   if (status_height > log_height) { max_height = status_height; }

  //   $('.hud-header').css('height', max_height);

  //   let old_height = $('.hud').css('height');
  //   let hud_close_delay = 550;
  //   let hud_cancel_close = 0;

  //   $('.hud').mouseenter(function() {
  //     hud_cancel_close = 1;
  //     $(this).animate({ height : max_height }, 200);
  //   });
  //   $('.hud').mouseleave(function() {
  //     hud_cancel_close = 0;
  //     setTimeout(() => {
	// if (hud_cancel_close == 0) {
  //         $(this).animate({ height : old_height }, 200);
	// }
  //     }, hud_close_delay)
  //   });
...
Game.prototype.initializeHTML = function initializeHTML(app) {
  ...
  let html = `
    <div id="hud" class="hud tall">
      <div id="dragbar" class="hud-clickable dragbar dragbar-default">
        <div id="controls" class="controls">
          <span id="sizer" class="fa fa-caret-down clicker"></span>
          <span id="clog" class="clog clicker"></span>
          <span id="clicker" class="clicker">&nbsp;</span>
          <span id="homer" class="fa "></span>
          <span id="pin" class="fa fa-arrows-alt"></span>
        </div>
      </div>
      <div id="zoombar" class="zoombar">
        <div id="zoomup" class="fa fa-plus-circle zoombutton zoomup"></div>
        <div id="zoomdown" class="fa fa-minus-circle zoombutton zoomdown"></div>
      </div>
      ...
      <div class="hud-content">
        <div class="hud_menu_overlay" id="hud_menu_overlay"></div>
  ...
  // // <div id="hud-zoom" class="zoom hud-zoom"></div>
  // // <div id="gameboard" class="gameboard">
  // // </div>
  // <div id="hud" class="hud tall">
  // // <div id="dragbar" class="dragbar dragbar-default">
  // //   <div id="controls" class="controls">
  // //     <span id="sizer" class="fa fa-caret-down clicker"></span>
  // //     <span id="homer" class="fa "></span>
  // //     <span id="clog" class="clog clicker"></span>
  ...
  //   // <div id="zoombar" class="zoombar">
  //   //   <div id="zoomup" class="fa fa-plus-circle zoombutton zoomup"></div>
  //   //   <div id="zoomdown" class="fa fa-minus-circle zoombutton zoomdown">
  //   //   </div>
  //   // </div>

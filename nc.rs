...
// Rust is lovely sometimes, we lose control of 
// network_peer so our saved version is canonical
// and so need to fetch a fresh clone of the version
// we just sent in...
{
...
let network_peer = {
    let mut peers = peers_lock.write().await;
    peers.get_peer_by_id_mut(peer_id).unwrap().clone()
};
            ...
            match receiver {
                PeerReceiver::Warp(mut receiver) => loop {
                    ...
                    if result.is_err() {
                        ...
                        } else if let Some(public_key) = peer.public_key {
                            network_controller
                    ...
                    if result.is_binary() {
                        ...
                        let services = if peer.is_verified {
                            vec![]
                        } else {
                        ...
                        if let Ok(buffer) = result {
                            if !buffer.is_empty() {
                                if let Some(socket) = socket_lock_for_closure.lock().await.as_mut()
                                {
                    ...
                    match result.unwrap() {
                        tokio_tungstenite::tungstenite::Message::Binary(buffer) => {
                            ...
                            if let Ok(buffer) = result {
                                if !buffer.is_empty() {
                                    if let Some(socket) =
                                        socket_lock_for_closure.lock().await.as_mut()
                                    {
                                        NetworkController::send(socket, buffer).await;
                                    }
                                }
...
fn run_websocket_server(
    ...
    // ✅ CRITICAL: create independent clones for each closure
...
//
// passing over network_peer, then grabbing a fresh
// one referencing the old network peer that no
// longer exists. i.e. ownership Rust dance
//
{
    let mut peers = peers_lock.write().await;
    peers.peers_v2.insert(peer_id, network_peer);
